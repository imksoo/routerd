...
| pfSense | 成熟した FreeBSD firewall appliance | appliance 向けで、GUI を中心に設定します | routerd は FreeBSD を Linux と同じ宣言モデルの対象ホストとして扱います |
| MikroTik RouterOS | 低価格な商用 appliance | closed source で、hardware lifecycle に寄ります | routerd は OSS として、汎用 mini PC、VM、lab host で動きます |
...
routerd はすべての appliance UI を置き換えるものではありません。
同じネットワーク意図を Proxmox ラボ、NixOS/FreeBSD ルーター、
Ubuntu 家庭用 gateway、ライブ ISO で起動するディスクレス mini PC の間で
移せることに強みがあります。
