...
`routerctl validate`, `plan`, and `apply` talk to the running `routerd` Unix
socket. For development checks that must not touch the host network, use the
sandbox wrapper shown below; CI's `make validate-example` uses the same path.
