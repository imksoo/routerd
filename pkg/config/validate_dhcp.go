...
func validateDHCPResource(res api.Resource, targetOS platform.OS) (bool, error) {
	...
	case "DHCPv4ServerLeaseSync":
		...
		spec, err := res.DHCPv4ServerLeaseSyncSpec()
		if err != nil {
			return true, err
		}
		...
		source := strings.TrimSpace(spec.Source.Resource)
		if source == "" {
			return true, fmt.Errorf("%s spec.source.resource is required", res.ID())
		}
		if kind, name, ok := strings.Cut(source, "/"); ok {
			if kind != "DHCPv4Server" || strings.TrimSpace(name) == "" {
				return true, fmt.Errorf("%s spec.source.resource must reference DHCPv4Server", res.ID())
			}
		}
	...
	case "DHCPv6ServerLeaseSync":
		...
		spec, err := res.DHCPv6ServerLeaseSyncSpec()
		if err != nil {
			return true, err
		}
		if strings.TrimSpace(spec.Interval) != "" {
		...
		if kind, name, ok := strings.Cut(source, "/"); ok {
			if kind != "DHCPv6Server" || strings.TrimSpace(name) == "" {
				return true, fmt.Errorf("%s spec.source.resource must reference DHCPv6Server", res.ID())
			}
		}
		if len(spec.Targets) == 0 {
			return true, fmt.Errorf("%s spec.targets is required", res.ID())
	...
	case "DHCPv6PrefixDelegationLeaseSync":
		...
		spec, err := res.DHCPv6PrefixDelegationLeaseSyncSpec()
		if err != nil {
			return true, err
		}
		if strings.TrimSpace(spec.Interval) != "" {
		...
		if kind, name, ok := strings.Cut(source, "/"); ok {
			if kind != "DHCPv6PrefixDelegation" || strings.TrimSpace(name) == "" {
				return true, fmt.Errorf("%s spec.source.resource must reference DHCPv6PrefixDelegation", res.ID())
			}
		}
		if len(spec.Targets) == 0 {
			return true, fmt.Errorf("%s spec.targets is required", res.ID())
