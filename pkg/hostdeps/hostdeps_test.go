...
func TestKernelModulesForFreeBSDUsePFRuntimeModules(t *testing.T) {
	...
	if got, want := KernelModulesForOS(router, platform.OSFreeBSD), []string{"ng_pppoe", "pf", "pflog"}; !reflect.DeepEqual(got, want) {
	...
	if !spec.Persistent || !reflect.DeepEqual(spec.Modules, []string{"ng_pppoe", "pf", "pflog"}) {
