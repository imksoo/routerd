...
func DHCP6CWithLeases(router *api.Router, binaryPath, configDir, runtimeDir, unitDir string, leases map[string]routerstate.PDLease) (DHCP6CConfig, error) {
	...
	for _, res := range resources {
		...
		hookPath := filepath.Join(configDir, "dhcp6c-"+name+".hook")
		pidPath := filepath.Join(runtimeDir, "dhcp6c-"+name+".pid")
		socketPath := filepath.Join(runtimeDir, "routerd.sock")
		...
		data, err := dhcp6cConfig(router, aliases, pd, hookPath)
		if err != nil {
			return DHCP6CConfig{}, err
		}
		config.Files = append(config.Files,
			ManagedFile{Path: confPath, Data: data, Perm: 0644},
			ManagedFile{Path: hookPath, Data: DHCP6CHookScript(res.Metadata.Name, socketPath), Perm: 0755},
...
func dhcp6cConfig(router *api.Router, aliases map[string]string, pd freeBSDPD, hookPath string) ([]byte, error) {
	...
	if hookPath != "" {
		buf.WriteString(fmt.Sprintf("  script \"%s\";\n", hookPath))
	}
	buf.WriteString("};\n\n")
...
func DHCP6CHookScript(resourceName, socketPath string) []byte {
	return dhcp6EventHookScript(resourceName, socketPath)
}
