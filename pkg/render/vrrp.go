...
func KeepalivedConfigWithOptions(router *api.Router, aliases map[string]string, opts KeepalivedOptions) ([]byte, error) {
	...
	// VRRP state transitions own the failover VMACs. Keepalived otherwise
	// suppresses notify scripts under its script-security policy.
	buf.WriteString("  enable_script_security\n")
	buf.WriteString("  script_user root\n")
	buf.WriteString("}\n\n")
...
func vrrpInstances(router *api.Router, aliases map[string]string, opts KeepalivedOptions) ([]vrrpInstance, error) {
	...
	for _, res := range router.Spec.Resources {
		...
		failoverVMACs := append([]api.VirtualAddressVRRPFailoverVMACSpec(nil), spec.VRRP.AdditionalFailoverVMACs...)
		if spec.VRRP.FailoverVMAC != nil {
			failoverVMACs = append([]api.VirtualAddressVRRPFailoverVMACSpec{*spec.VRRP.FailoverVMAC}, failoverVMACs...)
		}
		for i := range failoverVMACs {
			parent := aliases[failoverVMACs[i].ParentInterface]
			if parent == "" {
				return nil, fmt.Errorf("%s spec.vrrp.failoverVMAC parentInterface references interface with empty ifname %q", res.ID(), failoverVMACs[i].ParentInterface)
			}
			failoverVMACs[i].ParentInterface = parent
		}
