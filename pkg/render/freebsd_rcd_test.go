...
func TestFreeBSDRenderRoutesDHCPv6ClientThroughRouterd(t *testing.T) {
	...
	if strings.Contains(script, `'/usr/local/sbin/routerd' 'check'`) {
		t.Fatalf("routerd rc.d script must not run deprecated check verb:\n%s", script)
	}
...
func TestFreeBSDRenderSkipsDHCPClientRCDWhenRouterdSupervisesClients(t *testing.T) {
	...
	if !strings.Contains(routerdScript, "'serve'") || strings.Contains(routerdScript, "'check'") || strings.Contains(routerdScript, "--skip-service-manager") {
		t.Fatalf("routerd rc.d script must run serve only without skip-service-manager:\n%s", routerdScript)
	}
