...
func NftablesIPv4SourceNAT(router *api.Router) ([]byte, error) {
	...
	if len(firewallPolicies) > 0 {
		if err := writeFirewallFilterTable(&buf, aliases, zones, firewallPolicies, exposeServices); err != nil {
			return nil, err
		}
	}
...
func writeFirewallFilterTable(buf *bytes.Buffer, aliases map[string]string, zones []api.Resource, policies []api.Resource, exposes []api.Resource) error {
