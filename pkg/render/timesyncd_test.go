...
func TestTimesyncdConfigUsesFallbackForAutoSource(t *testing.T) {
	router := &api.Router{
		Spec: api.RouterSpec{Resources: []api.Resource{
			{
				TypeMeta: api.TypeMeta{APIVersion: api.SystemAPIVersion, Kind: "NTPClient"},
	...
	got := string(data)
	if !strings.Contains(got, "NTP=ntp.jst.mfeed.ad.jp ntp.nict.jp\n") {
		t.Fatalf("unexpected timesyncd config:\n%s", got)
	}
	if !strings.Contains(got, "FallbackNTP=ntp.jst.mfeed.ad.jp ntp.nict.jp\n") {
		t.Fatalf("unexpected fallback config:\n%s", got)
	}
}
