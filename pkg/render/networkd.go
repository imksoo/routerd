...
func NetworkdDropins(router *api.Router) ([]File, error) {
	...
	files, err := NetworkdBridgeFiles(router)
	if err != nil {
		return nil, err
	}
...
// owned by Bridge resources. Controllers use this narrow view so applying a
// bridge cannot accidentally write unrelated route, NTP, or VXLAN artifacts.
func NetworkdBridgeFiles(router *api.Router) ([]File, error) {
	if router == nil {
		return nil, nil
	}
	router = api.ExpandClusterNetworkRoutes(router)
...
func networkdAdditionalDropins(router *api.Router, aliases map[string]string, files []File) ([]File, error) {
