...
func TestClientIdentityDigestExcludesRROwnedAdmissionFields(t *testing.T) {
	claim := api.SAMEnrollmentClaimSpec{
		PolicyRef:     "SAMEnrollmentPolicy/cloudedge-leaves",
		RRSetRef:      "SAMRRSet/cloudedge-rrs",
		LeafID:        "leaf-b",
	...
	claim.JoinHMAC = JoinHMAC([]byte("test-join-token"), claim)
	identity := ClientIdentityDigest(claim)
	claim.ExpiresAt = "2026-06-28T01:00:00Z"
	claim.Revoked = true
	if got := ClientIdentityDigest(claim); got != identity {
		t.Fatalf("RR-owned admission fields changed identity: %q != %q", got, identity)
	}
	claim.Revoked = false
	claim.JoinNonce = "nonce-2"
	claim.JoinHMAC = JoinHMAC([]byte("test-join-token"), claim)
	if got := ClientIdentityDigest(claim); got == identity {
		t.Fatalf("rotated client identity did not change: %q", got)
	}
}
