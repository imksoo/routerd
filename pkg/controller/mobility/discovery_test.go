...
func TestDiscoveryControllerIgnoresARPObservationWhenRemoteProviderClaimExists(t *testing.T) {
		...
		Members: []api.MobilityPoolMember{
			{
				...
				OwnershipDiscovery: api.MobilityOwnershipDiscovery{
					Mode: "onprem-l2",
					Sources: []api.MobilityOwnershipDiscoverySource{
						{Type: OnPremSourceARPObserver, Interface: "eth-a", LeaseTTL: "2m"},
					},
				},
			},
			{
	...
	if err := store.RecordFederationEvent(routerstate.EventRecord{
		ID:         "provider:aws-leaf-a:10.77.60.11",
		Group:      "cloudedge",
		SourceNode: "aws-leaf-a",
		Type:       ObservedEventType,
		Subject:    "10.77.60.11/32",
		DedupeKey:  "mobility:provider-discovery:cloudedge:aws-leaf-a:10.77.60.11_32",
	...
	members := plannerMembers(spec.Members)
	self, ok := lookupMemberByNodeRef(members, "pve-leaf-a")
	if !ok {
		t.Fatal("missing self member")
	}
	controller := DiscoveryController{Store: store, Now: func() time.Time { return now }}
	changed, err := controller.recordOnPremObservation("cloudedge", spec, self, netip.MustParsePrefix("10.77.60.0/24"), onPremObservation{
		Action:     "observed",
		Address:    "10.77.60.11",
		MAC:        "bc:24:11:37:be:b1",
		Interface:  "eth-a",
	...
	if changed {
		t.Fatal("ARP observation must be suppressed when remote provider-discovery claim exists")
	}
	...
	if err != nil {
		t.Fatalf("recordOnPremObservation client: %v", err)
	}
	if !changed {
		t.Fatal("ARP observation for address without remote claim should still be recorded")
	}
	events, err := store.ListFederationEvents("cloudedge", false, now.Unix())
	if err != nil {
		t.Fatalf("ListFederationEvents: %v", err)
	...
	for _, ev := range events {
		if ev.Payload["source"] == "onprem-l2-discovery" {
			subjects = append(subjects, ev.Subject)
		}
	}
	if len(subjects) != 1 || subjects[0] != "10.77.60.12/32" {
		t.Fatalf("onprem-l2-discovery subjects = %v, want [10.77.60.12/32] only", subjects)
	}
}
