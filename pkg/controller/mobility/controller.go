...
func (c Controller) reconcileBGPDelivery(ctx context.Context, res api.Resource, spec api.MobilityPoolSpec, memberSetStatus map[string]any, now time.Time) error {
	...
	if err := c.recordBGPCaptureAssignmentTransitions(ctx, res.Metadata.Name, self, previousCaptureAssignments, delivery.CaptureAssignments, append(actionPlans, previousActionPlans...), ownerPlacement, livenessMarkers, mobilityPrefixCommunities, ownershipDecisions, actionJournal, poolStatus, status, now); err != nil {
		return err
	}
