...
// ---------------------------------------------------------------------------
// actionJournalAssignedAddresses tests (#631 regression)
// ---------------------------------------------------------------------------
...
func (f *fakeActionLister) ListActions(routerstate.ActionExecutionFilter) ([]routerstate.ActionExecutionRecord, error) {
	return f.actions, nil
}

func actionTargetJSON(addr string) string {
...
func TestActionJournalAssignNotHiddenByEnsureForwarding(t *testing.T) {
	store := &fakeActionLister{actions: []routerstate.ActionExecutionRecord{
		...
		{ID: 2, ProviderRef: "p1", Action: "ensure-forwarding-enabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
	}}
	got := actionJournalAssignedAddresses(store)
	if len(got) != 1 || got[0] != "10.0.0.1" {
		t.Fatalf("got %v, want [10.0.0.1]; ensure-forwarding must not mask assign", got)
	}
}

func TestActionJournalUnassignWinsOverAssign(t *testing.T) {
	store := &fakeActionLister{actions: []routerstate.ActionExecutionRecord{
		{ID: 1, ProviderRef: "p1", Action: "assign-secondary-ip", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 2, ProviderRef: "p1", Action: "ensure-forwarding-enabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 3, ProviderRef: "p1", Action: "unassign-secondary-ip", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
	}}
	got := actionJournalAssignedAddresses(store)
	if len(got) != 0 {
		t.Fatalf("got %v, want []; unassign must override assign for drain", got)
	}
}

func TestActionJournalIgnoresUnrelatedActions(t *testing.T) {
	store := &fakeActionLister{actions: []routerstate.ActionExecutionRecord{
		{ID: 1, ProviderRef: "p1", Action: "assign-secondary-ip", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 2, ProviderRef: "p1", Action: "ensure-forwarding-enabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 3, ProviderRef: "p1", Action: "ensure-forwarding-disabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 4, ProviderRef: "p1", Action: "update-route-table", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
	}}
	got := actionJournalAssignedAddresses(store)
	if len(got) != 1 || got[0] != "10.0.0.1" {
		t.Fatalf("got %v, want [10.0.0.1]; non-assign/unassign actions must be ignored", got)
	}
}

func TestActionJournalAssignSurvivesRepeatedCleanupCycles(t *testing.T) {
	store := &fakeActionLister{actions: []routerstate.ActionExecutionRecord{
		{ID: 1, ProviderRef: "p1", Action: "assign-secondary-ip", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 2, ProviderRef: "p1", Action: "ensure-forwarding-enabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 3, ProviderRef: "p1", Action: "ensure-forwarding-disabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 4, ProviderRef: "p1", Action: "ensure-forwarding-enabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
		{ID: 5, ProviderRef: "p1", Action: "ensure-forwarding-disabled", Status: routerstate.ActionSucceeded, TargetJSON: actionTargetJSON("10.0.0.1")},
	}}
	for i := 0; i < 5; i++ {
		got := actionJournalAssignedAddresses(store)
		if len(got) != 1 || got[0] != "10.0.0.1" {
