...
func TestDnsmasqPreviousDelegatedPrefixWithdrawalSurvivesConfigRewriteAndExpires(t *testing.T) {
	router := &api.Router{Spec: api.RouterSpec{Resources: []api.Resource{
		...
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "IPv6DelegatedAddress"}, Metadata: api.ObjectMeta{Name: "lan-base"}, Spec: api.IPv6DelegatedAddressSpec{
			PrefixDelegation: "wan-pd", Interface: "lan-vmac", AddressSuffix: "::1",
		}},
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "DHCPv6Server"}, Metadata: api.ObjectMeta{Name: "lan-v6"}, Spec: api.DHCPv6ServerSpec{
			Interface: "lan-vmac", Mode: "stateless", LeaseTime: "12h",
		}},
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "IPv6RouterAdvertisement"}, Metadata: api.ObjectMeta{Name: "lan-ra"}, Spec: api.IPv6RouterAdvertisementSpec{
			Interface: "lan-vmac", PrefixFrom: api.StatusValueSourceSpec{Resource: "IPv6DelegatedAddress/lan-base", Field: "address"},
		}},
	}}}
	statusKey := api.NetAPIVersion + "/IPv6DelegatedAddress/lan-base"
	now := time.Now().UTC()
	store := mapStore{
		statusKey: {
			"phase":   "Applied",
			...
			"previousAddresses": []map[string]any{
				...
				{"address": "2409:10:3d60:1241::99/64", "expiresAt": now.Add(30 * time.Minute)},
				{"address": "2409:10:3d60:1231::1/64", "expiresAt": now.Add(-time.Minute)},
				{"address": "2409:10:3d60:1221::1/64", "expiresAt": now.Add(time.Hour)},
				{"address": "2409:10:3d60:1241::23/128", "expiresAt": now.Add(time.Hour)},
			},
		},
	}
	dir := t.TempDir()
	...
	changed, _, err := writeDnsmasqConfig(router, store, configPath, pidFile, 53, nil)
	if err != nil || !changed {
		t.Fatalf("initial dnsmasq write changed=%t err=%v", changed, err)
	}
	assertConfig := func(wantPrevious bool) {
		t.Helper()
		...
		got := string(data)
		if want := "dhcp-range=set:lan-v6,::,constructor:lan-vrrp,ra-stateless,64,12h"; !strings.Contains(got, want) {
			t.Fatalf("current constructor PIO disappeared, missing %q:\n%s", want, got)
		}
		withdrawal := "dhcp-range=2409:10:3d60:1241::,ra-only,64,deprecated"
		if strings.Contains(got, withdrawal) != wantPrevious {
			t.Fatalf("previous-prefix withdrawal presence=%t, want %t:\n%s", strings.Contains(got, withdrawal), wantPrevious, got)
		}
		for _, forbidden := range []string{
			"dhcp-range=2409:10:3d60:1231::,ra-only,64,deprecated",
			"dhcp-range=2409:10:3d60:1221::,ra-only,64,deprecated",
			"2409:10:3d60:1241::23",
		} {
			if strings.Contains(got, forbidden) {
				t.Fatalf("expired, current, or DS-Lite /128 prefix was advertised by %q:\n%s", forbidden, got)
			}
		}
		wantCount := 0
		if wantPrevious {
		...
		if count := strings.Count(got, withdrawal); count != wantCount {
			t.Fatalf("withdrawal line count=%d, want %d:\n%s", count, wantCount, got)
		}
	}
	assertConfig(true)

	// A daemon restart consumes this same generated file. Re-rendering without
	// status changes must keep the explicit deprecated range rather than relying
	// on dnsmasq's in-process constructor history.
	changed, _, err = writeDnsmasqConfig(router, store, configPath, pidFile, 53, nil)
	if err != nil || changed {
		t.Fatalf("stable dnsmasq rewrite changed=%t err=%v", changed, err)
	}
	assertConfig(true)

	store[statusKey]["previousAddresses"] = []map[string]any{
		{"address": "2409:10:3d60:1241::1/64", "expiresAt": now.Add(-time.Minute)},
		{"address": "2409:10:3d60:1241::23/128", "expiresAt": now.Add(time.Hour)},
	}
	changed, _, err = writeDnsmasqConfig(router, store, configPath, pidFile, 53, nil)
	if err != nil || !changed {
		t.Fatalf("expired withdrawal rewrite changed=%t err=%v", changed, err)
	}
	assertConfig(false)
}
...
func TestDnsmasqHostsReloadFailureRestoresSidecarForNextReconcileRetry(t *testing.T) {
	...
	router := &api.Router{Spec: api.RouterSpec{Resources: []api.Resource{
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "Interface"}, Metadata: api.ObjectMeta{Name: "lan"}, Spec: api.InterfaceSpec{IfName: "ens19"}},
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "DHCPv4Server"}, Metadata: api.ObjectMeta{Name: "lan-v4"}, Spec: api.DHCPv4ServerSpec{
			Interface: "lan", LeaseFile: filepath.Join(dir, "state", "dnsmasq.leases"), AddressPool: api.DHCPAddressPoolSpec{Start: "192.168.10.100", End: "192.168.10.199", LeaseTime: "8h"},
		}},
		{TypeMeta: api.TypeMeta{APIVersion: api.NetAPIVersion, Kind: "DHCPv4Reservation"}, Metadata: api.ObjectMeta{Name: "printer"}, Spec: api.DHCPv4ReservationSpec{
...
func TestLANAddressControllerRemovesWhenFalseVRRPAddressesDespiteFreshSnapshot(t *testing.T) {
	...
	for name, address := range map[string]string{
		...
		if name == "lan-base" {
			status["previousAddresses"] = []managedPreviousDelegatedAddress{{
				Address: "2001:db8:1200:1241::1/64", ExpiresAt: now.Add(time.Hour),
			}}
		}
		if err := store.SaveObjectStatus(api.NetAPIVersion, "IPv6DelegatedAddress", name, status); err != nil {
			t.Fatal(err)
		}
	}
	if err := (&Runner{Router: declared}).saveWhenFalseStatuses(eventedStore{Store: store, Router: declared}); err != nil {
	...
	for _, want := range []string{
		...
		"ip -6 addr del 2001:db8:1200:1241::1/64 dev lan-vrrp",
	...
	for _, name := range []string{"lan-base", "wan-source"} {
		...
		if status["phase"] != "Pending" || status["reason"] != "WhenFalse" || status["address"] != nil || status["staged"] != nil || status["previousAddresses"] != nil {
