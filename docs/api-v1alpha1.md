...
| `SAMEnrollmentClient` | Runs leaf-side bootstrap/refresh: submits its local remote-request `SAMEnrollmentClaim` to bootstrap RR endpoints, fetches the allowed RRSet plus any eligible direct peer group as one topology snapshot, and persists it as dynamic state only when missing, near expiry, or claim material changes. Direct peers require every RR to attest the same client identity and topology; `joinTokenFrom` authenticates that identity, while token-less enrollment is trusted control-plane input. After an RR loses only volatile admission state, it re-admits automatically only from the identity-aware current-RR response; revoke, a different active identity during renewal, an old/mixed RR, or any uncertainty retains RR fallback. It does not require a local policy or NodeSet. |
...
| `SAMEnrollmentClient` | `backoff` (string), `claimRef` (string), `lastAttempt` (timestamp), `lastSuccess` (timestamp), `nextAttempt` (timestamp), `observedDirectPeerGroup` (string), `observedRRSet` (string), `phase` (string), `reason` (string) |
