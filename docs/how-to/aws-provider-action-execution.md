...
- [ ] `routerd-cloud` does **NOT** retain a BGP provider-secondary address as a
      local OS address, even while the provider secondary IP remains assigned on
      the ENI. Capture is provider ingress plus route/forwarding state, not a
      Linux local `/32`.
- [ ] `MobilityPool` reports a healthy capture plan.
