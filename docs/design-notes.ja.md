...
### DHCPv6 クライアントの Renew パケット比較

今回の失敗は、「Linux の DHCPv6 クライアントは NTT のプレフィックス委譲を
更新できない」という単純な話では説明できません。見るべき点は、各クライアントが
Renew/Rebind の IA_PD をどう作るか、リース状態をどれだけ routerd へ渡せるか、
同じクライアントを Linux と FreeBSD の両方で使えるかです。

確認した資料:

- [RFC 8415](https://www.rfc-editor.org/rfc/rfc8415.html)。主に 18.2.4 節と
  21.22 節。
- [systemd `sd-dhcp6-client.c`](https://github.com/systemd/systemd/blob/main/src/libsystemd-network/sd-dhcp6-client.c)
  と [`dhcp6-option.c`](https://github.com/systemd/systemd/blob/main/src/libsystemd-network/dhcp6-option.c)。
- [OpenWrt `odhcp6c` の `dhcpv6.c`](https://github.com/openwrt/odhcp6c/blob/master/src/dhcpv6.c)
  と、NTT 10G 光の DHCPv6-PD 更新問題を扱う OpenWrt の
  [issue #13454](https://github.com/openwrt/openwrt/issues/13454)。
- [dhcpcd の `dhcp6.c`](https://github.com/NetworkConfiguration/dhcpcd/blob/master/src/dhcp6.c)
  と [`dhcpcd.conf(5)`](https://man.archlinux.org/man/dhcpcd.conf.5.en)。
- [OPNsense/WIDE `dhcp6c`](https://github.com/opnsense/dhcp6c) の
  `dhcp6c.c`、`dhcp6c_ia.c`、`prefixconf.c`、`common.c`。
- [systemd `systemd.network` 文書ソース](https://github.com/systemd/systemd/blob/main/man/systemd.network.xml)。

寿命値については RFC 8415 がはっきりしています。Renew は、更新対象の
委譲プレフィックスを IA_PD と IA Prefix に入れて送ります。一方で 21.22 節は、
クライアントからサーバーへ送る IA Prefix の優先寿命と有効寿命は 0 にすべきで、
サーバーは受け取った値を無視しなければならない、としています。説明文でも、
昔の寿命ヒントは誤実装を招き、残り寿命が減っていく値をサーバーが誤解する問題が
あったため、現在は 0 を送るべきだと整理されています。したがって、router02 と
router03 の Renew/Rebind で寿命が 0 だったこと自体は RFC 違反ではありません。

| クライアント | Renew/Rebind の IA Prefix 寿命 | 関連する挙動 | routerd での意味 |
| --- | --- | --- | --- |
| systemd-networkd | 0 を送ります。`dhcp6_option_append_ia()` は IA_NA/IA_PD の T1/T2 を入れず、`option_append_pd_prefix()` はプレフィックス長とアドレスだけをコピーします。 | `UseDelegatedPrefix`、`UseAddress`、`PrefixDelegationHint`、`IAID`、`DUIDType`、`DUIDRawData`、`WithoutRA` は、PD を求めるか、識別子、ヒント、RA を待つかを制御します。IA Prefix の寿命値の出し方は変えません。 | `.network` の小さな設定変更だけで、Renew に 0 以外の IA Prefix 寿命を入れる道は見つかりません。単純な Linux PD には使えますが、routerd から見ると通知の仕組みとパケット単位の観測性が弱いです。 |
| OpenWrt odhcp6c | Renew/Rebind では 0 を送ります。Advertise/Reply から Request を作る時だけ寿命をコピーし、Renew/Rebind では現在の PD 状態を使いつつ IA Prefix の寿命欄は初期値 0 のままです。 | ルーター用途を強く意識しており、状態が変わるとスクリプトを呼べます。一方、OpenWrt issue #13454 では、NTT 10G 光で PD 更新に失敗し、odhcp6c の寿命 0 Renew と ISP ルーターの 0 ではない Renew の違いが報告されています。 | odhcp6c は Linux ルーター向けには有力ですが、NTT で必ず更新できる証拠にはなりません。OpenWrt 中心で、FreeBSD の通常の導入経路もありません。 |
| dhcpcd | Renew/Rebind では 0 を送ります。`dhcp6.c` では IA Prefix 用の領域を 0 で初期化し、その後にプレフィックス長とアドレスだけを入れます。 | `ia_pd`、`iaid`、`duid`、`script`、フックに対応し、Linux と FreeBSD の両方でパッケージがあります。マニュアルでもプレフィックス委譲とフック実行が説明されています。 | systemd-networkd の DHCPv6 と KAME `dhcp6c` の両方を、1 つの管理対象クライアントへ置き換えられる可能性があります。ただし Renew の寿命値は RFC 8415 型の 0 なので、PR-400NE で実際に更新できるかを先に試す必要があります。 |
| WIDE/KAME dhcp6c | 通常の Renew/Rebind 経路では 0 ではない寿命を送るように見えます。`prefixconf.c` が保存済みの site prefix から Renew/Rebind データを作り、`common.c` が `pltime` と `vltime` を IA Prefix へ書き出します。 | FreeBSD で現在使っている系統です。ただし手動 HUP 検証では本当の Renew ではなく Solicit へ戻ったため、このソース上の挙動は検証環境のパケットではまだ確認できていません。 | この比較では、動作中 ISP ルーターの「0 ではない寿命を送る」形に近い可能性がある唯一のクライアントです。PR-400NE が RFC 8415 と違ってこの古い形を好むなら、WIDE/KAME やその修正版は参考になります。 |

OpenWrt の NTT 10G 光の報告は重要です。「OpenWrt が動くなら odhcp6c でよい」
という単純な仮定を弱めます。その報告では、PD の有効寿命切れ後に問題が起き、
Renew パケットが ISP ルーターと違うとされています。そこで示されている差は
ソースとも一致しており、odhcp6c は Renew 時に IA Prefix の寿命を 0 で送ります。
これは RFC 8415 では正しい動きですが、脆い ISP 側実装やホームゲートウェイでは、
古い「0 ではない寿命」を送る形の方が通る可能性があります。

現時点の実用面での比較は次の通りです。

- odhcp6c は OpenWrt でのルーター連携が最も自然です。ただし、この用途では
  Linux 専用に近く、NTT での更新失敗報告もあります。
- dhcpcd は、Linux と FreeBSD で同じ管理対象クライアントにできる可能性が
  最も高いです。安定した DUID/IAID、プレフィックスヒント、routerd が読める
  フックを扱えます。ただし Renew の寿命値は RFC 8415 に沿った 0 なので、
  移行前に PR-400NE で検証します。
- WIDE/KAME は全環境でそろえるには弱いですが、通常の Renew パケットは
  動作中の商用ルーターに近い可能性があります。dhcpcd が検証環境の HGW で
  安定更新できない場合、FreeBSD では WIDE/KAME 互換の修正を残す案が
  現実的です。

今の推奨は、odhcp6c にすぐ寄せることではありません。まず router02 か router03 で
dhcpcd を routerd 管理下の DHCPv6 クライアントとして試します。これなら、
Linux 専用のクライアントへ寄せる前に、共通化できる道を検証できます。試作では
WAN 側の systemd-networkd DHCPv6 を止め、明示的な DUID-LL、安定 IAID、
/60 の `ia_pd` ヒント、routerd が望まない自動 LAN アドレス設定の抑止、
専用フックスクリプトを使います。フックからは、委譲プレフィックス、サーバー識別子、
T1/T2、優先寿命、有効寿命、発生した理由を routerd の状態へ渡します。並行して
FreeBSD では WIDE/KAME の本当の Renew をパケット取得し、0 ではない寿命を
送るか、PR-400NE が Reply を返すかを確認します。

段階的な移行案:

1. 設計上の項目として `dhcpv6Client.backend` を追加し、
   `systemd-networkd`、`odhcp6c`、`dhcpcd`、`kame-dhcp6c` を選べる形にする。
   既存の動作はまだ変えない。
2. 検証用の枝で `dhcpcd` の出力と監視だけを実装する。管理対象の設定ファイル、
   systemd unit、FreeBSD rc.d サービス、routerd 管理のフックスクリプトを作る。
3. まず router02 で dhcpcd を試す。Solicit、Request、Renew、Rebind、Reply、
   DUID、IAID、プレフィックスヒント、寿命値を既存の systemd-networkd 取得と比べる。
4. PR-400NE が dhcpcd の Renew も同じように無視するなら、自前クライアントや
   WIDE 修正へ進む前に、router01 で WIDE/KAME の自然な Renew を確認する。
5. その取得結果を見てから、NTT 向けプロファイルの既定を dhcpcd、odhcp6c、
   または明示的に修正した WIDE/KAME 経路のどれにするか決める。

### 2026-04-29 KAME 自然 Renew 検証

router01 は、前回の HUP 検証の影響を外すために次の手順で整理しました。

- `routerd` を停止
- `dhcp6c` を停止
- `/var/run/dhcp6c.pid` を削除
- `vtnet0` でパケット取得を開始
- `routerd` を再起動

最初の routerd 管理起動では新しい `dhcp6c` プロセスは作られましたが、
新しい取得ファイルには DHCPv6 パケットが出ませんでした。そのため、同じ生成設定を
使って `dhcp6c` を 20 秒だけ前景でデバッグ起動し、送信内容を確認しました。
...
デバッグ出力と対応する pcap から読み取れた結果:

| 確認点 | 観測結果 |
| --- | --- |
| 送信したメッセージ種別 | `dhcp6c -D -f` は Renew ではなく Solicit を送りました。デバッグログには `Sending Solicit` と `send solicit to ff02::1:2%vtnet0` が繰り返し出ています。 |
| IA_PD prefix の寿命欄 | Solicit には `IA_PD IAID:0 T1:0 T2:0` と、`2409:10:3d60:1220::/60` の正確なヒントが入りました。IA_PD prefix の優先寿命と有効寿命はどちらも `4294967295`、つまり無限大です。KAME のヒント付き Solicit 経路は、0 ではない寿命値を送ることが確認できました。 |
| HGW からの Advertise / Reply | 20 秒間のデバッグ起動中、Advertise も Reply も取れていません。pcap にあるのは外向き Solicit だけです。 |
| Reply 内の IA_PD prefix と新しい寿命 | Reply が無いため、新しい委譲プレフィックスや寿命値は確認できません。 |

決定的な自然 Renew 検証は、まだ始められていません。前提となる clean restart 後の
新規取得が成立しなかったためです。KAME は、正確なヒントと無限大の IA Prefix 寿命を
入れた Solicit を繰り返す状態に戻りました。この結果は、それでも重要です。
systemd-networkd や odhcp6c と違い、KAME のヒント付き Solicit 経路では
IA Prefix の寿命欄に 0 ではない値が入ることが、パケット単位で確認できました。
一方で、KAME の本当の T1 Renew がどういうパケットになるか、PR-400NE が
0 ではない寿命値を持つ Renew に Reply を返すかは、まだ確認できていません。

次の試行では、router01 が Advertise、Request、Reply の交換で本当に bound 状態に
入ったことを確認してから、T1 まで一切刺激せずに待つ必要があります。`dhcp6c -D -f`
はパケット確認には有用ですが、2 時間の自然 Renew 窓では使いません。前景実行の
timeout 自体が T1 前にクライアントを終了させてしまうためです。

追加実装として、FreeBSD の KAME `dhcp6c` レンダラを変更し、NTT 系プロファイルでは
プレフィックスヒントの寿命を既定で有限値にしました。

```text
prefix 2409:10:3d60:1220::/60 14400 14400;
```

`IPv6PrefixDelegation.spec.preferredLifetime` と `spec.validLifetime` を使うと、
検証用に別の値を指定できます。router01 での実機確認では、設定した値が
パケット上にも出ていることを確認しました。

| ヒントの寿命 | パケット上の結果 | HGW の応答 |
| --- | --- | --- |
| `14400 14400` | `2409:10:3d60:1220::/60` の Solicit に `pltime:14400 vltime:14400` が入りました。 | 取得中に Advertise / Reply は見えませんでした。 |
| `7200 7200` | Solicit に `pltime:7200 vltime:7200` が入りました。 | 取得中に Advertise / Reply は見えませんでした。 |
| `86400 86400` | Solicit に `pltime:86400 vltime:86400` が入りました。 | 取得中に Advertise / Reply は見えませんでした。 |

これで、PR-400NE が KAME の `infinity` ヒントだけを嫌っている、という単純な仮説は
弱くなりました。有限の寿命値は正しく出力され、実際に送信されています。それでも、
現在のリース表の状態では、HGW は新規 Solicit に応答していません。残る重要な検証は、
値を変えた Solicit ではなく、bound 状態に入ったクライアントからの本当の Renew です。
7200 と 86400 の一時検証後、router01 の router.yaml は元に戻しました。新しい既定値により、
NTT 系プロファイルでは `14400 14400` が出力されます。
...
### 2026-04-29 PR-400NE 検証環境のパケット比較

Proxmox ブリッジのマルチキャスト制御を無効化し、検証経路上のスイッチでも
同種の制御を外した後、router01 と router03 は PR-400NE からプレフィックスを
再取得しました。

| ルーター | クライアント | 結果 |
| --- | --- | --- |
| router01 / FreeBSD | KAME `dhcp6c` | `2409:10:3d60:1220::/60` を取得。LAN 側 `vtnet1` に `2409:10:3d60:1220::1/64` が付きました。 |
| router02 / NixOS | odhcp6c 実験後、systemd-networkd へ戻し | 現在の委譲プレフィックスはまだありません。`2409:10:3d60:1230::/60` は状態データベース内の過去値です。 |
| router03 / Ubuntu | systemd-networkd | `2409:10:3d60:1240::/60` を取得。LAN 側 `ens19` に `2409:10:3d60:1240::3/64` が付きました。 |

同じ取得ファイルに、動作中の NEC IX2215 の交換も入りました。`tcpdump` で読める
範囲の DHCPv6 フィールドを並べると、Solicit は次の通りです。

| 項目 | IX2215 | router01 / KAME | router03 / networkd | router02 / odhcp6c 実験 |
| --- | --- | --- | --- | --- |
| 送信元 MAC | `00:60:b9:f9:73:cf` | `bc:24:11:e3:c2:38` | `bc:24:11:40:32:de` | `bc:24:11:30:5d:76` |
| DUID 種別 | DUID-LL、ハードウェア種別 1 | DUID-LL、ハードウェア種別 1 | DUID-LL、ハードウェア種別 1 | DUID-LL、ハードウェア種別 1 |
| DUID 値 | `0060b9f973cf` | `bc2411e3c238` | `bc24114032de` | `bc2411305d76` |
| IA_PD IAID | `1568088` | `0` | `3394439514` | `1` |
| プレフィックスヒント | Solicit には無し | 正確な `/60` ヒント、`1220::/60` | 正確な `/60` ヒント、`1240::/60` | 正確な `/60` ヒント、`1230::/60` |
| ヒントの寿命 | Solicit には無し | `14400/14400` | `0/0` | `0/0` |
| 要求オプション | 無し | DNS のみ | DNS、SNTP、NTP、option 82、option 103 | SIP ドメイン、SIP アドレス、DNS、DNS 検索、SNTP、NTP、option 67、option 82、option 103 |
| 経過時間 | あり | あり | あり | あり |
| 再設定許可 | あり | 無し | 無し | あり |
| ベンダークラス | 無し | 無し | 無し | 無し |
| ベンダー固有情報 | 無し | 無し | 無し | 無し |
| クライアント FQDN | 無し | 無し | あり | 無し |
| Rapid Commit | 無し | 無し | 無し | 無し |
| IPv6 ホップ上限 | 64 | 1 | 1 | 1 |

もっとも大きな違いは DUID ではありません。現在は全クライアントが DUID-LL を
送っています。IX2215 の Solicit は小さく、プレフィックスヒント、要求オプション、
クライアント FQDN がありません。IA_PD と再設定許可だけに近い形です。
一方、router01 は正確なヒントと DNS だけの要求を付けても成功しています。
したがって、PR-400NE がすべてのヒントを拒否しているわけではありません。
router03 は systemd-networkd のクライアント FQDN とやや多い要求オプションを
付けたまま成功しているため、それらも常に失敗原因になるわけではありません。

残っている外れ値は router02 です。odhcp6c 実験中、PR-400NE は router02 へ
Advertise を返していました。送信元 UDP ポートは 49153、宛先は 546 で、
`2409:10:3d60:1230::/60`、T1 7200、T2 12600、寿命 14400 秒が入っていました。
しかし odhcp6c は Request/Reply の成立やプレフィックス設定まで進みませんでした。
その後、router02 を main の systemd-networkd 出力へ戻して再起動し、
`PrefixDelegationHint=2409:10:3d60:1230::/60` 付きの DUID-LL Solicit を
出しましたが、70 秒の取得では Advertise/Reply が見えませんでした。
router02 は別途整理が必要です。ただし、今回の観測では odhcp6c を main に
採用する理由はありません。

今回の運用上の教訓:

- DHCPv6 の取得では `udp port 546 or udp port 547` で取ります。`udp port 547`
  だけでは、PR-400NE が UDP 49153 から 546 へ返す Advertise/Reply を見落とします。
- 仮想環境で判断する前に、Proxmox ブリッジと経路上のスイッチが IPv6
  マルチキャストを確実に通す状態にしておく必要があります。
- 今回の odhcp6c 実験は main に入れません。しばらくは FreeBSD では KAME
  `dhcp6c`、Linux では systemd-networkd を基本線にします。
- プレフィックスヒント、DUID-LL の既定化、SQLite の状態保存、
  `get`/`describe`/`show`、プレフィックス取得イベントは残す価値があります。
  HGW の状態を避けるために入れた防御的な仕組みは、router02 の整理後に
  どこまで必要か見直します。
