...
| 項目 | 動作中ルーター | FreeBSD/KAME | Ubuntu/systemd-networkd | NixOS/odhcp6c 実験 |
| --- | --- | --- | --- | --- |
...
トランザクション ID を引きずる、IAID が途中で変わる、要求オプションが多い、という
別問題が見えています。odhcp6c 実験は main へ採用しません。

### 1.4 OS クライアント実装から分かること

| クライアント | 測定または引用した挙動 | routerd での扱い |
| --- | --- | --- |
| systemd-networkd | cite/measure: `DUIDType=link-layer`、`IAID`、`PrefixDelegationHint`、`WithoutRA`、`SendRelease` を設定できます。Renew/Rebind の IA Prefix 寿命は 0 です。 | Linux の基本経路として残します。通知や詳細状態の見え方は弱いため、routerd 側で観測を補います。 |
| KAME/WIDE `dhcp6c` | cite/measure: DUID はファイル、IAID と IA_PD は設定で扱います。`-n` と SIGUSR1 で Release を避けられます。ヒント付き Solicit では IA Prefix 寿命を出力できます。 | FreeBSD の基本経路として残します。NTT 向けでは DUID-LL ファイルと Release 方針を routerd 管理対象にします。 |
| odhcp6c | cite/observe: OpenWrt ではルーター連携に向きますが、今回の NixOS 実験では Advertise 後に成立しませんでした。 | main には入れません。再検証するなら別枝で行います。 |
| dhcpcd | cite: Linux と FreeBSD の両方で使え、DUID、IAID、フック、IA_PD を扱えます。Renew/Rebind の IA Prefix 寿命は 0 です。 | 将来の共通クライアント候補です。採用前に実機で Solicit/Request/Renew を短時間で比較します。 |
...
observe: NixOS 検証機は odhcp6c 実験の途中状態が残りました。Advertise は届くのに
Request/Reply が成立しない挙動があり、systemd-networkd に戻した後もすぐには
取得できていません。
...
以下の旧記述は、仮想環境のマルチキャスト透過性が不完全だった時期の観測であり、
PR-400NE の仕様や NTT 網の挙動としては残しません。

- 「PR-400NE は再起動直後にしか応答しない」
- 「T1/T2 の自然 Renew に PR-400NE が応答しない」
- 「KAME の `infinity` ヒント寿命が PR-400NE に嫌われる」
- 「プレフィックスヒントを状態から入れても PD は復活しない」
- 「Rapid Commit の有無が今回の主因である」

これらは履歴としては有用でしたが、現在は未検証または反証済みです。
...
- observe: systemd-networkd の `networkctl renew` は、今回使った版では
  ...
  routerd の恒久仕様にはしません。
- believe: dhcpcd は Linux と FreeBSD の共通クライアント候補です。採用には、
  PR-400NE 配下での短時間取得、Request/Reply、自然 Renew の確認が必要です。
- assert: odhcp6c 実験は main へ入れません。再検証する場合は、要求オプション、
  IAID、トランザクション ID、フックを整理した別枝で行います。
...
- NTT 向けプロファイルでは DUID-LL、IA_PD のみ、Rapid Commit 無効、
  `releasePolicy: never` を既定にします。
