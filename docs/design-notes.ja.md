...
| Release | observe: 不要な Release はリース表を揺らす可能性があります。NTT 向けでは停止時に Release を送らない方針を保ちます。 |
...
| KAME/WIDE `dhcp6c` | cite/measure: DUID はファイル、IAID と IA_PD は設定で扱います。`-n` と SIGUSR1 で Release を避けられます。ヒント付き Solicit では IA Prefix 寿命を出力できます。 | FreeBSD の基本経路として残します。NTT 向けでは DUID-LL ファイルを routerd 管理対象にします。 |
