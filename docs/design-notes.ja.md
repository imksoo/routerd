...
成功したクライアントは DUID-LL、安定した IAID、IA_PD、Rapid Commit 無効、`/60`
...
assert: DUID-LL と安定した IAID は NTT 向けプロファイルの強い既定値です。
運用者が `spec.iaid` を指定しない場合、routerd は上流インターフェースの
MAC アドレス下位 4 バイトから IAID を作り、明示的に出力します。要求オプション、
再設定許可、クライアント FQDN は動くクライアント間でも差があるため、routerd では
主要なプロファイル要素として扱いません。
...
assert: NTT 系プロファイルでは、実際の MAC アドレスから作った DUID-LL と IAID を既定にします。`duidRawData` と `iaid` は、高可用構成の切り替え、ルータ交換、移行のために明示的に使う上書き設定であり、通常の復旧経路では使いません。
...
- FreeBSD のホスト情報でコマンド検出を直します。`/usr/local/sbin` に入る
  ...
  見つけたコマンドのパスも表示します。
- 古い委譲プレフィックスの撤回を仕上げます。反映処理は、委譲プレフィックスが
  変わったあと、routerd が作った LAN 側アドレスと DS-Lite の送信元アドレスのうち、
  管理対象の末尾アドレスを持つ古いものを削除します。委譲プレフィックスが消えた時に、
  LAN 側 RA/DHCPv6 から古い情報をすばやく撤回する設計はまだ残っています。
