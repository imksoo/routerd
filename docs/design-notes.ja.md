...
observe: 2026-05-01 のラボで 5 台 (router01 FreeBSD、router02 NixOS、router03
Ubuntu noble、router04 FreeBSD、router05 Ubuntu noble) を 1 つの `VXLANSegment`
(VNI 100、UDP 4789) と `br-vxlan-test`、オーバーレイ `10.99.100.0/24` で
接続した。NixOS 側は `routerd render nixos` と `nixos-rebuild switch`、
Linux/FreeBSD 側は `routerd apply --once` を実行した結果、25/25 のオーバーレイ
ping が冷状態でも router02 再起動後でも全成功した。systemd-networkd の VXLAN
netdev に `Independent = true` を付ける挙動、各ホストで全ピアの flood FDB を
追加する `routerd-vxlan100-fdb` oneshot サービス、VXLAN underlay UDP と
...
#### 5.13.2 OS DHCPv6 client を信頼する。routerd は HGW を監視しない

訂正: 本節の旧版では「routerd に HGW 状態 adapter を持たせて (SNMP が
ある機種は SNMP、PR-400NE は WebUI scraping) apply が HGW state を
参照して判断する」と書いていた。これは 2 つの理由で誤り:

- 実装面: PR-400NE には PD slot テーブルを公開する SNMP が無い。
  lab で参照した「証拠」は手書きの WebUI スクレイパーであり、家庭の
  普通のユーザーが持っているもの・維持するものではない。特定機種の
  WebUI HTML に依存する future work は持続可能でない
- 設計面: 「ルーターがルーターを監視する」設計を提案していた。routerd
  の責務は DHCPv6-PD client と LAN router であって、上流 HGW の内部を
  特権的に覗く責務ではない。DHCPv6 が既に規定している client/server
  契約の外に責任を伸ばす smell。

assert: 修正後の方向性は逆 — routerd の挙動を OS DHCPv6 client 周辺に
narrow し、canonical な RFC 8415 retry cadence に任せる。

- OS DHCPv6 client (Linux/NixOS は `dhcpcd`、FreeBSD は `dhcp6c`) は
  RFC 8415 §15 の IRT/MRT/MRC で Solicit を再送し続け、諦めない。
  routerd は対応リソースが存在する間その loop を回し続けさせ、生成
  config が実際に変わった時のみ OS client を再起動する。OS client の
  自然な retry timing には介入しない
- routerd は HGW が健全かどうか知っているふりをしない。OS DHCPv6
  client が retry budget 内で取得できなかった場合、それは operator が
  HGW を直接 (Web UI / ベンダーツール) 確認するシグナルであって、
  routerd が「別 packet 形を試す」シグナルではない
- active controller は **Reply-backed binding に対する maintenance**
  (Renew/Rebind/Release/Information-Request) で有用。取得 path では
  ない

修正方向性での future work:

- `IPv6PrefixDelegation.spec.acquisitionRetry` block (例
  `restartAfterIdle: 1h` / `restartOnConfigChange: true`) を追加し、
  OS client を進捗無く動かし続ける時間の上限を設ける。再起動はプロトコル
  を変えず、ただ OS client の retry タイマーを clean state に戻すだけ
- `routerctl describe ipv6pd/<name>` に OS client の最近の Solicit
  cadence を表示 (Solicit 観測は既に記録済) し、operator が「client が
  X 秒ごとに Solicit を出してるが Advertise が返らない」を HGW を覗かず
  確認できるように
- operator playbook に明記: "N 時間 Advertise が返らない時は HGW を
  out-of-band で見るシグナル"。routerd がエスカレートするのではなく、
  人間が HGW を見るタイミング

完了条件: routerd の取得 path は OS DHCPv6 client の canonical retry。
それで終わり。routerd は HGW 状態 observer を持たないし、持たない方針。
routerd が HGW 健全性について判断する場面は maintenance path のみで、
そこでも自分の DHCPv6 transaction しか観察しない。
