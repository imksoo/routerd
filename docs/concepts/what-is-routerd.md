...
routerd is a declarative router control plane with Ubuntu Server as its primary
host target. NixOS and FreeBSD have selected integration paths and groundwork;
they are not first-lab equivalents. You write the router intent as YAML resources. routerd turns that
...
Stateful firewall filtering is intentionally scoped groundwork. routerd renders
NAT44, zone policy inputs, service holes, denial logging, and traffic
inspection, but it is not a general-purpose firewall rule language or a
security certification. Do not rely on it as the only boundary for an
Internet-facing router. NixOS and FreeBSD use the same resource model where
their native activation paths are available. Platform-specific host surfaces
are tracked in the platform matrix.
