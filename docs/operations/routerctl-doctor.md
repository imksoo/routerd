...
| `sam-enrollment-client` | Leaf-side `SAMEnrollmentClient` diagnostics: referenced claim existence, RR bootstrap endpoint/socket shape, controller phase, last success/backoff reason, and fetched `SAMRRSet` dynamic-state freshness. `Degraded` is FAIL; `Backoff` and missing/expired RRSet state are WARN. |
