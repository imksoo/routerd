...
| `hybrid` | `HybridRoute` / `OverlayPeer` references, default-route safety, MTU estimate, optional `HealthCheck` status, and read-only route-table observation (`ip -4 route show <prefix>`). |
