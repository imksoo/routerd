...
| `sam` | `MobilityPool` resource status and CloudEdge SAM convergence status. `ownershipResolverPhase=Resolved` is not treated as dataplane readiness; `samConvergencePhase=Ready` is required for a PASS. `Degraded` or missing convergence status WARNs, `Failed` or split-brain evidence FAILs, and FIB convergence is reported separately. |
