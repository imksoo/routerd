...
passes the closed managed-create and declared-data-read guards; there is no
...
The cloud plan requires 38 managed resources and, separately, exactly
three declared `oci_core_vnic_attachments` data reads for the three OCI nodes.
The guard matches their exact addresses, count, and `mode: data` in both planned
resources and changes, with change actions exactly `["read"]`. Missing,
duplicate, extra, wrong-mode, or wrong-action entries fail closed; even an
entirely missing data-read set is invalid. These queries add no paid VMs or
budget allowance. PVE plans admit no data reads, and retaining PVE `prior_state`
requires no relaxed plan rule.
...
Set `topology_scale = "full"` and `clients_per_site = 1` (the defaults).
Client cardinality is independent of router redundancy: reducing clients must
...
`clients_per_site = 2` is available only for a separately scoped engineering
topology and is rejected by this representative qualification profile.

The cost-scoped VM sizes are AWS leaf `t3.small` (2 vCPU, 2 GiB), AWS client
`t3.micro` (2 vCPU, 1 GiB), Azure `Standard_B1s` (1 vCPU, 1 GiB), OCI
`VM.Standard.E4.Flex` (1 OCPU, 1 GB), and PVE guests (1 vCPU, 1024 MiB).
One surviving AWS leaf needs its primary IPv4 plus three remote client
addresses; `t3.small` has four IPv4 slots on one ENI. The profile explicitly
sets `SAM_E2E_MAX_SECONDARY_IPS=3`, without changing the generic generator's
default. The former two-client topology could need six remote addresses plus
the primary IP, so that former `t3.large` size cannot be judged solely by RAM.
These are source defaults, not a live performance or low-memory reliability
claim. Certify the reduced-memory guests on PVE before any paid run.
...
The evidence must contain a successful individual row for each declared RR
and `pve_rr_nic_coverage` must show the exact two verified identities. A summary
that says "both PVE RR" without both rows is incomplete, not certification.
Keep any such historical evidence unchanged and record the failed acceptance
separately; obtain fresh complete evidence before cloud qualification.
The bridge-list SSH receives its `bash -s` script on stdin, but each RR's
`qm config` SSH must read from `/dev/null` so it cannot consume the next RR
inventory row. The offline test models that stdin consumption and independently
checks that incomplete, empty, and failed RR queries cannot produce a PASS.
...
The closed PVE template profile uses `eth0` for management on all five guests,
...
QGA `ping`/`network-get-interfaces` success is not management IPv4 readiness.
Only a valid response with exactly one declared management NIC and no reported
IPv4 is re-observed within the existing bounded retry loop. Missing/wrong or
duplicate NICs, malformed responses, and invalid/multiple IPv4 addresses are
failures. Retry limits, delays, deadlines, and budgets do not increase. Preserve
pending and failure evidence without inferring DHCP delay or server fault from
an empty report. Offline retry tests do not prove live DHCP recovery. This adds
no DHCP reconfiguration or qualification-time repair.
...
7. In order, run one independent harness invocation for `aws-leaf-a`,
   ...
   control/dataplane, provider/ownership, and RR membership gates, all 12
...
| Cost policy estimate / admission ceiling | USD 1.05 / USD 1.60 |
