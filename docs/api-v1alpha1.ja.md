...
- `spec.preferredLifetime` と `spec.validLifetime` は、FreeBSD の KAME `dhcp6c` に渡すプレフィックスヒントの寿命値を調整します。正の秒数で指定します。省略時、NTT 系プロファイルでは PR-400NE で観測したリース寿命に合わせて `14400 14400` を出力します。それ以外のプロファイルでは従来通り `infinity` を使います。
...
- FreeBSD の KAME `dhcp6c` では、NTT 系プロファイルかつ実効 DUID 型が `link-layer` の場合、routerd が `/var/db/dhcp6c_duid` を管理します。既存ファイルが DUID-LL でなければ `.bak.<時刻>` として退避し、上流インターフェースの MAC アドレスから作った DUID-LL を `dhcp6c` 起動前に書き込みます。
