...
このページは、その 3 点に対する routerd の答えを説明します。ローカル所有台帳は 2 番目への永続的な答えであり、単なるキャッシュではありません。破壊的なクリーンアップを安全に行うための判断材料そのものです。
...
これは Kubernetes のコントローラが取っている所有モデルに近い考え方です。Kubernetes では、生成されたオブジェクトに owner reference が付き、削除前のクリーンアップは finalizer で行い、フィールド単位の所有者は Server-Side Apply で記録します。routerd には API サーバが無いので、所有モデルを反映器とディスク上の JSON 台帳で管理しています。
...
- **台帳による判断**: `/var/lib/routerd/artifacts.json` に、過去に routerd リソースの所有物として記録されている。
...
| `IPv4DHCPAddress` | DHCPv4 クライアントのバインディング |
...
このコマンドは読み取り専用です。設定上は必要で、ホストにも存在するが、`/var/lib/routerd/artifacts.json` にまだ記録されていない構成物を一覧します。
...
dry-run でない `reconcile` が成功した場合も、routerd は在庫照合できる owned な構成物を台帳に記録します。安定した識別子に紐付けられない派生構成物は、意図的に台帳の対象から外しています。
...
routerd だけで新規構築したルータでは、反映成功時に owned な構成物を自動で台帳に記録するため、ステップ 4 が不要になることが多いです。
