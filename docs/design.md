...
| tier | scope | 主機能 |
|------|-------|-------|
| **H** (Home) | 1 host / 1-2 uplink / 1-3 LAN VLAN | WAN acquire (PD/RA/PPPoE/DHCPv4/DS-Lite)、LAN service (RA/DHCPv6/dnsmasq)、NAT44、firewall、EgressRoutePolicy |
...
通常フィールドへ `${...}` を直接書く方式は廃止した。
動的な値は `deviceFrom`、`gatewayFrom`、`addressFrom`、`ipv6From`、`upstreamFrom` のような専用フィールドで参照する。
...
`resource` は `Kind/name` 形式で書く。`field` は参照先 `status` のフィールド名である。
参照元が未確定の間、対象リソースは `Pending` のまま待機する。

`EventRule.emit.attributes` template、daemon `POST /v1/config/update`、`routerctl eval` の式評価は別機能として残す。
リソース構成の依存表現には使わない。
...
OR 条件を小さな述語として各リソースに散らす方式はやめる。
DS-Lite の AFTR は `aftrFrom`、`aftrFQDN`、`aftrIPv6` の優先順で解決する。
DNS リゾルバーなど、外部リソースの状態が必要な場合だけ `dependsOn` に書く。
...
`...From` 状態参照、`ownerRefs`、`dependsOn:` で **data-flow 依存** を表現。全依存の 80% をカバー。
...
probe ごとに `routerd.healthcheck.<name>.passed|failed|timeout` を publish し、status は `phase`, `lastResult`, `lastCheckedAt`, `lastTransitionAt`, `consecutivePassed`, `consecutiveFailed` を持つ。state machine は `Unknown → Passing → Healthy ↔ Failing → Unhealthy`。EgressRoutePolicy の candidate が `healthCheck: <name>` を持つ場合は `HealthCheck/<name>.status.phase == Healthy` を ready 条件に追加する。
...
Tier S の VPN/overlay primitive は protocol ごとに Kind を分ける。抽象 `VPNTunnel` は置かず、`WireGuardInterface`/`WireGuardPeer`, `IPsecConnection`, 将来の `TailscaleNode`, `SoftetherSession` を additive に足す。`EgressRoutePolicy` は VPN 種別を知らず、ready な candidate を weight/hysteresis で選ぶ。
...
- VRF: Linux VRF device (`ip link add <name> type vrf table <id>`) は L3 routing table separation であり network namespace ではない。process/socket の namespace isolation はしない。guest/staff/IoT の L3 分離と per-VRF egress route 選択に使う。
...
- controllers (LAN address / RA / firewall / route / DNS /  egress route / ServiceLifecycle)
...
初期 status を bootstrap して `DSLiteTunnel -> IPv4Route -> EgressRoutePolicy -> NAT44Rule`
まで再収束できる。
...
spec:
  ...
  candidates:
    - source: PPPoESession/wan-pppoe
      deviceFrom: { resource: PPPoESession/wan-pppoe, field: interface }
      gatewayFrom: { resource: PPPoESession/wan-pppoe, field: peerAddress }
      weight: 100
      dependsOn:
        - resource: PPPoESession/wan-pppoe
          phase: Up
        - resource: HealthCheck/wan-pppoe-internet
          phase: Healthy
...
`EgressRouteController` が候補の status 変化 + `routerd.healthcheck.<probe>.{passed,failed}` を購読、selection 再評価、変化時に `routerd.lan.route.changed` を publish。`FirewallController` / `MSSClampController` / `DNSResolverController` が独立に反応。手書き if/else 消滅。

Phase 2-B.1 の初期実装は `selection: highest-weight-ready` のみ実行する。`weighted-ecmp` は enum 予約で、status は `Pending(UnsupportedSelection)` とする。route install はまだ sink に流さず、`EgressRoutePolicy.status` に `selectedCandidate`, `selectedDevice`, `selectedGateway`, `lastTransitionAt` を保存し、selection 変化時だけ `routerd.lan.route.changed` を publish する。hysteresis は default 30s。
...
| **2. cascade** | `IPv6RouterAdvertisement`, `DHCPv6Information`, `DSLiteTunnel`, `IPv4Route` 関連 controller、`EgressRoutePolicy`、`HealthCheck`、`EventRule`、`DerivedEvent` engine | PD → DS-Lite → AAAA cascade 全 ~1.5s で完走 |
...
コントローラーは明示的な状態参照、ownerRefs、dependsOn、change_on で依存関係を扱います。
