...
│   routerd-dhcp6-client    routerd-ra-receiver                     │
│   routerd-pppoe-client    routerd-dhcp4-client                    │
...
|--------|---------|-----------------|
| `routerd-dhcp6-client` | DHCPv6 IA_PD / IA_NA / info-request | `routerd.dhcp6.client.{solicit,advertise,request,reply,prefix,address,info,server}.<verb>` |
...
| `routerd-dhcp4-client` | DHCPv4 lease | `routerd.dhcp4.client.{discover,offer,lease}.<verb>` |
...
- 1 process = 1 resource インスタンス (`routerd-dhcp6-client@wan-pd.service`)
- 起動は `ServiceLifecycleController` が `POST /v1/commands/start` で kick (常時 enabled だが daemon 内部 idle/active を切替)

LAN service (DHCPv6 server / RA sender / DNS) は当面 wrap (dnsmasq / radvd)。将来 `routerd-dhcp6-server` 等を Layer 1 に追加する余地は残す (§ 12 の build vs wrap)。
...
# Layer 1 (daemons)
routerd.dhcp6.client.{solicit,advertise,request,reply,prefix,address,info,server}.<verb>
routerd.dhcp4.client.<subject>.<verb>
...
type Subscription struct {
    Topics   []string         // glob: "routerd.dhcp6.client.prefix.*"
...
例:
${IPv6PrefixDelegation/wan-pd.status.currentPrefix}
${event(routerd.dhcp6.client.prefix.bound).attributes.prefix}
${daemon(routerd-dhcp6-client/wan-pd).status.observed.serverDUID}
${self.spec.interface}
...
metadata:
  ownerRefs: [{ kind: IPv6PrefixDelegation, name: wan-pd }]
```
...
```yaml
ready_when:
  - field: ${IPv6PrefixDelegation/wan-pd.status.phase}
    equals: Bound
...
[validate] ok
[diff]    ~ IPv6PrefixDelegation/wan-pd
...
```
routerd-dhcp6-client    routerd-dhcp4-client
...
過去の `routerd-pdclient` は `routerd-dhcp6-client` に **改名してから** 出荷。実装途中の今がチャンス。

### 10.2 9 次元の一貫性

| 次元 | 形式 | 例 (`routerd-dhcp6-client@wan-pd`) |
|------|------|----------------------------------|
| binary | `routerd-<protocol>-<role>` | `routerd-dhcp6-client` |
| systemd unit | `routerd-<p>-<r>@<resource>.service` | `routerd-dhcp6-client@wan-pd.service` |
| FreeBSD rc.d | `routerd_<p>_<r>_<resource>` | `routerd_dhcp6_client_wan_pd` |
...
verb は 過去形 / 受動態 (`sent, received, bound, renewed, expired, started, ready, executed, changed`)。area 内で `.` 多階層 (例 `dhcp6.client`) は OK、ただし最終 verb は必ず最後。
...
H 必須: `IPv6PrefixDelegation`, `IPv6DelegatedAddress`, `IPv6RouterAdvertisement` (LAN 送出), `IPv6RAObservation` (WAN 受信、旧 `IPv6RAAddress`), `IPv6DHCPv6Server`, `IPv4Address`, `IPv4Route`, `Link`, `Bridge`, `VLAN`, `NAT44Rule`, `Firewall*`, `DNSAnswerScope`, `DNSResolverUpstream`, `WANEgressPolicy`, `HealthCheck`, `EventRule`, `DerivedEvent`
...
- routerctl で `routerctl debug packets routerd-dhcp6-client/wan-pd` を提供

`pkg/dhcp6recorder` の機能をここに吸収して廃止する。

### 11.6 ablation 機能の保持

memory「dhcp6 ablation CLI を残すべき」。HGW 個体差調査用 flag を daemon の `--once` モードに移植:

```
routerd-dhcp6-client --once \
  --src-mac <mac> \
  --src-ll <ll> \
  --hop-limit <h> \
  --client-duid <duid> \
...
- WAN acquire daemons (`routerd-dhcp6-client`, `routerd-ra-receiver`, `routerd-pppoe-client`, `routerd-dhcp4-client`)
...
- DHCPv6 server stateful: 当面 dnsmasq、要件増えたら自前 `routerd-dhcp6-server`
...
spec:
  interface: lan
  prefixSource: ${IPv6PrefixDelegation/wan-pd.status.currentPrefix}
  subnetID: "1"
  addressSuffix: "::1"
  ready_when:
    - field: ${IPv6PrefixDelegation/wan-pd.status.phase}
...
spec:
  interface: lan
  prefixSource: ${IPv6PrefixDelegation/wan-pd.status.currentPrefix}
  rdnss: ${DHCPv6Information/wan-info.status.dnsServers}
  ready_when:
...
spec:
  localIPv6Source: ${IPv6PrefixDelegation/wan-pd.status.currentPrefix}
...
cmd/routerd-pdclient/    # routerd-dhcp6-client に改名
cmd/routerd dhcp6 サブコマンド + ablation flag 群  # daemon の --once モードに移植
pkg/api/specs.go の以下 spec field:
  IPv6PrefixDelegationSpec.Client (enum 削除、routerd 1 値)
  IPv6PDRecoverySpec
