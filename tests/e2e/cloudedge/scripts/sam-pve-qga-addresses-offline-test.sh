...
case "$*" in
  ...
  *"network-get-interfaces"*)
    ...
    ;;
  *"99-routerd-lab-dhcp.yaml"*)
    if [ -n "${QGA_IDENTITY_UNAVAILABLE_ONCE:-}" ] && [ ! -e "$QGA_IDENTITY_UNAVAILABLE_ONCE" ]; then
      : >"$QGA_IDENTITY_UNAVAILABLE_ONCE"
      printf 'QEMU guest agent is not running\n' >&2
      exit 1
    fi
    printf '{"exitcode":0,"out-data":"network:\\n  version: 2\\n  ethernets:\\n    eth0:\\n      dhcp-identifier: mac\\n      dhcp4-overrides:\\n        send-hostname: false\\n"}\n'
    ;;
  *"10-netplan-eth0.network"*)
    printf '{"exitcode":0,"out-data":"[Match]\\nName=eth0\\n[Network]\\nDHCP=ipv4\\n[DHCPv4]\\nClientIdentifier=mac\\nSendHostname=false\\n"}\n'
    ;;
  *"qm guest exec"*)
...
jq -n '[
  {name:"eth0", "hardware-address":"02:00:00:00:18:00", "ip-addresses":[{"ip-address-type":"ipv6","ip-address":"2001:db8::20"}]},
...
write_output template
jq '.nodes.value["pve-client-a"].management_mac = "02:00:00:00:18:00"' \
  "$tmp/in.json" >"$tmp/stable-in.json"
PATH="$fake_bin:$PATH" QGA_SSH_LOG="$tmp/stable-ssh.log" "$SCRIPT" \
  --pve-ssh-key "$ssh_key" --pve-known-hosts "$pve_known_hosts" \
  --tofu-output "$tmp/stable-in.json" --out "$tmp/stable-out.json" \
  --guest-known-hosts-out "$tmp/stable-known_hosts" --retries 1 --retry-sleep 0 \
  --evidence "$tmp/stable-evidence" >/dev/null
grep -q '^dhcp_identity=stable-mac send_hostname=false source=netplan active_renderer=systemd-networkd$' "$tmp/stable-evidence" || die "stable DHCP identity was not attested"
grep -q '99-routerd-lab-dhcp.yaml' "$tmp/stable-ssh.log" || die "persistent Netplan identity was not read through QGA"
test -s "$tmp/stable-evidence.dhcp-identity-source.pve-client-a.json" || die "raw DHCP identity source attestation was not retained"
test -s "$tmp/stable-evidence.dhcp-identity-rendered.pve-client-a.json" || die "raw DHCP identity rendering attestation was not retained"

PATH="$fake_bin:$PATH" QGA_SSH_LOG="$tmp/stable-retry-ssh.log" \
  QGA_IDENTITY_UNAVAILABLE_ONCE="$tmp/identity-unavailable-once" "$SCRIPT" \
  --pve-ssh-key "$ssh_key" --pve-known-hosts "$pve_known_hosts" \
  --tofu-output "$tmp/stable-in.json" --out "$tmp/stable-retry-out.json" \
  --guest-known-hosts-out "$tmp/stable-retry-known_hosts" --retries 2 --retry-sleep 0 \
  --evidence "$tmp/stable-retry-evidence" >/dev/null
grep -q '^dhcp_identity_attempt=1 state=agent-unavailable$' "$tmp/stable-retry-evidence" || die "transient QGA identity failure was not retained"
grep -q '^dhcp_identity=stable-mac ' "$tmp/stable-retry-evidence" || die "transient QGA identity failure did not recover"
...
write_multi_output
# The generic QGA cases above keep the six-guest engineering topology. The
# closed release profile has five guests: one client plus both leaves and RRs.
jq 'del(.nodes.value["pve-client-b"])' "$tmp/in.json" >"$driver_root/pve-output.json"
jq -e '.nodes.value | keys == ["pve-client-a", "pve-leaf-a", "pve-leaf-b", "pve-rr-a", "pve-rr-b"]' \
  "$driver_root/pve-output.json" >/dev/null || die "release driver fixture must retain two leaves and two RRs with only one client"
...
run_release_driver_case() {
  ...
  if [ "$expected" = pass ]; then
    ...
    jq -e '[.nodes.value[] | select(.pve_management_source == "qga-dhcp" and .ssh_host_key_source == "qga")] | length == 5' \
      "$case_dir/evidence/certification/pve/tofu-output-pve-qga.json" >/dev/null ||
      die "release driver lost five-guest QGA/IP/host-key attestation"
    [ "$(wc -l <"$case_dir/evidence/certification/pve/guest-known_hosts")" -eq 5 ] ||
