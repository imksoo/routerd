...
check_matrix initial 9
check_matrix after-failover-aws-leaf-a 2
# Read by the production function loaded above.
# shellcheck disable=SC2034
full_cloud_ingress=1
check_matrix after-failover-aws-leaf-a 9
check_matrix after-rejoin-aws-leaf-a 9
check_matrix after-failover-pve-leaf-a 9
...
jq -n '{
  ...
  topology:{routerCount:10,clientCount:4,cloudClientCount:3,rrFaultDomain:"host-redundant"},
...
preflight
if [ "$quiesce_calls" -ne 0 ] || [ "$probe_calls" -ne $((${#routers[@]} + ${#clients[@]})) ]; then
