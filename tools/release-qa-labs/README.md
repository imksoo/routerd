...
The cloud plan requires 43 managed resources plus exactly four declared
read-only `oci_core_vnic_attachments` queries, one for each OCI node. Both the
planned resources and changes must identify the four
`module.oci_leaf.data.oci_core_vnic_attachments.node` instances keyed by `router`,
`client`, `oci-leaf-b`, and `oci-client-b`; their mode is `data` and their change
actions are exactly `["read"]`. Missing (including all four absent), duplicate,
