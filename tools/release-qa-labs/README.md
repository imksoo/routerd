...
Source publication is a separate explicit policy. The default
`execution.sourcePolicy: canonical-remote` continues to require both a remote
tracking ref and a fresh canonical-origin advertisement of the exact RC.
An authorized local test may use `sourcePolicy: local-pinned` only with
`hostPolicy: local-supervised`. It skips only those two publication checks:
exact clean HEAD, canonical origin, untagged RC, frozen-main parent/ancestry,
script blobs, artifact SHA-256, root-owned immutable checkout, and all supervisor
pins remain mandatory. Guard output records `sourceClaim: local-test-only`;
this does not claim a public RC or authorize push, merge, tagging, or Release.
Keep the local commit/tree and build provenance with private run evidence.

The current [cost-bounded profile](cost-bounded-profile.md) fixes ten routers
and one client per site (14 guests total), not two clients per edge pair.
Its exact saved-plan counts and VM dimensions are admission rules, not
permissive upper bounds. Historical two-client evidence is retained separately.
...
the staged PVE template and the five pinned PVE guests, performs the QGA and
...
- `safety.pveManagementControlPlane: none`, `safety.pveTLS: pinned-ca`, and
  ...
  shared underlay whose existing DHCP service assigns the five guest addresses.
...
- separate PVE identities: `pve.sshHost` is a DNS FQDN used by every PVE
  ...
  is a five-entry map keyed by the exact PVE topology node names; the guard
  binds every value to its corresponding tfvars input, including the two leaves and
  single client. `pve.templateStage` adds one distinct, unstarted sixth VMID:
  ...
  qnap-backed data disks, and only then plans the five leaf/client/RR clones. The
  ...
  all six run VMIDs absent. It never calls the PVE network write API,
...
Successful QGA `ping` and `network-get-interfaces` calls do not prove management
DHCP IPv4 readiness. A valid interface response is re-observed within the
existing bounded QGA retry loop only when the declared management NIC appears
exactly once and reports no IPv4 address. A missing/wrong or duplicate NIC,
malformed response, or invalid/multiple IPv4 addresses is a failure, not a
pending-address condition. Retry limits, delays, deadlines, and campaign budgets
are unchanged. Retain pending observations and failure evidence; an empty IPv4
report alone does not establish DHCP delay or a DHCP-server fault. Offline retry
tests do not prove live DHCP recovery. This is certification-time observation,
not DHCP reconfiguration or qualification repair.
...
hosts, bridges, source template, qnap datastore and a fresh six-ID range.

`qa_guard.py` rejects dirty or mismatched provenance under the explicit source
policy and validates saved OpenTofu
plans against closed resource-mode, type, count, and action rules before apply.
The cloud plan requires 38 managed resources plus exactly three declared
read-only `oci_core_vnic_attachments` queries, one for each OCI node. Both the
planned resources and changes must identify the three
`module.oci_leaf.data.oci_core_vnic_attachments.node` instances keyed by `router`,
`client`, and `oci-leaf-b`; their mode is `data` and their change
actions are exactly `["read"]`. Missing (including all three absent), duplicate,
extra, wrong-mode, or wrong-action data entries fail closed; data sources are
not ignored wholesale. These queries do not create paid VMs or increase the
existing VM, instance-flavor, or budget limits. PVE plans admit no data queries,
and retained PVE `prior_state` does not require an extra plan allowance.
If this guard rejects a plan, retain the failed plan and diagnostic evidence,
reproduce the mismatch in a sanitized offline fixture, and verify the correction
before another cloud run. Do not turn paid provisioning into a debugging loop.
...
runs still consume Azure quota. See [Azure capacity admission](azure-capacity.md).
...
- the five exact PVE workload VMIDs, the disposable shared-template stage VMID,
...
Provider indexes can retain terminated VM records. Only exact-identity,
corroborated instance termination is excluded from actionable residue; raw
responses and raw/terminated/active counts remain in the evidence. Stopped
instances and non-instance resources are not exempt. See
[inventory residue classification](inventory-residue.md).
...
PVE certification intentionally leaves its stage template, five guests, capture
...
managed-create and declared-data-read guards above. Do not rerun all-scope zero or
...
A failed mutation must not suppress recovery retries: an incomplete cleanup,
inventory, or token revocation returns retryable exit 2, even when mutation
already failed. Once recovery completes, the original failed qualification
still terminates as `FAILED` (exit 1), never `DONE`. Restarting a non-PRECHECK
run executes recovery only, not a second mutation.
...
It first obtains authoritative cluster inventory, admits only the six
...
provider, 12 directed client flows, and 9 cloud-ingress flows), then proves
...
both stop and rejoin it requires all 12 directed client flows and all 9
...
before authoritative zero inventory. The cost-bounded topology is ten routers
plus one client per site, as described in [the profile](cost-bounded-profile.md).
The approved policy estimate is USD 1.05 and its admission ceiling is USD 1.60;
...
Plain discovery skips `test_systemd_confinement` and `test_launcher_restart`
before any `sudo`, systemd, or namespace call unless
`ROUTERD_RELEASE_QA_ISOLATED_TESTS` is exactly `1`. Unset, empty, and other values
remain disabled. Harmless fake/source checks and local socket tests are not
gated. These intentional skips do not establish host-integration coverage.
...
Enable the host-affecting cases only for an explicitly authorized, isolated
release-QA environment; the opt-in does not provide isolation or bypass the
existing prerequisite checks. Do not export it globally or enable it on a
shared development host under source-only testing authorization:

```sh
ROUTERD_RELEASE_QA_ISOLATED_TESTS=1 python3 -m unittest discover -s tools/release-qa-labs/tests -v
```

There is currently no dedicated CI invocation of these host suites to opt in;
ordinary discovery deliberately leaves them skipped. The host-safe
`test_isolated_host_gate.py` regression uses forbidden/mocked subprocesses to
verify both the default skip and the opt-in prerequisite boundary.
