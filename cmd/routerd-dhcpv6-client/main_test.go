...
func TestLinkLocalFromMAC(t *testing.T) {
	if got, want := linkLocalFromMAC(net.HardwareAddr{0x00, 0x00, 0x5e, 0x00, 0x01, 0x13}), "fe80::200:5eff:fe00:113"; got != want {
		t.Fatalf("linkLocalFromMAC() = %q, want %q", got, want)
	}
	if got := linkLocalFromMAC(net.HardwareAddr{0x00, 0x00}); got != "" {
		t.Fatalf("linkLocalFromMAC(short MAC) = %q, want empty", got)
	}
}
