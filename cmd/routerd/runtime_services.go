...
func applyDnsmasqConfig(configPath, servicePath string, configData []byte) ([]string, error) {
	if len(configData) == 0 {
		return removeManagedDnsmasqConfig(configPath, servicePath)
	}
...
const routerdGeneratedConfigMarker = "# Generated by routerd. Do not edit by hand.\n"

func removeManagedDnsmasqConfig(configPath, servicePath string) ([]string, error) {
	configData, err := os.ReadFile(configPath)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("read dnsmasq config %s: %w", configPath, err)
	}
	if !bytes.HasPrefix(configData, []byte(routerdGeneratedConfigMarker)) {
		return nil, nil
	}
	...
	if err != nil && !os.IsNotExist(err) {
		return nil, fmt.Errorf("read dnsmasq service %s: %w", servicePath, err)
	}
	serviceOwned := false
	if serviceExists {
		if platformDefaults.OS == platform.OSFreeBSD {
			serviceOwned = bytes.Contains(serviceData, []byte("# PROVIDE: routerd_dnsmasq")) &&
				bytes.Contains(serviceData, []byte(`name="routerd_dnsmasq"`))
		} else {
			serviceOwned = bytes.Contains(serviceData, []byte("Description=routerd managed dnsmasq DHCP service")) &&
				bytes.Contains(serviceData, []byte("--conf-file="+configPath))
		}
	}
	// A routerd-owned config paired with an administrator-owned service is an
	// ownership collision.  Removing the config would break that foreign
	// service, so preserve both paths until an operator resolves the collision.
	if serviceExists && !serviceOwned {
		return nil, nil
	}
	...
	if serviceOwned {
		if platformDefaults.OS == platform.OSFreeBSD {
			if err := runLogged("service", "routerd_dnsmasq", "onestop"); err != nil {
				return changed, err
			}
			if err := runLogged("sysrc", "routerd_dnsmasq_enable=NO"); err != nil {
				return changed, err
			}
		} else if platformFeatures.HasSystemd {
			if err := runLogged("systemctl", "disable", "--now", routerdDnsmasqService); err != nil {
				return changed, err
			}
		}
		if err := os.Remove(servicePath); err != nil {
			return changed, fmt.Errorf("remove routerd dnsmasq service %s: %w", servicePath, err)
		}
		changed = append(changed, servicePath)
	}
	if err := os.Remove(configPath); err != nil {
		return changed, fmt.Errorf("remove routerd dnsmasq config %s: %w", configPath, err)
	}
