...
運用時可使用 `routerctl get BGPRouter`、`routerctl get VirtualAddress`、
`routerctl get IngressService` 以表格形式（而非原始 status JSON）確認 peer 狀態、VIP 角色及 backend 健康狀況。
若需偵錯 live dataplane，請直接檢查主機的 `ip`、`nft` 和 conntrack state。
