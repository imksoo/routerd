...
![routerd 將 YAML resource 轉換為 local host networking、daemon、state、status 與 owned cleanup 的流程](/img/diagrams/concept-what-is-routerd.png)

routerd 是用來將 Linux 主機、FreeBSD 作為路由器運行的宣告式控制平面。將路由器的設定以 YAML 資源的形式撰寫後，routerd 會將該意圖反映至介面、位址、DHCP、DNS、NAT、路由、通道、健康檢查、套件、sysctl、服務單元、記錄等實際狀態。
...
- 介面位址分散在 netplan、systemd-networkd、rc.d 中。
...
FreeBSD 使用相同的資源模型，只有反映目標會依各 OS 採用對應的機制。各平台的差異記載於對應表中。
