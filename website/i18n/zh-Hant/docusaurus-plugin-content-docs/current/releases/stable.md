...
| --- | --- |
| 版本 | **v20260528.2308** |
| 定位 | 推薦穩定版（取代 v20260528.1805；將 reverse DNS lookup 的 goroutine 數量上限化，並新增持續觀測 heap/goroutine/fd 的 `routerctl doctor runtime`，經實機 runtime soak 驗證） |
| 運行實績 | 已在正式環境路由器（homert02）驗證。`routerctl doctor runtime -o json` soak（10 分鐘間隔 ×4 取樣）中 `numGoroutine` 始終為 123、`openFds` 始終為 25（/ 524287），每次取樣 `status=pass`。heap 為健康的 GC sawtooth（heapObjects 28k → 52k → 41k → 83k，numGC 29 → 277 頻繁回收），非單調成長。BGP 維持 2/2 Established，`routerctl doctor dslite` / `routerctl doctor reconcile` 皆 PASS，routerd-bgp PID 不變，NRestarts=0。這是在 v20260528.1805 的 2 小時 fd+heap soak（all_fd / sockets / SQLite ledger fd flat、RssAnon plateau）之上累積的。在 v20260528 系列中，3 個 fd 洩漏根因（#39 SQLite ledger、#40 control/status socket keep-alive、#40 BGP gobgp client）、2 個 heap 成長源（每請求的 OTel instrument 重建、無上限的 reverse DNS cache）以及 reverse DNS lookup 的 goroutine fan-out 均已分別解決 |
| 二進位檔 | 靜態連結（`CGO_ENABLED=0`），通過 CI 與 Release workflow |

## 推薦 v20260528.2308 的理由

推薦理由是**維運成熟度，不是新功能數量**。v20260528.2308 完整承襲
v20260528.1805 的正式環境安全特性（#39 / #40 fd 洩漏修正、v20260528.1805
的 heap 洩漏修正＝OTel instrument singleton ＋ bounded reverse DNS cache、
#36 / #37 / #38 可觀測性契約、BGP 的 idempotent reconcile、doctor dslite
的 selectedSource 對齊、Gateway Health 獨立畫面、`install.sh` 立即失敗、
secrets 伏字化、`ManagementAccess` apply 保護、可機讀的
`routerctl doctor`、推薦穩定版顯示的一致性 CI 守護），並加入資源洩漏
調查的最後兩塊拼圖以及一個小的 Web Console UX 修正：

- **reverse DNS lookup 的 goroutine 數量現已上限化。**
  `reverseDNSCache.lookupMany` 先前為每個 pending 位址各起一個 goroutine
  （僅以 semaphore 將並行 lookup 限制為 8，但 goroutine 本身大量建立）。
  單次 `/api/v1/summary`（最多 1000 列）可能產生 ~1000 個 blocked
  goroutine。現在使用固定大小的 worker pool
  （`reverseDNSLookupConcurrency = 8`），並以 `reverseDNSPendingMax = 1000`
  不依賴呼叫方即對單次處理量上限。homert02 soak 確認 summary polling 下
  `numGoroutine` 維持 flat（123）。

- **`routerctl doctor runtime` 提供持續的資源可見性。** 新增 doctor area
  ＋唯讀 control-API `/runtime` 端點，回報 routerd 自身的 heap / goroutine
  / GC / fd footprint，使本系列這類洩漏調查變為 self-service（無需 ssh
  ＋ /proc）。`numGoroutine` 超過 10000 或 fd 達到 `RLIMIT_NOFILE` 的 80%
  以上時為 WARN；僅作觀測，不會 FAIL。

- **Web Console 的 Firewall「Deny activity」改為帶軸標籤的長條圖**
  （先前為無標籤的模糊 sparkline），配明確的縱軸（peak / 0，「越高表示
  拒絕越多」）與橫軸（「24h ago」→「now」）。

自 v20260528.1805 承襲並在 homert02 v20260528.2308 上重新驗證的正式
環境安全契約：
