...
| --- | --- |
| 版本 | **v20260526.2241** |
| 定位 | 推薦穩定版（取代 v20260526.1607） |
| 運行實績 | 已在正式環境路由器（homert02）通過 **兩次連續的 in-place 升級**（1607 → 2152 → 2241）驗證：每次 routerd 重新啟動時 `routerd-bgp` 均未被觸動（MainPID 不變），BGP 始終維持 2/2 Established，uptime 跨越每次重啟持續增長（1h19m → 1h27m → 2h0m → 2h15m），2-way ECMP（.38/.53）保持在 kernel 中，`routerctl doctor dslite` 結果為 pass=12 warn=0，Web Console Gateway Health 畫面 180s / 90 samples 觀察為 good=90 / bad=0 |
| 二進位檔 | 靜態連結（`CGO_ENABLED=0`），通過 CI 與 Release workflow |

## 推薦 v20260526.2241 的理由

推薦理由是**維運成熟度，不是新功能數量**。v20260526.2241 完整承襲
v20260526.1607 的正式環境安全特性（Web Console secrets redaction、
`gatewayHealth` 彙總、可機讀 `routerctl doctor`、`ManagementAccess` apply
保護），並新增 5 項已在正式環境路由器（homert02）觀察到的維運契約：
