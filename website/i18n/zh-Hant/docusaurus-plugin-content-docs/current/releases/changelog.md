...
- Connections 顯示 DS-Lite 本機 IPv4 SNAT 項目數（總計/TCP/UDP/其他）。從完整 conntrack snapshot 按轉換後位址統計，不受 600 行上限及篩選影響。共用位址顯示合計，未取得與零分別顯示；不是 AFTR 端項目數或連接埠容量（#1253、#1254）。
- 直接存取與自動更新均取得 DS-Lite 資源；新增瀏覽器回歸測試並納入 Web Console CI。

### 驗證

- 候選版本 `714e2cd5` 已回報通過雙機頁面直接存取、自動更新、HTTPS 480/480 和四條隧道通訊測試。舊手動統計混入非 NAT 監控流量，不能作為 SNAT 基準。官方資產仍需單獨部署驗證。推薦 Stable 維持 `v20260914.0729`。
...
- 將 Web Console build chain 更新至 Vite 8.3.0、PostCSS 8.5.28 與 nanoid 3.3.19，
  移除了 `npm audit` 回報的開發環境及供應鏈中已知的高風險安全公告。發布的 Web Console
  仍為預先產生的靜態資源，不會在正式環境中執行 Vite development server（#1242）。
...
- 未過期的 DHCPv6 PD 租約在用戶端處於 Renewing/Rebinding 時仍以 `Bound` 狀態提供給相依資源，
  並將 `LeaseReady` condition 保持為 true；詳細交換狀態可透過 `observed.leaseState` 查看。
  Renew/Rebind 會依 RFC 8415 的間隔持續重試，直到收到 Reply、T2 觸發狀態轉換或租約過期，
  因而在更新期間維持 delegated address、DS-Lite 與運作中的 VRRP MASTER VIP（#1237）。
- 當明確的 `DHCPv4Reservation` 與 IPv4 sticky hold 使用相同 MAC 位址或 IP 位址時，抑制衝突的
  sticky hold，避免產生重複的 `dhcp-host` 項目，並以宣告的保留為準（#1235）。
- 在 `Type=notify` 啟動期間，不等待 systemd 完成便提交受管理 HealthCheck unit 的重新啟動，
  避免 routerd 傳送 `READY=1` 前與 `After=routerd.service` 形成循環等待（#1232）。
- 將 routerd 產生的 sticky DHCP host record 納入 DNS resolver runtime snapshot，使 resolver
  重新啟動或 HA 租約同步後，即使用戶端尚未更新 active lease，也能還原本機 A、AAAA 與 PTR
  回應。reload 會保留較新的記憶體 active lease event，並使宣告的 record 與目前的 active
  lease file 優先於 sticky recovery data（#1233）。
...
- 防止 `service-unit` 控制器在 `Type=notify` 仍處於啟動階段時重新啟動
  `routerd.service`。守護程式現在可以完成啟動、傳送 READY，並啟動 BGP 等受管理服務
  （#1228）。
...
- 即使未設定 `LogRetention`，SQLite 事件日誌現在也會限制在24小時、100,000列及
  64 MiB邏輯負載以內，且不會清除操作狀態資料表。儲存空間已滿時的日誌寫入失敗不再
  中斷本機執行階段事件傳遞。`EventJournalReadOnly` 可透過執行階段狀態、
  `routerctl doctor`、資料庫外的執行階段警示檔案及systemd服務狀態觀測，並會分別為
  揮發性Live ISO與持久性磁碟提供復原指引（#1223）。
...
- 共用的 `WhenFalse` 狀態處理現在會保留 `Disabled` 等控制器專用的終止狀態，避免 HA
  備援節點上的 DS-Lite 狀態來回切換及重複產生資源變更事件。`routerctl doctor dslite`
  現在會略過因條件為 false 而刻意停用的通道。
...
- VRRP 降級時會在關閉面向用戶端的 VMAC 後、關閉 WAN VMAC 前，先關閉其直屬的 `ip6tnl`。
  如此在待機協調刪除受角色控制的 DS-Lite 隧道期間，本機健康檢查也不會再使用已失效的
  外層 IPv6 來源位址傳送流量。
...
- VRRP 降級時會先關閉面向用戶端的 VMAC，再關閉 WAN VMAC，避免外層 IPv6 來源位址失效後
  轉送封包仍進入 DS-Lite 隧道。升級為主節點時仍維持 WAN 優先的啟動順序。
...
- 將 keepalived 的 VRRP 通知與容錯移轉 VMAC 定期協調改為依序執行。等待角色切換完成的
  定期協調會重新讀取已持久化的選舉標記，並捨棄已過期的操作，避免新當選 MASTER 的
  VMAC 在 graceful activation 開始前又被關閉。
...
- Cloud SAM direct mesh 復原現在只會在最後一個 RR topology 讀取完成後，才開始有上限的
  重試計時。因此，即使序列 RR 請求本身耗盡了重試間隔，也不會形成即時重試迴圈。Enrollment
  status 變更會立即喚醒 transport controller，transport status 變更會立即喚醒 BGP；一旦
  所有 RR 對 direct peer group 達成一致，就無須等待原有的 30 秒和 15 秒輪詢。已驗證的
  僅 RR 回退，以及復原 direct peer group 必須由所有 RR 一致確認的要求維持不變。
...
- `routerd` 現在為可選的 Cloud SAM graceful-stop handoff 設定了 daemon 結束的
  硬性期限。即使 SQLite、provider 或 BGP 相依項目忽略取消要求，shutdown 也不會無限等待；
  程序會結束，作業系統會關閉剩餘 descriptor。在原有寬限期內完成的 handoff 行為不變。
...
- `routerd-bgp` 現在會限制 SIGTERM 時本機 control socket 的排空時間與 GoBGP 的停止
  時間。即使仍有卡住的 request 或 peer，daemon 停止也不會無限期阻塞主機關機；超過上限後
  會關閉自己擁有的 socket 並結束。
...
- Ubuntu Live ISO 現在會在第一個守護程序啟動前轉譯依賴設定的
  `routerd.service`，並把 setup 相依性保留在獨立的 systemd drop-in 中。這可避免
  ServiceUnit controller 在 control socket 就緒後排程延遲的自我重新啟動；之後真正的
  unit 變更仍會依原有方式延遲重新啟動。
...
- Ubuntu Live ISO 現在會在映像檔的初始 systemd 啟動 transaction 中啟用
  `routerd.service`，並確保它在 `routerd-live-setup.service` 完成後才啟動。不再由
  setup script 第二次啟動 routerd，因此已可用的 control socket 不會短暫消失。
...
- BGP FIB 現在直接採用 GoBGP 已選取的 path 集合。Cloud SAM 的 direct path 被選為優於
  RR 備援時，routerd 不會再因不完整的屬性比較把它擴大回 ECMP。GoBGP 選取的等價多路徑
  仍會一同安裝；只有在 admission 與 stale 排除後剩餘候選沒有 selected path 時才使用原有的
  優先權備援。
...
- 更新了 Cloud SAM direct-mesh leaf 復原時使用的內部 BGP FIB 觀測路徑。將 GoBGP 的
  selected path 投影到 FIB 的修正將在下一次維護版本中完成。

## v20260822.1210

### 修正

- 舊 direct profile 即使明確使用 `nextHopRewrite: unchanged` 也能安全啟動；在更新 YAML 前，
  其有效 RR import policy 會正規化為 `peer-address`，避免備援依賴另一台 leaf 的 transport address。
...
- Cloud SAM direct leaf-mesh 現在只有在每個已設定的 route reflector 都證明目前有效的已簽章
  enrollment claim，並傳回相同的 direct topology 時才會加入。control API 為此驗證帶有可選的
  claim digest。leaf 或 reflector 處於舊版的滾動更新期間，會只使用既有 RR 路徑，而不會加入
  未驗證的 direct peer。

### 修正

- 若同名 resource 的 claim 輪替、驗證資訊遺失或不相符、receipt 被拒、timeout、direct policy
  opt-out，或 RR 之間不一致，現在會立即移除 direct peer group，並保留已建立的 RR 路徑作為
  回退。所有 RR 對目前 claim 達成一致前不會重新建立 direct topology，因此 policy 或 identity
  變更後不會殘留舊 tunnel 或 BGP peer。

## v20260822.0714

### 新增

- Cloud SAM leaf 現在可以選擇啟用由已簽章 claim 動態推導的 leaf 間 BGP mesh。direct `/32`
  使用 local preference 200，既有 route reflector 路徑繼續以 100 保留為安全回退。只有在已驗證的
  RRSet、精確的 peer identity、pair-stable tunnel addressing 與 direct peer group 都可用時，才會生成
  direct peer。這是 L3 最短路徑路由，不是 EVPN/VXLAN 或 L2 延伸功能。

### 變更

- `routerctl mobility leaf-config`、schema、範例與 Cloud SAM 文件現在公開可選的 direct peer group
  及明確的 direct/RR local-preference policy。已套用的 BGP state 會在變更 live peer 前記錄 direct peer
  的新增、移除與 policy transition，因此 daemon 重啟或 topology 變化不會遺留 stale direct state，並會
  保留 RR 回退路徑。
...
- SQLite 設定世代現在會去除未變更 source YAML 的重複項。僅有註解、空白或 mapping key
  順序差異時不會建立新的 snapshot；reconcile 仍會繼續，並將 observed state 記錄在既有
  generation。snapshot 插入為 atomic，失敗的 apply 仍可重試。

## v20260820.1423

### 修正

- 恢復不含 local `MobilityPool` 的 static direct-IPIP route reflector 相容性：profile
  可保留明示的 IPv4 `allowedPrefixes`，同時省略
  `allowedPrefixLengthMin` 與 `allowedPrefixLengthMax`。transport 產生的 RR-client
  peer 在導出型別化 transit FIB scope 前仍強制 exact `/32` 與逐 peer 的
  node-identity 約束。只設定一個 bound 或明示設定 non-`/32` bounds 仍然無效。
...
- Linux `NAT44SessionSync` 的 conntrackd mode 現在要求明確宣告 runtime 有效且
  non-optional 的 `Sysctl`：
  `net.netfilter.nf_conntrack_tcp_be_liberal=1`。`routerctl doctor nat`
  會在不修改 host 的情況下檢查此 live 前置條件。
- 離散 event consumer 現在透過 durable cursor 讀取 SQLite event store；即使遺失
  memory 內的 wake-up 仍可復原，restart 後也不會重播已處理的 event。
- live config apply 現在會切換明確的 runtime generation、取消舊 generation，並以
  有界的 single worker 串行執行 controller 工作。

### 修正

- release qualification 不再依賴只存在於文件中的 script contract；現在使用可執行的
  環境認證、schema/preflight、exact artifact、獨立 TTL/heartbeat watchdog，以及
  依 run 限定的 cleanup/inventory gate。
- 修正 RouterdCluster lease close 與 leader generation race 遺留舊工作的問題；
  peer 同步現在依 revision 與 digest 收斂。
- FreeBSD DHCPv6-PD 現在使用 platform native 的 link-local 探測，並等待新 address
  完成 DAD 後才使用。
- IPv6 `hostTraffic` policy route 會跟隨持有 PD address 的 failover VMAC；desired
  policy 消失時會移除過期 rule。
- CloudEdge qualification 在 PVE QGA 不可用時會快速停止，並優先使用目前 PVE
  inventory 探測到的 management address。
...
- HA router 在 BACKUP 狀態仍保留已驗證的 DHCPv6-PD snapshot，並在設定的
  VRRP VMAC 上預先配置委派的 LAN/WAN address。升為 MASTER 後會先取得新的
  upstream lease，再啟用 DS-Lite 與新的 NAT44 流量。
- DNS/DHCP/RA 依 active VRRP role 運作。DS-Lite HealthCheck 必須使用設定的
  source interface 與 mark，不會改走 management path。

### 修正

- active 的 `routerd-bgp` 僅在 GoBGP socket 或 control socket pathname 缺失時
  自動修復。
- 在兩台 Linux router 上觀測到雙向 VRRP 切換；role 變更後 PD、DHCP/RA/DNS、
  DS-Lite 與新的 IPv4/IPv6 流量均會復原。既有 TCP session 不保證持續。

## v20260725.1413

### 修正

- BGP 滾動升級現在使用 GoBGP v4 的 RFC 4271 平行連線衝突處理，在保留舊版
  default-active peer transport 的同時，避免 active/active peer 持續相位鎖定。
...
- Cloud SAM 現已通過 amd64 實機 Linux/FreeBSD 混合環境驗證，涵蓋兩台 CARP
  router、多個 client、AWS route reflector 與 leaf、雙向流量、active router
  故障、非搶佔式 rejoin，以及僅在成功後執行的 infrastructure cleanup。

### 變更

- FreeBSD Cloud SAM capture 在使用原生 route-aware proxy ARP、PF、CARP 與
  cloned tunnel interface 的同時，現遵循與 Linux 相同的面向使用者 ownership
  與 fail-closed lifecycle。

### 修正

- CARP backup router 現在會撤銷自身的 mobility BGP `/32` path，避免 rejoin
  後 ECMP traffic 被傳送到 silent backup。
- FreeBSD SAM reconciliation 現在會保留 proxy ARP ownership、拒絕 foreign
  enabled state，並在 cleanup 前停用 owned proxy ARP；同時會推導穩定的
  tunnel peer/name，並在不發生 active-open livelock 的情況下復原 BGP state。
...
- FreeBSD 14.3 amd64 現支援 provider-secondary-IP 加 BGP `/32` path 的完整
  Cloud SAM local-capture dataplane。native evidence 在隔離 L2 上使用兩個獨立
  router 與兩個獨立 client，驗證 published ARP、三次 gratuitous ARP、PF forwarding
  與 return traffic、CARP master/backup 切換、restart/idempotence、owned cleanup，
  以及 foreign ARP/PF 的 byte-preserved rejection。

### 修正

- SAM forwarding 現會 reconcile empty desired state，保留 provider-secondary
  neighbor ownership 以供 teardown，在 neighbor cleanup 前執行 owned route
  removal，並透過 resource status 回報 fail-closed capture error。ownership 與
  status 行為已在 Linux 和 FreeBSD 上涵蓋。

## v20260723.1609

### 變更

- `TunnelInterface.spec.peerAddress` 明確表示 FreeBSD 點對點 IPIP/GRE interface 的 IPv4 inner destination。在 FreeBSD 使用 `spec.address` 時必填，在 Linux 上會被拒絕，且絕不從設定 CIDR 推論。
- FreeBSD package 的 install/upgrade/uninstall、owned cleanup 與 foreign service preservation 已有 native amd64 與 arm64 evidence。TunnelInterface gif/GRE dataplane/lifecycle 與 owned/foreign preservation、credential-free Tailscale external-auth rollback/foreign preservation、CARP 三 VM failover/recovery/cleanup/foreign preservation 僅有 amd64 evidence；不主張 Tailscale authenticated enrollment。
- FreeBSD support boundary 已明確：`spec.family: ipv6` 的 `EgressRoutePolicy` 被拒絕；已認證的 slice 僅為 IPv4 static route host source affinity。`ClientPolicy` 使用明確 address identity 而非 MAC/L2。FreeBSD 14.3 GRE key 僅用於 outbound，kernel 不強制 inbound key matching。

## v20260721.1054

### 新增

- FreeBSD 14.3 amd64 現已成為 provisioned native PR gate，執行完整且不省略的
  `go test ./...`、current binary 與 live `routerd` smoke、render/apply 驗證、PF、
  dnsmasq、rc.d、ARP/RA observer 和 native nDPI 檢查。FreeBSD arm64 維持
  cross-compile 認證。
- FreeBSD ARP 與 rogue-RA 觀測現使用 base-system tcpdump/libpcap BPF capture；
  proactive ARP write 繼續使用 direct BPF。native DPI backend 支援 ports nDPI 5.0
  ABI，並驗證 TLS/SNI classification。

### 變更

- FreeBSD parity 現涵蓋 native doctor、healthcheck route lookup、runtime kernel
  module、BGP IPv4 FIB ownership、FRR BFD、BGP/DPI rc.d、以位址為基礎的 IPv6
  ClientPolicy，以及支援範圍內的 PF `route-to` source affinity。
- FreeBSD 會明確拒絕 fwmark healthcheck 與 non-local resolver bind；ClientPolicy
  使用 IP 而非 MAC/L2 identity。已認證的 policy routing 限於 IPv4 static
  routehost，SAM dataplane 依設計仍僅支援 Linux。

### 修正

- federation doctor 的 deterministic-ordering test 現在固定 clock，避免較慢的
  native runner 跨越秒邊界時 TTL text 在重複比較中發生變化。
...
- `scripts/release.sh` 計算 `vYYYYMMDD.HHmm` 時間戳時，發布標籤現在預設使用
  UTC；發布流程文件也明確 UTC 是標準的 tag clock。
- CloudEdge qualification evidence collection 新增 success-minimal mode：保留失敗時
  的 diagnostics，但跳過僅在成功時收集的高成本 provider/convergence snapshot。
- CloudEdge E2E 現在會把 Terraform provider profile 傳給 provider inventory
  collection，重試暫時性的 preflight SSH probe 失敗，並分別報告 provisioning、
  dataplane convergence、provider convergence 與 teardown 階段。
- 在 status persistence、flow/DNS SQLite logging、nftables steady-state check
  與 BGP reconvergence visibility 等路徑上降低了 runtime churn 與 write pressure。

### 修正

- CloudEdge SAM convergence 現在避免 full topology qualification run 中觀察到的
  route 與 ownership drift，並恢復了 drift 的 policy-route nftables table。
...
- sandbox 驗證現在會將 path-MTU nftables artifact 保留在 sandbox runtime
  ...
  pending。因此 CloudEdge SAM 產生的 config 可以在不觸碰 host
  `/run/routerd` path 的情況下完成驗證。
...
- 移除了 `MobilityPool.spec.deliveryPolicy.conntrackCleanupOnSeize` opt-in
  欄位和 SAM leaf scoped cleanup hook。已知行為變更：仍設定該欄位的 config
  會被接受，但該欄位會被忽略。在參考 SAM leaf dataplane 中，routerd 不會讓 delivered
  overlay flow 進入 conntrack，因此該功能是 no-op。未來如重新引入，應偵測
  routerd-managed ct-engage dataplane 並自動啟用 cleanup，而不是恢復為手動 opt-in flag。
...
- 明確了分離 RR/leaf Cloud-SAM admission 的邊界。僅作為 RR 的部署透過
  `SAMEnrollmentPolicy.spec.mobilityPrefixes` 授權 leaf 擁有的 `/32` claim，
  而 `SAMRRSet.spec.mobilityPrefixes` 將相同的 prefix metadata 發布給已取得
  RRSet 的 consumer，不再需要在 RR 側宣告 placeholder `MobilityPool`。

### 修正

- `DNSResolver.listen.addressFrom` 現在可以在 status 發布前，從同一 router
  config 中宣告的 `IPv4StaticAddress` resource spec 解析位址。初始 render
  不再需要為靜態 LAN resolver 監聽位址重複寫 literal。
- `FirewallEventLog` 的 systemd rendering 現在會向 `routerd-firewall-logger`
  傳遞 `--nflog-group`，使用 `spec.nflogGroup` 或預設 group `1`。這避免了原本
  應使用 NFLOG 輸入時 logger 以 stdin mode 啟動、遇到 EOF 後正常結束並進入重啟循環。
- `SAMRRSet` description 文案調整後，已同步產生的 config schema 與 website
  schema copy，恢復 `make check-schema check-website-schemas` 通過。

## v20260701.0804

### 新增

- `ControlAPI` HTTP listener 現在可以要求 bearer token、HTTPS 與 mTLS client
  certificate。`SAMEnrollmentClient` 和 `routerctl mobility enrollment-join`
  在向 RR submit/fetch 時也可使用 token/TLS 設定。source-CIDR admission 仍然最先執行。
- 新增 `routerctl doctor sam-enrollment-client` 與
  `routerctl doctor bgp-dynamic-peer`，用於檢查 leaf RRSet fetch freshness
  和 RR dynamic BGP admission counters。
- `routerctl mobility leaf-config` 現在可以產生 automatic enrollment 所需的最小
  Cloud-SAM leaf config，包含 local underlay、owned address、
  `SAMTransportProfile`、`SAMEnrollmentPolicy`、`SAMEnrollmentClaim` 和
  `SAMEnrollmentClient`，並支援 join HMAC、RR ControlAPI bearer token 與 mTLS
  client 設定。
- 新增 `routerctl mobility enrollment-revoke` 與 ControlAPI
  `POST /sam-enrollment-claims/{name}/revoke`，可在 RR 上撤銷 accepted dynamic
  `SAMEnrollmentClaim`，使舊 RRSet fetch 與 dynamic BGP admission 停止使用該 leaf。
- Cloud-SAM dynamic RR/leaf enrollment 已進入 mainline。PVE live redundancy gate
  使用 dual RR、leaf-a 到 leaf-d、`SAMEnrollmentClient`、`BGPDynamicPeer/samred-leaves`、
  FOU transport，以及 client-999 與 client-998 的雙向 ping/SSH 完成驗證。
  evidence archive 保存在
  `/home/imksoo/routerd-labs-archive/evidence/samred-20260629T035652Z/`，run tarball
  與 cleanup tarball 的 SHA256 驗證已記錄。
- Dynamic SAM RR/leaf enrollment 已記錄為無 static per-leaf inventory 的 stable
  ...
  RRSet fetch/refresh、join token/HMAC 驗證和 runtime admission state。
- release evidence 現在記錄 Dynamic SAM enrollment 的 PVE redundancy、Azure+PVE
  cloud/on-prem、public underlay/private capture 分離，以及 AWS/Azure/OCI/PVE
  client matrix live tests。
- 新增 AWS/Azure/OCI/PVE hub-leaf architecture guidance，涵蓋 runtime leaf
...
- Dynamic SAM RR admission 現在支援分離的 RR/leaf topology，不需要在 RR 端宣告
  placeholder `MobilityPool`。`SAMEnrollmentPolicy` 透過 `mobilityPrefixes`
  授權 RR-side claim/route admission，`SAMRRSet` 向 leaf consumer 發布相同
  metadata。dynamic BGP route admission 會拒絕 policy 未授權的 accepted claim prefix。
- `ControlAPI` TCP listener 現在只有在宣告 `ControlAPI` resource 或傳入
  `routerd serve --http-listen` 時才啟用。宣告的 `ControlAPI` 預設仍為 loopback
  `127.0.0.1:65432` 和 loopback-only CIDR admission，除非明確覆蓋。
- SAM leaf 的 dynamic BGP admission 現在要求通過 effective import policy 的嚴格
  `/32` route admission。pool aggregate、default、underlay route、其他 claim 的
  `/32`、未 claim 的 MobilityPool address 會在 FIB install 前被拒絕。
- SAM transport 範例繼續以 `TunnelInterface` 作為 dataplane primitive：
  private-underlay FOU/IPIP `encryption: none` 是主要輕量路徑，WireGuard 則作為
  public underlay 等場景的 optional transport-specific encryption path。
...
- 已在 full client matrix gate 前修復 PVE testing 中發現的 FOU listener reuse
  和 live redundancy 問題。
- `routerd serve --http-listen` 的 release note 現在明確提醒：HTTP mutation/control
  API 必須只 bind 到 protected/private address，或由等價的 network policy 保護。
...
- `SAMTransportProfile` 新增 pair-stable addressing mode
  ...
  的 fnv64a 雜湊確定性分配 /31 slot，節點增減時現有 peer 的位址保持穩定。
  leaf 節點不再需要 `topologyNodeRefs`。現有 `edge-index` 模式不受影響（#330, #331）。

## v20260608.0642

### 新增

- **ADR 0014 — CLI 體系重新設計。** `routerd` 僅作為守護程式（`routerd serve`），
  所有管理操作移至 `routerctl`（`validate` / `plan` / `apply` / `doctor` /
  `get` / `describe` / `status` / `ledger` 等）。舊有 `routerd apply` /
  `routerd validate` / `routerd run` 及 `--once` 已移除（#254–#262）。
- DNS 解析器支援 `IP_FREEBIND` / `IPV6_FREEBIND`，允許在 VRRP VIP 尚未
  指派時即啟動監聽（#319）。
- `routerd serve` 啟動時自動啟用 loopback（`ip link set lo up`），
  適用於 Live ISO 和容器環境（#321）。
- 新增 curl 一行安裝的 bootstrap installer（`bootstrap.sh`）（#295）。
- 新增資源生命週期註冊表與 GC planner，資源刪除時確定性地清理
  衍生 artifact（#222–#229，ADR 0014）。
- 路由器設定精靈：瀏覽器端的初始設定產生 UI，支援 Home Router / SAM /
  Kubernetes BGP profile（#233–#240）。
- 發布用於 YAML 編輯器補全的 JSON Schema（#232）。
- 新增 CloudEdge Selective Address Mobility Phase G：跨 AWS、Azure、
  OCI 與 on-prem 站點的自主 BGP /32 address mobility。WireGuard overlay +
  iBGP 上運行，ownership = BGP best path，liveness = per-node marker /32，
  cloud trap = RIB 驅動，standby seize = liveness 驅動。資料平面無 NAT、
  保留 source IP、client default gateway 不變。
- 新增 pluggable overlay underlay（IPIP / GRE / FOU / GUE，ADR 0009）。
- 新增 IPv4 force-fragment 控制（ADR 0013）。
- 擴充 `MobilityPool` 宣告式 authoring model（identity-only remote peer 等）。
- 新增 least-privilege IAM template（`examples/cloudedge-mobility-demo/iam/`）。
- DHCP 租約同步（#100, #107）、NAT44 session sync（#106）。
- 文件：日語正本翻譯 37 篇 + 中文翻譯 80 篇（#322）。全部圖表以
  gpt-image-2 重新產生（#261）。

### 變更

- Mobility delivery 以 BGP best path 作為唯一 ownership plane（ADR 0012，Option B）。
- `SAMTransportProfile` 從共享拓撲宣告匯出 per-peer 的 tunnel / BGP / route 資源。

### 移除

- 作為 clean Option B migration 的一部分，移除了 AddressLease、ownershipEpoch、
  heartbeat-event 的 mobility control plane。
- 舊有 `routerd apply` / `routerd validate` / `routerd run` CLI 進入點及
  `--once` 旗標（ADR 0014）。

### 修正

- forcefrag DF 清除從 forward hook 移至 prerouting hook，使用
  `fib daddr oifname` 查詢路由表。修復部分轉發路徑 MSS clamp 失效（#328）。
- BGP peer watch 不再觸發不必要的 `UpdatePeer`。`reflect.DeepEqual` 替換為
  穩定比較函式，忽略 `dynamicExportPrefixes` 變化和 GracefulRestart
  格式差異（#329）。
- OpenRC DNS 解析器雙重管理消除（#306）；升級時停止舊 `routerd serve`
  （#311, #313）；重啟時清理 helper（#315）；DNS 解析器 helper 監控
  （#283）；殘留 helper 更新（#280）；nodeps 重啟（#278）。
- bootstrap installer EXIT trap 可靠觸發（#324）。
- installer apply state 偵測改用 `routerctl get status -o json`（#327）。
- BGP peer state 變更透過 watch 即時反映到 status（#304）。
- 重啟不活躍的 keepalived 修復 VRRP 故障轉移（#299）。
- GoBGP 將 `BGPPeer.spec.exportPolicy.allowedPrefixes` 實際套用為 peer export
  policy（#95），變更時觸發 soft reset out（#98）。
- 刪除資源的 stale status 清理（#189）。
- 生命週期 GC 清理資源刪除時的衍生 artifact（#222–#229）。
...
- `routerctl doctor runtime`：新增 doctor area，從新的唯讀 control-API
  `/runtime` 端點回報 routerd 自身的程序 footprint（heap、goroutine 數、
  GC、open / max 檔案描述符）。當 `numGoroutine` 超過 10000，或 open fd
  達到 `RLIMIT_NOFILE` 的 80% 以上時為 WARN；僅作觀測，不會 FAIL。該
  端點同時接入 control socket 與免 sudo 的唯讀狀態 socket，並已記錄
  `routerctl doctor runtime -o json` 的形態。

### 變更

- Web Console 的 Firewall「Deny activity」圖表由無標籤的裸 sparkline
  改為帶軸標籤的長條圖。24h 內每個 5 分鐘桶一根長條，配縱軸（頂部為
  peak，底部 baseline 為 0，「越高表示拒絕越多」）、橫軸（「24h ago」→
  「now」）、可存取的 `role="img"` 標籤，以及拒絕為 0 時的「No denies
  in the last 24 hours」空狀態。

### 修正

- `reverseDNSCache.lookupMany` 不再為每個 pending 位址各起一個 goroutine。
  固定大小的 worker pool（`reverseDNSLookupConcurrency = 8`）使 goroutine
  數與單次 `/api/v1/summary` 解析的位址數無關而被上限化；並新增
  `reverseDNSPendingMax = 1000`，不依賴呼叫方 limit 即對單次處理量上限
  （超出部分於後續呼叫解析）。`Options.ReverseLookup` 的 contract 現已
  註明實作必須尊重 ctx cancellation，`RuntimeStats.OpenFDs` 也註解為
  sample-time 的近似計數。
...
- `reverseDNSCache.lookupMany` 現在會在存入新解析項目後再次執行
  `pruneLocked`，使 `reverseDNSCacheMaxEntries`（4096）的硬上限不只
  在呼叫入口、而是在每個呼叫邊界都維持。先前若單次請求解析的新位址
  超過空閒槽位數，快取可能短暫超過上限，直到下次 lookup 的 prune
  才回落；exit 時 prune 讓該不變量得以持續維持。新增回歸測試
  (`TestReverseDNSCacheLookupManyEnforcesCapAfterStore`)，先填充到
  cap-100，再解析 200 個全新位址，並斷言呼叫後大小仍在上限內。這是
  v20260528.0832 堆積洩漏修正的外部審查收尾項，單調成長本身已消除。
  homert02 的 2 小時 soak 確認 `RssAnon` 進入 plateau（從 warm-up
  的 ~70 MB 到帶 GC 回落的 ~104 MB 穩態區間），fd 維持
...
- 修正 `/api/v1/summary` 輪詢每次都為 OpenTelemetry instrument 重新
  配置的問題。`recordConsoleMetrics` 先前在請求路徑中呼叫
  `meter.Int64Gauge` / `meter.Float64Gauge`，使得 7 個 gauge
  ...
  `routerd.dhcp.sticky.held`、`routerd.client.active.count`) 每次
  輪詢都被重新建構。本次改為使用 `sync.Once` 單例
  (`getConsoleMetrics()`) 在整個程序生命週期中只建構一次並重複使用。
  搭配 #39 / #40，這次修正關閉了 summary polling 仍在產生、按 API
  呼叫累積的 heap 成長路徑。
- 修正 `reverseDNSCache` 既不清理過期項目也沒有上限的問題。先前
  TTL 僅用於決定是否重新查詢，過期項目仍會留在 map 中，使得
  firewall log / connection 表 / traffic flow 中每個不同的遠端
  位址都會成為永久條目。新增的 `pruneLocked` 會在每次 `lookupMany`
  入口移除過期項目；若仍超過 `reverseDNSCacheMaxEntries = 4096`，
  則依過期時間最早的順序剔除直到回到上限內。新增 2 個測試
  (`TestReverseDNSCachePrunesExpiredEntries`、
  `TestReverseDNSCacheCapsAtMaxEntries`) 鎖定該行為。
...
- 修正 `routerd serve` 在 BGP 控制器週期 reconcile 中洩漏
  `/run/routerd/bgp/control.sock` 上 Unix socket fd 的問題
  （homert02 v20260528.0325 上即便已套用伺服器端
  `SetKeepAlivesEnabled(false)`，仍殘留 fd 成長的根本原因）。
  `pkg/controller/bgp/gobgp_client.go` 是唯一未在 Transport 設定
  `DisableKeepAlives: true` 的內部 HTTP 用戶端。每次 BGP reconcile
  （約 30 秒）都會對 routerd-bgp 控制 socket 進行兩次 dial
  （AppliedConfig + SaveAppliedConfig），連線被 Transport 的閒置池
  保留直到 GC，正好對應 +約 4 fd/分鐘 的漂移。修正比照
  conntrack-observer / dhcpv4-client 的做法: Transport 設
  `DisableKeepAlives: true`、請求設 `req.Close = true`、返回時
  `defer client.CloseIdleConnections()`，以確保下一次 reconcile 之前
  連線已關閉。其他內部 HTTP 用戶端
  （ingressservice / conntrackobserver / dhcpv4client / chain / phase2 /
  pppoesession / dnsresolver）原本即為相同模式，本次一併稽核。
...
- 修正 Release / CI 工作流的「Capture Web Console screenshots」工作在
  瀏覽完成後等待 `networkidle` 時無限掛起的問題。Web Console 在掛載
  時會開啟 `/api/v1/events/stream` 的 Server-Sent Events 長連線，
  使得 `playwright.page.goto({ waitUntil: "networkidle" })` 始終無法
  完成。`webconsole/scripts/screenshot.mjs` 改為 `waitUntil:
  "domcontentloaded"` + 30 秒導航逾時、15 秒 `waitForSelector("main")`、
  以及 5 秒軟性 `waitForLoadState("networkidle")`（吞掉自身逾時）。
  `.github/workflows/quality.yaml` 同時為 screenshot 步驟加上
  `timeout-minutes: 10` 作為保險，使未來 flaky 執行也無法卡住整條
  release 流水線。`v20260528.0114` 標籤因這次掛起未能發布，本次
  release 在功能上與之完全等價，並附帶該 CI 修正。
...
- **影響正式環境**: `routerd serve` 不再每次 reconcile 都洩漏
  `/var/lib/routerd/routerd.db` 的 SQLite 檔案描述符 (#39)。`Ledger`
  介面新增 `Close()` 方法，`SQLiteLedger.Close()` 會關閉底層
  `*sql.DB`，並在 `resource.LoadLedger()` 的全部呼叫點加入
  `defer Close()`。主要的洩漏來源是大約 30 秒週期執行的
  `IPv4PolicyRouteController.cleanupLedgerOwnedPolicyRoutes`，在
  homert02 的 v20260526.2335 上，每次 reconcile 都會新增一組
  `routerd.db` / `routerd.db-wal` 檔案描述符。同時為
  `OpenSQLiteLedger` 加上 `SetMaxOpenConns(1)` / `SetMaxIdleConns(1)`，
  與 `pkg/state/sqlite.go` 採取同樣的防護措施，作為即便漏掉
  `Close()` 也不會超過 1 個連線的保險。新增 2 個 Linux 限定回歸測試
  （`pkg/resource` 與 `pkg/controller/chain`），驗證 10 次 open /
  close 迴圈後 `/proc/self/fd` 不會成長。
- 修正 `routerctl doctor` 的 NAT / firewall nftables 檢查在失敗時
  僅顯示「exit status 1」的問題 (#34)。檢查失敗時現在會以
  `table=<family>/<name> cmd=<command> exit=<N>
  stderr=<≤200 字元> stdout=<≤200 字元>` 的結構化資訊呈現。當 `nft`
  以非 0 結束但標準輸出實際上包含表的列表時，會降級為 **warn** 而非
  fail。`NAT44Rule` / `FirewallZone` / `FirewallPolicy` /
  `FirewallRule` 的 active / pending / missing 統計也會附加在 detail
  中，方便一次性對照 nft 端與資源端的訊號。

### 新增

- `routerctl` 全部子指令在 `--help` 時現在顯示標準的 `Usage: /
  summary / Flags: / Examples:`，而不是過去的「flag: help requested」
  (#35)。涵蓋：`dns-queries`、`connections`、`traffic-flows`、
  ...
  summary 明確說明 `--since` 接受 duration 形式，並指出絕對時間指定
  `--from` / `--to` 已於本次同步釋出。
- `routerctl dns-queries` 與 `routerctl traffic-flows` 新增絕對時間
  範圍與彙整 (#36)。`--from` / `--to` 接受 `RFC3339`、
  `2006-01-02T15:04:05`（省略時區時視為 UTC）、
  `2006-01-02 15:04:05`。新增過濾項: `--rcode`、`--upstream`、
  `--qname-suffix`、`--duration-min`（DNS）；`--peer-suffix`、
  `--protocol`、`--asymmetric`（flows）。新增 `--agg` / `--stats`
  模式輸出 `SUMMARY`，DNS 列出 `BY RESPONSE CODE` / `BY CLIENT` /
  `BY UPSTREAM` / `BY QNAME SUFFIX`，flows 列出 `BY CLIENT` /
  `BY PEER` / `BY PROTOCOL`，並附上 duration 的 p50 / p95 / p99
  分位。直接讀取 DB 時支援 `--chunk-size` 分塊，每個 chunk 擁有自己的
  ctx 截止時間。發生 `DeadlineExceeded` 時錯誤訊息會包含「已取得
  N 列，最後的 `last ts` 是 …」之類的提示。`--limit` 預設值從 100
  提升到 500，`--timeout` 從 5 秒提升到 30 秒，內部
  `DNSQueryFilter` / `TrafficFlowFilter` 的上限從 1000 提升到 10000。
  Web Console 新增 `/api/v1/dns-queries/aggregate` 與
  `/api/v1/traffic-flows/aggregate` 端點，並在既有的列取得端點上
  加入相同的過濾查詢參數（本次發布 UI 不變）。
...
- 新增 `scripts/check-active-stable.sh` CI 守護指令稿，當 homepage hero、
  文件 intro tip、announcement bar、`docusaurus.config.ts` 與
  `website/src/pages/index.tsx` 中的 `STABLE_VERSION` 常數出現分歧時
  於 CI 中 fail。release changelog 與 `stable.md` 中的 supersedes /
  carry-forward 歷史引用屬於有意保留，已從守護對象中排除。

### 修正

- homepage 的 "Latest stable" 卡片、4 個語言的 intro tip、
  `website/src/pages/index.tsx` 的 `STABLE_VERSION` 現在全部指向
  `v20260526.2241`。在 announcement bar 與 `stable.md` 完成 promote
  時，這 5 處遺留為 `v20260526.1607`，造成首頁與 announcement bar
  公示的穩定版互相矛盾。
- 重寫 `v20260526.2241` 中 install.sh 的 changelog 條目，使之與實際
  發行的實作一致：`install.sh` 仍以 cwd 相對方式定位 payload（保留
  `tests/install` 測試輔具的相容性），並在目前工作目錄不含
  `bin/routerd` 時以明確診斷訊息 exit 2 結束。先前的描述對應的是
  `cd $script_dir` 設計，因破壞 `tests/install` 已在 commit
  `d9f8817c` 中還原。
...
- `install.sh` 仍以 cwd 相對方式定位 release payload（為相容
  `tests/install` 的測試輔具），但現在若目前工作目錄下沒有可執行的
  `bin/routerd`，會拒絕繼續執行。它不再靜默地讓 `bin/*` 萬用字元展開
  0 次後以 `routerd upgrade completed` 的成功訊息結束 0，而是以明確
  的診斷訊息非 0 結束。先前若從 release 目錄之外執行（例如
  `cd /tmp/routerd-release-vYYYYMMDD.HHmm && sudo ./pkg/install.sh ...`），
  cwd 會位於 payload 之外，標準 routerd / routerctl 執行檔完全沒有
  更新，指令稿卻仍以 `routerd upgrade completed` 結束 0（只有
  `--with-ndpi-archive` 的 payload 會被裝上）。今後除非從解壓後的
  package 目錄內啟動，否則將以 exit 2 結束；CI 中已加入回歸 smoke
  (`scripts/install-sh-cwd-smoke.sh`)，涵蓋缺漏 payload 與正確 cwd
  兩種情境。
- Web Console 的 Gateway Health 畫面在 partial refresh 期間不再瞬時
  顯示 `Components 0 / Unknown / No gateway component status observed`。
  先前 `reconcileSummary` 使用 `next.gatewayHealth ?? current.gatewayHealth`，
  `??` 只在 `null`/`undefined` 時回退，因此
  `{ overall: "unknown", components: [] }` 這類瘦 snapshot 會覆蓋已
  populated 的舊狀態。現在若新 snapshot 的 components 為空而舊的有
  資料，則保留舊 `gatewayHealth`。

## v20260526.2152

### 新增

- `/api/v1/summary` 中的 `gatewayHealth` 現在依元件輸出
  `selectedPath` / `preferredPath` / `fallbackReason` / `failedProbes` /
  `lastTransition` 等佐證欄位。當選擇中的 egress path 與 preferred 不
  一致時，Web Console 會突顯目前使用中的 fallback 目標。

### 變更

- Web Console 將 Gateway Health 從 Overview 拆出獨立畫面（與
  Connections / Clients 一致）。Overview 僅保留彙整卡片：overall 狀態、
  pass / warn / fail / skip 件數、跳轉按鈕，以及 degraded / down 時
  顯示 worst 元件名稱的一行提示。

### 修正

- BGP 控制器在 reconcile 入口會先 hydrate 已套用的策略狀態，因此
  routerd 重新啟動時不會再次 PUT 內容未變的 import-policy 指派並
  重置所有 BGP 工作階段。先前在生產環境（homert02）每次 routerd 重新
  啟動都會讓所有 peer 中斷後重建，ECMP 必須等到 hold-time 過期才能
  復原。
- `routerctl doctor dslite` 現在會把 DSLiteTunnel `phase=Up` 視為健康，
  並透過 `status.selectedSource = "DSLiteTunnel/<name>"` 辨識
  EgressRoutePolicy 的選擇（同時保留舊有 `selectedCandidate` 名稱比對
  作為相容回退）。先前在採用 `dslite-pd-balanced` 等彙總候選名稱的
  生產設定下，即使 `gatewayHealth` 判定為 `ok`，所有健康的 DSLiteTunnel
  仍會顯示為 WARN。
...
- `routerctl ledger prune-events` 非 dry-run 執行後會發出稽核事件
  `routerd.ledger.events.pruned`（屬性包含 `cutoff`、`deletedRows`、
  呼叫端 `uid`/`gid`），prune 自身可在 events 表中追蹤。

### 變更

- `/api/v1/summary` 的 `gatewayHealth` 現在還會彙總 `EgressRoutePolicy` /
  `NAT44Rule` / `HealthCheck`。Web Console 的 Overview 橫幅顯示目前選擇
  的 egress path 與 preferred 的一致狀態，啟用 fallback 候選時會明顯
  警告。

### 安全

- Web Console 的 `/api/v1/config` 與 generation config/diff 端點在
  序列化前會對 secrets 進行 redact（WireGuard `privateKey` /
  `preSharedKey`、Tailscale `authKey`、BGP/PPPoE/IPsec `password`、
  WebConsole `initialPassword`、bearer/token 欄位等）。鍵保留並以標記值
  取代，不會影響 UI 結構。特權通道（control socket、`routerctl describe`）
  不變。修復了管理 LAN 上的維運人員即使在唯讀模式下也可能透過 Web Console
  看到原始 secrets 的問題。
...
- `routerctl doctor [area]`：執行 wan / dns / dslite / dhcpv6-pd / nat /
  firewall / rollback / disk / mgmt 的一組唯讀檢查，並以 PASS/WARN/FAIL 與
  修復提示回報；若有 FAIL 則以非 0 結束，方便由腳本呼叫。
- SQLite state DB 維護指令：`routerctl ledger integrity-check` / `vacuum` /
  `backup <dest>` / `prune-events --older-than <dur>`。prune 僅作用於
  events，保留支撐 rollback 與稽核的 generations / objects / artifacts。
- `ManagementAccess` 資源：宣告管理用介面與管理來源 CIDR。宣告後，非
  dry-run 的 `apply` 會在「宣告的管理介面缺失 / firewall 會阻擋 SSH（管理
  介面未歸屬 mgmt 或 trust 的 FirewallZone）/ 啟用的 WebConsole 繫結到所有
  位址」時失敗（可用 `--allow-mgmt-lockout` 覆寫）。
- `api/v1/summary` 新增 `gatewayHealth` 物件：彙總 `DNSResolver` /
  `DSLiteTunnel` / `DHCPv6PrefixDelegation` 並回傳整體判定與各元件狀態。
  Web Console 在 Overview 頂端顯示 Gateway Health 橫幅，degraded / down
  時強調原因與 waiting。
- `examples/home-router-mgmt-protected.yaml`：替換家庭路由器的「安全最小起點」
  canonical 範例，包含 3-role 防火牆（untrust / trust / mgmt）、DS-Lite 優先
  + PPPoE 備援、`ManagementAccess`，以及繫結到 mgmt 位址的 `WebConsole`。

### 變更

- Go module path 改為 `github.com/imksoo/routerd`（舊值：`routerd`）。從
  release 壓縮檔安裝的使用者不會受影響，但可以使用 `go install
  github.com/imksoo/routerd/...`，外部 Go 專案也可以將其作為 module 引入。
