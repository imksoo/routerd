...
日常運維請使用 `routerctl get events`，而非直接操作 `sqlite3`：
...
節點重啟或 rejoin 後的重新確認事件可能會重用原始 journal 接受時間作為
`issuedAt`。此時，`timestamp - issuedAt` 會包含節點停止或缺席的時間。
請將這類差值視為重新確認經過時間，不要解釋為收斂延遲。
