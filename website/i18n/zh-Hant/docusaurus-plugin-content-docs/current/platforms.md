...
目前的發布認證刻意比可產生的功能清單更窄。FreeBSD 會明確拒絕 `spec.family: ipv6` 的 `EgressRoutePolicy` resource，因為已認證的 PF `route-to` 僅涵蓋 IPv4 static route host。`TunnelInterface` 的 gif/GRE 與發布 package 的 install/upgrade/uninstall 仍在完成 native 認證。已產生的 Tailscale 與 CARP rc.d artifact 可以使用，但其 lifecycle/failover 尚非發布宣告。
