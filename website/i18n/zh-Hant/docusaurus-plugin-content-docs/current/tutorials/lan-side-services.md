...
- 請先從 `routerctl plan` 開始。在確保管理路徑與已知的回滾路徑後，再啟用生產環境的 LAN 監聽。
- 手動修改 dnsmasq 租約檔案不會自動通知 routerd。請透過受支援的設定工作流程重新協調受影響的租約；`routerd-dhcp-event-relay` 是由 dnsmasq 每次呼叫的一次性 hook，不是可重新啟動的服務。租約的變更請盡量透過 routerd 進行。
