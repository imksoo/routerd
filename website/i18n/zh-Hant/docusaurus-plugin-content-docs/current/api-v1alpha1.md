...
| `ManagementAccess` | 宣告管理用介面與 apply 前的 lockout 檢查。宣告後，若偵測到管理 IF 缺失、被 firewall zone 阻擋、或啟用的 WebConsole 繫結到所有位址，則在未指定 `--allow-mgmt-lockout` 時中止 apply。 |
...
| `SAMEnrollmentPolicy` | 僅 hub/RR 使用的 SAM transport enrollment claim 授權。它繫結 transport profile、輸出 `rrSetRef`、必要的靜態 RR topology `rrNodeSetRef`、join-token source、join audience、tunnel/endpoint prefix、選用 leaf ID pattern、TTL、明確的 claim revoke、選用 WireGuard materialization，以及作為 RR 端 admission authority 的 `MobilityPool` reference 或直接 `mobilityPrefixes`。`directMesh.peerGroupRef` 還可授權一個選用的 leaf 間直連群組；RR 一律保留為備援。leaf 不宣告副本。 |
| `SAMEnrollmentClaim` | leaf 本機的遠端 enrollment request：leaf identity、join nonce/timestamp/HMAC、遠端 policy/RRSet reference、tunnel address、endpoint、選用且經簽署的 `directMesh` opt-in、選用 BGP identity、選用由 MobilityPool 擁有的 `/32`、選用 WireGuard credential、expiry 和 revocation state。 |
| `SAMEnrollmentClient` | leaf 端 bootstrap/refresh controller：向 bootstrap RR endpoint 提交本機遠端請求的 `SAMEnrollmentClaim`，取得允許的 `SAMRRSet` 與符合資格的直連 peer group；僅在缺失、接近到期或 claim material 變更時將它們儲存為 dynamic state。直連 peer 必須由所有 RR 對同一 client identity 和 topology 進行證明。若所有 RR 都已接受同一 claim、但 peer 清單暫時不同，status 會顯示 `directTopologyPending: true`：繼續使用 RR，不使用部分直連路徑，並只以 GET 重新檢查。設定 `joinTokenFrom` 時 identity 由 HMAC 驗證；未設定時屬於 trusted control-plane 輸入。RR 僅遺失易失 admission state 時，只接受目前 RR 的 identity-aware 回應來自動重新提交；revoke、renewal 期間不同的 active identity、損壞回應、舊版／混合 RR 或任何不確定性都會保留 RR fallback。它不需要本機 policy 或 NodeSet。 |
| `SAMTransportProfile` | 宣告 router 穩定的 `selfNodeRef`、`mode`（`ipip`、`gre`、`fou` 或 `gue`）、`encryption`（`none` 或 `wireguard`）、inner tunnel prefix、underlay interface、BGP router 和 peer source。`fou`/`gue` 使用既有 `TunnelInterface` FOU/GUE 路徑並要求 `encapSport` 與 `encapDport`。它透過 `peersFrom` 從 `SAMNodeSet`、執行期同步的 `SAMPeerGroup`、`SAMEnrollmentPolicy` 或已取得的 `SAMRRSet` 讀取 topology 和 peer endpoint；`SAMNodeSet` 是 canonical identity/topology/endpoint source，`nodeRefs` 可選擇相鄰 peer。enrollment 回傳的 `SAMPeerGroup` 標記為 `direct: true` 時，會新增一條選用的 leaf 間直連路徑，並賦予較高的 BGP local preference；它必須使用 `pair-stable` 位址配置，並且必須排在一個非 optional `SAMRRSet` 備援 source 之後。direct profile 必須明確設定一般 RR 的 `bgp.importPolicy.localPreference`，`bgp.directLocalPreference` 必須更高。省略 `bgp.timers`、`bgp.timersPreset` 和 `bgp.convergenceProfile` 時，產生的 peer 會繼承所參照 `BGPRouter` 的預設值；profile 內的設定優先。`SAMPeerGroup` 不能作為可 author 的頂層 resource。routerd 透過 replace-on-reconcile 的 `DynamicConfigPart` 為每個 peer 導出 `TunnelInterface`、endpoint `/32` `IPv4Route`，以及（除非 `spec.bgp.generatePeers: false`）`BGPPeer`。 |
...
| `BFD` | 宣告 BFD session intent。在 Linux 上，routerd render FRR `bfdd` 設定並記錄觀測到的 BFD 狀態；被參照的 GoBGP peer 不會因 BFD false-down 而被 deconfigure。 |
...
`ManagementAccess` 宣告 routerd 必須保持可達的管理介面與管理來源 CIDR，
避免非 dry-run 的 `apply` 把維運人員自己鎖在門外。當存在至少一個 `ManagementAccess` 時，
apply 前的 preflight 會執行以下檢查，未指定 `--allow-mgmt-lockout` 時**會中止 apply**。
`validate` / `plan` / `show` 不受影響；dry-run apply 只顯示 findings，不會中止。
...
spec:
  ...
  allowSourceCIDRs:
    - 192.168.100.0/24
    - fd00:100::/64
  requireWebConsoleBound: true  # 預設
```
...
| 檢查 | 失敗條件 |
| --- | --- |
| 介面存在 | `interfaces[]` 中宣告的 IF 在 `Interface` 資源中不存在（管理介面被刪除或改名）。 |
| firewall self-access | 存在至少一個 `FirewallZone`（firewall 啟用），但宣告的管理 IF 未屬於 role `mgmt` / `trust` 的 `FirewallZone` — input 鏈的 `policy drop` 會阻擋對路由器自身的 SSH。 |
| WebConsole 繫結 | `WebConsole` 啟用且繫結到 `0.0.0.0` / `::`。`requireWebConsoleBound: true`（預設）為 fail，false 為 warn。 |

相同檢查也可在 `routerctl doctor mgmt` 中執行（不會 apply）。

`spec.allowSourceCIDRs` 目前是**資訊性**欄位（用於 status 與 doctor 顯示），尚未由 firewall guard 強制執行。

`--allow-mgmt-lockout` 是**緊急覆寫**旗標。例如將管理介面移到新 VLAN，須在備妥 PVE console 等復原路徑的情況下，刻意套用會被阻擋的設定時才使用。日常維運不需要它。
