...
| `SAMEnrollmentClient` | leaf 端 bootstrap/refresh controller：向 bootstrap RR endpoint 提交本機遠端請求的 `SAMEnrollmentClaim`，取得允許的 `SAMRRSet` 與符合資格的直連 peer group；僅在缺失、接近到期或 claim material 變更時將它們儲存為 dynamic state。它不需要本機 policy 或 NodeSet。 |
