...
- [ ] 對於 BGP `provider-secondary-ip`，`routerd-cloud` **不持有**該位址作為 OS 本機位址；即使 provider secondary IP 仍保留在 ENI 上也是如此（捕獲僅用於路由/轉發，無 OS 位址）。
- [ ] `MobilityPool` 達到 **Ready** 狀態，目標位址的 provider action 已成功或確認無須執行。
