...
`routerctl vpn tailscale peers -o json` 會讀取 `tailscale status --json`，並以 routerd CLI 格式顯示對等節點清單。Web 管理介面的 Resources 頁面也會在 `TailscaleNode` 中顯示對等節點的線上狀態、relay、最後上線時間及允許的路由。
