...
作为出发点，先在 daemon 未启动时进行独立检查。以下命令需要具有 `sudo` 权限的本地用户，但不会应用网络变更：
...
| `DNSResolver/lan` | 从 VIP 的 `hostname` 字段自动响应 `k8s-api.cluster.example`，同时提供 control plane 与 worker 的静态记录。 |

DHCP 地址池请确保不与 VIP、control-plane 地址、worker 地址及 LoadBalancer /
Service 广播范围重叠。

服务运行后，才可使用 `sudo routerctl get BGPRouter`、`sudo routerctl get VirtualAddress`、
`sudo routerctl get IngressService` 以表格形式（而非原始 status JSON）确认 peer 状态、VIP 角色及 backend 健康状况。
若需调试 live dataplane，请以 `sudo ip`、`sudo nft` 和 `sudo conntrack` 检查主机状态。
