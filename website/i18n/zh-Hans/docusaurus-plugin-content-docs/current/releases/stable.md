...
| --- | --- |
| 版本 | **v20260528.2308** |
| 定位 | 推荐稳定版（取代 v20260528.1805；将 reverse DNS lookup 的 goroutine 数量上限化，并新增持续观测 heap/goroutine/fd 的 `routerctl doctor runtime`，经实机 runtime soak 验证） |
| 运行实绩 | 已在正式环境路由器（homert02）验证。`routerctl doctor runtime -o json` soak（10 分钟间隔 ×4 采样）中 `numGoroutine` 始终为 123、`openFds` 始终为 25（/ 524287），每次采样 `status=pass`。heap 为健康的 GC sawtooth（heapObjects 28k → 52k → 41k → 83k，numGC 29 → 277 频繁回收），非单调增长。BGP 维持 2/2 Established，`routerctl doctor dslite` / `routerctl doctor reconcile` 均 PASS，routerd-bgp PID 不变，NRestarts=0。这是在 v20260528.1805 的 2 小时 fd+heap soak（all_fd / sockets / SQLite ledger fd flat、RssAnon plateau）之上累积的。在 v20260528 系列中，3 个 fd 泄漏根因（#39 SQLite ledger、#40 control/status socket keep-alive、#40 BGP gobgp client）、2 个 heap 增长源（每请求的 OTel instrument 重建、无上限的 reverse DNS cache）以及 reverse DNS lookup 的 goroutine fan-out 均已分别解决 |
| 二进制文件 | 静态链接（`CGO_ENABLED=0`），通过 CI 与 Release workflow |

## 推荐 v20260528.2308 的理由

推荐的理由是**运维成熟度，不是新功能数量**。v20260528.2308 完整继承
v20260528.1805 的正式环境安全特性（#39 / #40 fd 泄漏修复、v20260528.1805
的 heap 泄漏修复＝OTel instrument singleton ＋ bounded reverse DNS cache、
#36 / #37 / #38 可观测性契约、BGP idempotent reconcile、doctor dslite 的
selectedSource 对齐、Gateway Health 独立画面、`install.sh` 立即失败、
secrets 伏字化、`ManagementAccess` apply 保护、可机读的
`routerctl doctor`、推荐稳定版表示的一致性 CI 守护），并加入资源泄漏
调查的最后两块拼图以及一个小的 Web Console UX 修复：

- **reverse DNS lookup 的 goroutine 数量现已上限化。**
  `reverseDNSCache.lookupMany` 此前为每个 pending 地址各起一个 goroutine
  （仅以 semaphore 将并发 lookup 限制为 8，但 goroutine 本身大量创建）。
  单次 `/api/v1/summary`（最多 1000 行）可能产生 ~1000 个 blocked
  goroutine。现在使用固定大小的 worker pool
  （`reverseDNSLookupConcurrency = 8`），并以 `reverseDNSPendingMax = 1000`
  不依赖调用方即对单次处理量上限。homert02 soak 确认 summary polling 下
  `numGoroutine` 保持 flat（123）。

- **`routerctl doctor runtime` 提供持续的资源可见性。** 新增 doctor area
  ＋只读 control-API `/runtime` 端点，报告 routerd 自身的 heap / goroutine
  / GC / fd footprint，使本系列这类泄漏调查变为 self-service（无需 ssh
  ＋ /proc）。`numGoroutine` 超过 10000 或 fd 达到 `RLIMIT_NOFILE` 的 80%
  以上时为 WARN；仅作观测，不会 FAIL。

- **Web Console 的 Firewall「Deny activity」改为带轴标签的柱状图**
  （此前为无标签的模糊 sparkline），配明确的纵轴（peak / 0，「越高表示
  拒绝越多」）与横轴（「24h ago」→「now」）。

自 v20260528.1805 继承并在 homert02 v20260528.2308 上重新验证的正式
环境安全契约：

- **`/api/v1/summary` 轮询不再使 heap 无限增长。**
  `recordConsoleMetrics` 此前每次请求都重建 7 个 OpenTelemetry gauge，
  现在通过 `sync.Once` 单例 (`getConsoleMetrics`) 只构造一次。
  `reverseDNSCache` 此前仅用 TTL 决定是否重新查询，既不清理过期项也
  无大小上限，导致 firewall log / connection 表 / traffic flow 中每个
  不同的远端地址都成为永久条目。现在会清理过期项，并在调用入口与出口
  两处都强制 4096 条硬上限。homert02 的 2 小时 soak 确认 `RssAnon`
  进入 plateau 而非单调增长。这些作为 v20260528.0402 fd 泄漏工作对应
  的 heap 侧修复，收尾整个调查。

自 v20260528.0402 继承并在 homert02 v20260528.1805 上重新验证的 2 项
影响生产的 fd 泄漏修复与 3 项可观测性契约：
