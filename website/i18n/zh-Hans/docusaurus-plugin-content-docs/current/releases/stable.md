...
| --- | --- |
| 版本 | **v20260528.1805** |
| 定位 | 推荐稳定版（取代 v20260528.0402；新增收尾 fd 泄漏工作的 heap 泄漏修复，并经 2 小时生产 soak 验证） |
| 运行实绩 | 已在正式环境路由器（homert02）验证 fd 与 heap 均 bounded。fd 在 2 小时 / 24 个采样的 soak 中完全 flat（all_fd=24、sockets=16、SQLite ledger 系=4 每次采样都不变，NRestarts=0，PID 不变）。`RssAnon`（不含 page cache 的真实 heap）在第 1 小时从 ~70 MB warm-up 到 ~104 MB 的稳态区间后，第 2 小时在 96–107 MB 之间振荡并 plateau，伴有明显的 GC 回收 dip——这是 bounded working set 的特征，而非泄漏。BGP 维持 2/2 Established，`routerctl doctor dslite` 返回 pass=12 / warn=0，`routerctl doctor reconcile` 返回 pass=1 / warn=0。在 v20260528 系列中，3 个 fd 泄漏根因（#39 SQLite ledger、#40 control/status socket 的 keep-alive、#40 BGP gobgp client）与 2 个 heap 增长源（每请求的 OTel instrument 重建、无上限的 reverse DNS cache）已分别定位并修复 |
| 二进制文件 | 静态链接（`CGO_ENABLED=0`），通过 CI 与 Release workflow |

## 推荐 v20260528.1805 的理由

推荐的理由是**运维成熟度，不是新功能数量**。v20260528.1805 完整继承
v20260528.0402 的正式环境安全特性（fd 泄漏修复 #39 / #40、#36 / #37 /
#38 可观测性契约、BGP idempotent reconcile、doctor dslite 的
selectedSource 对齐、Gateway Health 独立画面、`install.sh` 立即失败、
secrets 伏字化、`ManagementAccess` apply 保护、可机读的
`routerctl doctor`、推荐稳定版表示的一致性 CI 守护），并新增收尾这次
长期资源泄漏调查的 heap 泄漏修复：
