...
| --- | --- |
| 版本 | **v20260526.2241** |
| 定位 | 推荐稳定版（取代 v20260526.1607） |
| 运行实绩 | 已在正式环境路由器（homert02）通过 **两次连续的 in-place 升级**（1607 → 2152 → 2241）验证：每次 routerd 重启 `routerd-bgp` 都未被触碰（MainPID 不变），BGP 始终维持 2/2 Established，uptime 跨越每次重启持续增长（1h19m → 1h27m → 2h0m → 2h15m），2-way ECMP（.38/.53）保持在 kernel 中，`routerctl doctor dslite` 结果为 pass=12 warn=0，Web Console Gateway Health 画面 180s / 90 samples 观察为 good=90 / bad=0 |
| 二进制文件 | 静态链接（`CGO_ENABLED=0`），通过 CI 与 Release workflow |

## 推荐 v20260526.2241 的理由

推荐的理由是**运维成熟度，不是新功能数量**。v20260526.2241 完整继承
v20260526.1607 的正式环境安全特性（Web Console secrets redaction、
`gatewayHealth` 汇总、可机读 `routerctl doctor`、`ManagementAccess` apply
保护），并新增 5 项已在正式环境路由器（homert02）观察到的运维契约：
