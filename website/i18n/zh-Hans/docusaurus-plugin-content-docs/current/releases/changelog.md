...
- FreeBSD support boundary 已明确：IPv6 `EgressRoutePolicy` 被拒绝，`ClientPolicy` 使用显式 address identity 而非 MAC/L2；TunnelInterface、Tailscale 和 CARP 的当前 lifecycle qualification 尚不是 release claim。
