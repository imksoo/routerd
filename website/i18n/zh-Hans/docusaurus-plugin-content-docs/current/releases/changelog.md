...
- 直接访问与自动刷新均取得 DS-Lite 资源；新增浏览器回归测试并纳入 Web Console CI。

### 验证

- 候选版本 `714e2cd5` 已报告通过双机页面直接访问、自动刷新、HTTPS 480/480 和四条隧道通信测试。旧手工统计混入非 NAT 监控流量，不能作为 SNAT 基准。官方资产仍需单独部署验证。推荐 Stable 保持 `v20260914.0729`。
...
- 暂时的 readiness 下降不再撤销已成功发布的 MASTER VIP。同一地址及接口的发布历史
  在观测错误后仍保留，同时报告错误；降级、确认 VIP 不存在或发布失败会使历史失效。
  初次发布仍须满足就绪条件（#1248、#1252）。
- 运行时配置代际切换会保留配置未变的受管理 DHCP 客户端，并重建已结束的 supervisor。
  删除、配置变更及 serve 退出时仍执行相应生命周期处理（#1249、#1252）。
- 升级时保留当前生成的 `routerd.service` 及其 Capability、Environment；仍会迁移含
  已移除 `--controller-chain` 参数的旧 unit（#1250、#1252）。

### 验证

- 候选版本 `f43b4451` 通过本地测试、CI、两台路由器的升级及 reload 测试，以及隔离 Linux
  netns 的观测故障测试。报告的客户端 HTTPS 测试为 360/360 成功。较早候选版本的一次 DNS
  超时仍未查明原因；这些结果不代表所有条件下均无中断，也不代表新的 AWS/Azure/OCI
  实机认证。证据见 PR #1252。
...
- 将本地 proxy-ARP leaf 已解析的 capture source address 作为非 owner 的 SAM return-route
  进行通告。路由器自身发起的诊断流量现在可以接收远端 Mobility site 的响应，同时不会把
  router address 纳入 provider capture 候选（#1244）。

## v20260913.1222

### 修复

- 在内部保留 routerd 自身发送 `READY=1` 和 `STATUS=` 所需的 systemd 通知套接字，同时从
  子命令继承的环境中移除 `NOTIFY_SOCKET`。这可避免在 `Type=notify` 与
...
- Live ISO构建现在会将 `/tmp` 和 `/var/tmp` 规范为root所有的1777模式，包括从
  rootfs缓存恢复的构建。启动后即可正常使用标准临时文件和 `routerctl doctor disk`
  （#1230）。
...
- `routerd-dns-resolver` 现在会在有意通过 SIGINT 或 SIGTERM 停止后正常退出，不再将已取消的
  运行上下文报告给服务管理程序并误判为进程失败。
...
- 升级安装程序现在会等待已计划的 routerd 自行重启完成，并重新检查状态 socket 与应用时间
  后才报告成功。
- 只有在 loopback 的 53 端口监听配置可使用默认 (`.`) upstream source 时，派生的主机 DNS
  接管才会将 systemd-resolved 指向 `127.0.0.1`。仅含条件区域的 resolver 不再替换正常的
  主机 DNS。
...
- VRRP 降级时会先关闭面向客户端的 VMAC，再关闭 WAN VMAC，避免外层 IPv6 源地址失效后
  转发报文仍进入 DS-Lite 隧道。升级为主节点时仍保持 WAN 优先的启动顺序。

## v20260826.1943

### 修复

- 待机节点从 PD 快照预置 VMAC 时也会检查持久化的 VRRP 选举角色，避免过期的待机协调再次
  关闭新当选 MASTER 的 VMAC。活动节点的委派 IPv6 地址和 DS-Lite 隧道会保持待就绪状态，
  直到 DAD 确认每个外层源地址可用。
...
- 将生成的 HealthCheck systemd unit 绑定到 `routerd.service`。这样，受角色条件控制的探测
  会随控制器停止，不会在故障转移 VMAC 和委派 IPv6 源地址撤销后继续通过 DS-Lite 隧道发送。
...
- 启用 IPv4 VRRP graceful activation 时，当选路由器会先恢复 conntrack 状态，并等待配置的
  IPv6 PD、委派地址、DS-Lite、健康检查和 EgressRoutePolicy 全部就绪，随后才通告 LAN VIP。
  降级时会在拆除数据路径前撤回 VIP；就绪等待超时时保持不通告，并可再次尝试。
- keepalived 角色 hook 会在 VMAC 与 conntrackd 切换成为持久状态后，通过 `SIGUSR1` 唤醒
  routerd，避免 DS-Lite、HealthCheck 和 EgressRoutePolicy 的重新协调等待 VRRP 轮询。
  已应用的 EgressRoutePolicy 状态会记录从角色切换到就绪所需的时间，并明确显示收敛期间新的
  未标记流量使用下一个已就绪的替代路径。
- `EgressRoutePolicy` 多目标哈希改用固定256个 bucket 的 rendezvous 分配。目标健康状态、
  顺序、添加、删除或恢复发生变化时，只要原 bucket 所有者仍可用，就不会重新映射该来源。
  已有且 conntrack mark 非零的连接仍会绕过新流量哈希。

## v20260824.1404

### 修复

- `routerctl get IPv4Route` 及其小写写法 `ipv4route` 现在会查询
  `IPv4Route`，不再被错误改写为 `EgressRoutePolicy`。具名 IPv4Route 查询已恢复；
  既有的 `route`、`routeset` 和 `ipv4policyrouteset` 仍是 EgressRoutePolicy 的别名。
...
- Cloud SAM direct mesh 收敛现在把每分钟 topology 重新验证作为 controller 的明确 deadline
  调度。发现较短的 RR lease 时会约束 client 刷新提前量，并且会并行读取所有 RR topology 后才
  保留或恢复 direct peer。仅 RR 回退、所有 RR 一致确认和既有指数退避保持不变。
- 持久化 DynamicConfigPart 发生变化后会立即唤醒 consumer。routerd-eventd 原子替换 peer
  配置、重试失败的配置交接，并隔离不可达 peer，避免其阻塞健康 peer。除非 transport profile
  明确覆盖，生成的 SAM BGP peer 会继承引用 BGPRouter 的 timer 和 convergence 默认值。

### 移除

- 移除了未实际生效的 `SAMEnrollmentPolicy.revokeAfterInactive` 字段。包含该字段的配置现在会
  明确失败；请改用 claim TTL 或显式撤销 claim。
...
- Cloud SAM direct leaf 现在会在 RR control endpoint 重启期间暂时不可达时保留已验证的
  仅 RR 回退，并按有上限的收敛间隔重试可选 direct topology GET。来自可达 RR 的响应，例如
  revoke、identity mismatch、无效 payload 和 validation failure，仍使用原有的普通指数退避。
  leaf 不会仅依据一个 RR 或陈旧数据恢复 direct peer group。
...
- `routerd` 收到 SIGTERM 或 SIGINT 后，会先停止 controller reconcile 和 control API，
  然后才执行可选的 Cloud SAM make-before-break handoff。它不再无限等待已经受理的配置变更。
  若该变更仍在执行，会跳过可选 handoff，并且不等待该变更持有的 SQLite 锁就退出；
  操作系统会关闭进程持有的 descriptor。空闲时仍会执行原有的有时限 handoff。

## v20260823.0054

### 修复

- Cloud SAM direct leaf 现在能区分“重启后的 RR 正在重新学习 8 个 leaf topology，
  因而 direct peer 列表暂时不一致”和真正的 enrollment 失败。leaf 保留已经验证的
  RR 路径，显示 `directTopologyPending`，并且只重试 topology GET；不会安装部分
  mesh，也不会反复重新提交 claim。对于曾丢失原始 refresh 诊断的短暂
  v20260822.2333 status，只进行一次只读恢复检查。
...
- `routerd-bgp` 现在会限制 SIGTERM 时本地 control socket 的排空时间和 GoBGP 的停止
  时间。即使仍有卡住的请求或 peer，守护进程停止也不会无限期阻塞主机关机；超过上限后
  会关闭自己拥有的 socket 并退出。
...
- Cloud SAM direct leaf 现在只会在一个 RR 在 clean boot 后明确报告丢失当前 client identity、且其他
  RR 都确认该 identity 或返回相同的明确缺失响应时，才把 claim 重新提交给所有 RR。这样无需等待很长的
  RR lease 到期即可恢复可选的 direct mesh；旧版/混合 RR、revoke 或其他 refresh 失败仍会移除 direct
  group 并保留安全的 RR fallback。
- 明确被 revoke 的 enrollment claim 不会被当作 RR 状态丢失处理。direct leaf 不会自动重新提交它；
  在 claim 完成 rotate 前，RR 会拒绝重放已 revoke 的 client identity。
...
- Ubuntu Live ISO 现在会在镜像的初始 systemd 启动事务中启用 `routerd.service`，并确保它
  在 `routerd-live-setup.service` 完成后启动。不再由 setup script 第二次启动 routerd，
  因而已经可用的 control socket 不会短暂消失。

## v20260822.1852

### 修复

- 当 `routerd serve` 仍在 bootstrap 时收到 systemd restart，现在会取消 bootstrap 并迅速退出，
  不再让 SIGTERM 一直挂起到 service manager 强制杀死进程。正常的 MobilityPool graceful stop
  会与正在运行的 controller generation 隔离。

- Mobility graceful stop 现在从 GoBGP 的 live selected RIB 确认 peer 接管；不再把本机
  `applied.json` dynamic-path journal 当作 remote peer 已发布路由的证据。
...
- 对于尚未获得 mobility `/32` 的 direct leaf，`routerctl mobility leaf-config` 现在允许省略
  `--owned-address`。在提供真实 owned address 前，生成的配置不会创建 service address、BGP export
  或 redistribute route。

- 已签名的 Cloud SAM direct leaf 即使当前 `ownedAddresses` 为空，也会保留在 pair-stable
  direct BGP transport 中。在出现已签名的 owner `/32` 前，该 peer 专属 import policy 会拒绝
  所有 route，因此无需编造 IP，RR 转发仍是安全的回退路径。
...
- 普通 GoBGP peer 的 Cloud SAM import admission 和路由优先级现在通过 GoBGP 实际评估的
  neighbor-scoped global RIB policy 应用。因此 direct leaf path 的 local preference 为 200，
  高于 RR fallback 的 100，不会意外形成等价成本路径集合。`routerd-bgp` 重启后也会重建相同的
  policy，包括没有 prefix 的 direct peer 优先级规则。
...
- Cloud SAM direct leaf 现在会区分正常、尚未到期的 RR lease 更新计划和失败请求的 retry backoff。
  先加入的 leaf 不必等待整个 lease 到期，就会通过计划中的 GET-only topology refresh 发现后加入的
  direct peer。
- 与 SAM transport `innerPrefix` 重叠的 BGP 路由仍可在 RIB status 中观察，但不会安装到 kernel FIB。
  这段 point-to-point 地址空间已由 tunnel interface 拥有，因此 RR 反射的 aggregate 不会再让自己的
  next hop 不可达。
...
- 使用经过验证的 Event Federation 的 SAM `onprem-l2` leaf，现在会在 CE 请求尚未解析的 ARP target
  时共享短生命周期的观测事实。符合条件的 remote leaf 会通过现有的 `on-demand-arp` observer
  对目标地址执行带有 rate limit 的 probe，因此不必等待完整的 proactive prefix scan 就能找到
  quiet client。group、member、pool、prefix、scope、subject、expiry、source node 和 observer
  readiness 检查会限制 fanout；不会延伸原始 L2 frame。

## v20260820.1911

### 变更

- 以日语为起点重写初学者文档，并同步英语、繁体中文和简体中文入口。第一段导览现在说明
  WAN、LAN、DHCP、DNS、YAML resource、独立 validation、隔离 dry-run，以及何时可使用
  `routerctl`。
- 修正可直接复制的 IPv4 NAT、访客隔离、DS-Lite 和 multi-WAN 说明，使其与 YAML 一致，
  并清晰标明操作安全边界。

### 修复

- `routerd apply --once --dry-run` 现在会将 DHCPv4/DHCPv6 的 lease path、dnsmasq、
  firewall 和 Path MTU artifact 保留在临时 artifact directory，而不是普通 runtime
  path。生成的 dnsmasq RA parameter 也会用 dnsmasq 接受的语法表示派生 MTU。
...
- 修复刻意不设置 `MobilityPool` 的 static direct-IPIP route reflector：现在只有通过
  transport 拥有的 RR-client peer scope（显式 prefix、精确 `/32`、peer neighbor 和
  node-identity community）的 remote mobility `/32` 才会进入 FIB。此 profile 必须在
  `spec.bgp.importPolicy` 显式声明 IPv4 mobility prefix 与 `/32` bounds；未设置边界的
  no-pool RR 会由 validation 拒绝，而不会在 control plane 显示正常时遗漏 FIB route。
  Proxy-ARP local capture intent 也会直接输出 tunnel-to-capture forwarding path。
...
- `Bridge` 和 `VXLANTunnel` 现在具有 operational controller，不再只依赖
  rendered artifact；controller 会 reconcile persistent 与 kernel state，并报告
  实际的 member、MTU、STP、VXLAN 和 forwarding health。
- `VXLANTunnel.spec.outerDF` 可明确选择 outer IPv4 DF 的 `inherit`、`set` 或
  `unset`，并验证 live setting。
- opt-in 的 `VXLANTunnel.spec.tcpMSSClamp` 会根据声明的 overlay MTU 推导
  downward-only IPv4/IPv6 bridge-family MSS rule。private generation、token 和
  table handle identity 会在 activation、recovery 和 cleanup 时保护 foreign
  nftables state。
- release operator 现有 binary-first rollback 流程。config rollback 必须明确
  选择，并记录 UTC provenance、SHA-256 evidence、canonical commit 验证和
  readiness gate。

### 修复

- VXLAN active/standby forwarding 可通过使用 VRRP role 和 external witness 的
  `spec.when` 进行 gate。unknown、stale 或 revision 变更时会 fail closed；
  revision barrier、ownership token/ifindex、crash recovery 和 exact-ifindex
  teardown 可防止 split-brain forwarding 和 foreign deletion。
...
- DNS 查询日志的 SQLite 写入失败时，现在会重新打开连接并重试一次记录，避免临时或
  失效的日志连接让 resolver 的可观测性在重启前停止（#857）。
...
- `NAT44SessionSync` 的 event-stream mode 现在会在初始 snapshot 时移除 standby
  端匹配的 stale conntrack entry，并持久化 worker lifecycle 与 sync counter，使
  `routerctl describe` 无需等待下一次 reconcile 即可显示运行中的 stream 状态。

### 修复

- NAT44 event replay 现在会忽略没有有效 state 的 TCP conntrack event，以避免错误的
  restore operation。已在两台 router 上验证 snapshot、standby 提升和 fallback，
  `RestoreFailed` 与 `insertFailed` 均为 0。本版本未更改 CloudEdge transport、BGP、
...
- stale state cleanup 现在会在删除前，只将即将删除的 object status row 写成
  JSON snapshot，而不是复制整个 `routerd.db`。在 state database 很大的系统上，
  这可避免 restart 时产生巨大的 backup write (#795)。

### 修复

- 待机系 router 对已经 inactive 的 active-only resource，不再反复改写
  `WhenFalse` status detail，也不再反复送出 no-op 的 IPv4 static address
  removal event (#794)。
...
- v20260707.1514 已提升为 production-recommended stable release。面向新部署的
  stable milestone 现在指向固定 release tag，而不是会移动的 `latest` URL。
- release qualification 现在记录已接受的 AWS/Azure/OCI/PVE 冗余 full
  topology 结果：8 clients、8 leaves、2 个 AWS route reflector、matrix
  56/56、provider 收敛 4s、dataplane 收敛 567s、cleanup state 0。同一次
  qualification 还确认生成的 config/control schema 与 website schema copy byte
  一致，并且 `make check-schema check-website-schemas` PASS。
...
- mobility holder transition timing event 现在公开 dataplane
  `seize-complete` 与 provider `capture-confirmed` milestone (#719)。
- Azure provider executor 的 action history 现在记录每次 az 调用的观测信息，
  并可在启用后采集 ARM rate-limit header (#720)。
- 新增 fabric-wide pair-stable `/31` 碰撞检测 warning，可在 transport address
  conflict 变成运行时故障前发现问题 (#721)。
- MobilityPool 现在可以 opt-in seize 时的 scoped conntrack cleanup；默认关闭
  (#726)。
- route-table capture strategy 的行为已由 provider-contract simulator test
  覆盖 (#724)。

### 变更

- 文档现在将 route-table strategy 标记为 uncertified，并记录 release
  qualification policy 与 environment certification contract (#717, #723)。
...
- PVE minimal dynamic RR 示例已更新为实机 small-topology test 形状：RR x2、
  leaf a/b 加 leaf c/d、pseudo client 位于不同 client LAN、无 host route、
  明确的 proxy-ARP `RemoteAddressClaim`、BGP delivery、capture source address，
  以及 client-LAN link route。已在 tofu-backed PVE VM 上验证
...
- Release 工作流不再将慢的 Web Console screenshot 任务视为发布阻塞。
  v20260528.0751 已实际产出 release commit 与 tag，但 screenshot
  任务的 13 张截图在 CI runner 上耗时 10 分 21 秒，触发了 #40 期为
  防止 SSE hang 而设置的 `timeout-minutes: 10`，Quality 工作流被
  判失败，依赖它的 build / publish 任务被全部 skip，因此本想搭载
  heap leak 修复的二进制未能上架 GitHub Releases。screenshot 仅
  是文档站点的参考图，并非 routerd 二进制的契约。本次为
  `webconsole-screenshot` 任务加上 `continue-on-error: true`，任务
  失败仅作记录，不再传播到 Release 工作流的 `needs: [quality]`。
  同时把 `Capture Web Console screenshots` 步骤的 `timeout-minutes`
  从 10 提到 15，给较慢的 runner 一些余量。`v20260528.0751` 标签
  因这次失败未能发布，本次 release 以相同的 heap leak 修复加上该
  CI 守护取而代之。
...
- HealthCheck 每次探测结果都会记录 egress / source / route 证据，
  并按 resource 保留滚动历史 (#37)。`pkg/healthcheck` 的 `State`
  增加 `FirstFailureTime` / `LastFailureTime` / `LastSuccessTime` /
  `FailureCount` / `History []ProbeRecord` / `LastEvidence`。每个
  `ProbeRecord` / `ProbeEvidence` 携带 `FailureKind`（timeout /
  ...
  `RouteSource`、`TunnelLocal`、`TunnelRemote`。Linux 上调用
  `ip -j route get` 获取 nexthop / oif / src，非 Linux 走 stub，
  跨平台编译不受影响。`cmd/routerd-healthcheck` 新增
  `--source-origin` / `--tunnel-local` / `--tunnel-remote` 运维提示
  flag，可标注探测无法自动推断的信息。事件属性
  ...
  `firstFailureAt` / `failureCount`) 与 `StatusMap` 都已纳入新字段，
  因此 `routerctl show / describe` 经由现有 status map 即可显示。
  历史默认 20 条，可通过 `ROUTERD_HEALTHCHECK_HISTORY` 覆盖。
- 通过 control API 公开每个 controller 的 reconcile 失败历史 (#38)。
  `ControllerStatus` 增加 `ReconcileErrorHistory
  []ReconcileErrorEntry` 与 `MaxDurationAt *time.Time`。每个
  `ReconcileErrorEntry` 含 `StartedAt` / `CompletedAt` / `Duration`
  / `DurationMs` / `Trigger` / `ResourceKind` / `ResourceName` /
  `Error`。controller framework 新增可选 `ResourceObserver` 接口，
  让 runtime store 在每次 reconcile 中能接收 resource kind / name，
  既有 Observer 实现完全兼容。历史仅保存在内存中（持久化为本
  issue 的 out-of-scope），每个 controller 默认 20 条，可通过
  `SetErrorHistoryLimit` 覆盖。`routerctl status --show-errors`
  在 table 模式下以纵向块呈现每个 controller 的错误历史；JSON /
  YAML 输出通过现有 StatusMap 路径自动包含新字段。新增
  `routerctl doctor reconcile --since <duration>` 子检查，会查询
  只读 status socket，在指定窗口内统计 reconcile 错误，按
  pass / warn (≥1) / fail (≥10) 判定，并在 detail 中给出至多
  5 条样本。`parseDiagnoseOptions` 同步新增 `--since` 与
  `--status-socket` flag。

### 修复

- 即使 polling 客户端以小于 `IdleTimeout` 的间隔不断请求，
  `routerd serve` 在控制 / 状态 socket 上也不再泄漏 Unix socket
  file descriptor (#40 的后续修复)。v20260528.0244 的 #40 修复仅设
  超时，但 polling 让 keep-alive 连接始终保持非 idle，IdleTimeout
  从不触发，结果 homert02 v20260528.0244 上 `routerd.db` fd 按 #39
  保持 4 不变，而 `all_fd` 仍然以约 +4/分钟的速度增加。本次修复
  在两个内部 API server 上调用
  `http.Server.SetKeepAlivesEnabled(false)`，同时让
  `controlapi.NewUnixClient` 在 `Transport.DisableKeepAlives` 中
  设置 `true`。每个请求在 response 之后立即关闭连接，长时间运行
  也不会让 socket fd 漂移上行。read / write / idle 超时仍保留作为
  对异常 peer 的保险。Unix socket accept 成本很低，本修复在任意
  hot path 上都不会造成实质的重新拨号开销。
...
- 将 BGP controller backend 替换为基于 GoBGP 的长生命周期 `routerd-bgp`
  ...
  类型化的 GoBGP API object，`apply` 不再渲染 FRR artifact，`routerd`
...
- `routerd apply` 的 VRRP 处理现在复用与 daemon mode 相同的
