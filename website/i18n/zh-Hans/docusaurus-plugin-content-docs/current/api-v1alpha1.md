...
| `SAMEnrollmentClient` | leaf 侧 bootstrap/refresh controller：向 bootstrap RR endpoint 提交本地远程请求的 `SAMEnrollmentClaim`，获取允许的 `SAMRRSet` 和符合条件的直连 peer group；仅在缺失、接近到期或 claim material 变化时将它们保存为 dynamic state。它不需要本地 policy 或 NodeSet。 |
