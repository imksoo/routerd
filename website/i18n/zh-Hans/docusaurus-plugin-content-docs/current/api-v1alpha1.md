...
| `MobilityPool` | CloudEdge 的本地 address/capture intent。声明 pool prefix、federation group、BGP delivery、cloud capture profile、local value expansion、provider trap placement 与一个 local self overlay；共享 topology 只能通过 `membersFrom` 从 `SAMNodeSet` 导入。 |
| `SAMNodeSet` | 共享 SAM fabric 的 identity/topology registry：node identity、site、role、placement、maintenance、Event Federation endpoint、SAM transport endpoint 与非秘密 WireGuard peer identity。它是 MobilityPool 唯一的 topology source。 |
| `SAMRRSet` | 返回给已接纳 leaf 的仅运行时、以 policy 为作用域的 enrollment snapshot。它包含从 policy 的静态 `SAMNodeSet` 投影出的已选 RR node，不能在顶层 router YAML 中声明。 |
| `SAMEnrollmentPolicy` | 仅 hub/RR 使用的 SAM transport enrollment claim 授权。它绑定 transport profile、输出 `rrSetRef`、必需的静态 RR topology `rrNodeSetRef`、join-token source、join audience、tunnel/endpoint prefix、可选 leaf ID pattern、TTL、显式 claim revoke、可选 WireGuard materialization，以及作为 RR 侧 admission authority 的 `MobilityPool` reference 或直接 `mobilityPrefixes`。`directMesh.peerGroupRef` 还能授权一个可选的 leaf 间直连组；RR 始终保留为回退。leaf 不声明副本。 |
| `SAMEnrollmentClaim` | leaf 本地的远程 enrollment request：leaf identity、join nonce/timestamp/HMAC、远程 policy/RRSet reference、tunnel address、endpoint、可选且签名的 `directMesh` opt-in、可选 BGP identity、可选由 MobilityPool 拥有的 `/32`、可选 WireGuard credential、expiry 和 revocation state。 |
| `SAMEnrollmentClient` | leaf 侧 bootstrap/refresh controller：向 bootstrap RR endpoint 提交本地远程请求的 `SAMEnrollmentClaim`，获取允许的 `SAMRRSet` 和符合条件的直连 peer group；仅在缺失、接近到期或 claim material 变化时将它们保存为 dynamic state。直连 peer 必须由所有 RR 对同一 client identity 和 topology 进行证明。如果所有 RR 已接受同一 claim、但 peer 列表暂时不同，则状态会显示 `directTopologyPending: true`：继续走 RR，不使用部分直连路径，并只用 GET 重新检查。配置 `joinTokenFrom` 时 identity 由 HMAC 认证；未配置时属于 trusted control-plane 输入。RR 仅丢失易失 admission state 时，只接受当前 RR 的 identity-aware 响应来自动重新提交；revoke、renewal 期间不同的 active identity、损坏响应、旧版/混合 RR 或任何不确定性都会保留 RR fallback。它不需要本地 policy 或 NodeSet。 |
| `SAMTransportProfile` | 声明 router 稳定的 `selfNodeRef`、`mode`（`ipip`、`gre`、`fou` 或 `gue`）、`encryption`（`none` 或 `wireguard`）、inner tunnel prefix、underlay interface、BGP router 和 peer source。`fou`/`gue` 使用现有 `TunnelInterface` FOU/GUE 路径并要求 `encapSport` 与 `encapDport`。它通过 `peersFrom` 从 `SAMNodeSet`、运行时同步的 `SAMPeerGroup`、`SAMEnrollmentPolicy` 或已获取的 `SAMRRSet` 读取 topology 和 peer endpoint；`SAMNodeSet` 是 canonical identity/topology/endpoint source，`nodeRefs` 可选择相邻 peer。enrollment 返回的 `SAMPeerGroup` 标记为 `direct: true` 时，会增加一条可选的 leaf 间直连路径，并赋予较高的 BGP local preference；它必须使用 `pair-stable` 地址分配，并且必须排在一个非 optional `SAMRRSet` 回退 source 之后。direct profile 必须显式设置普通 RR 的 `bgp.importPolicy.localPreference`，`bgp.directLocalPreference` 必须更高。省略 `bgp.timers`、`bgp.timersPreset` 和 `bgp.convergenceProfile` 时，生成的 peer 会继承所引用 `BGPRouter` 的默认值；profile 中的设置优先。`SAMPeerGroup` 不能作为可 author 的顶层 resource。routerd 通过 replace-on-reconcile 的 `DynamicConfigPart` 为每个 peer 导出 `TunnelInterface`、endpoint `/32` `IPv4Route`，以及（除非 `spec.bgp.generatePeers: false`）`BGPPeer`。 |
...
CloudEdge Mobility 保持声明式的 operator-authored surface：`MobilityPool` 是高层
address/capture intent，`SAMNodeSet` 是 write-once node identity registry 和生成 peer 的
共享 membership source。`SAMEnrollmentPolicy` 通过必需的 `rrNodeSetRef` 选择静态 RR
identity/topology，并用 `rrSetRef` 命名获取的 runtime snapshot；它仅用于 hub/RR，
而 `SAMEnrollmentClaim` 是 leaf 的远程请求。`SAMEnrollmentClient` 是 leaf 的
bootstrap/refresh controller，它无需本地 policy 或 NodeSet 即可获取该 snapshot。
`SAMTransportProfile` 是高层 transport/BGP intent；federation event 是 observed fact，
BGP best path 是 mobility ownership/delivery view。mobility planner 生成 BGP `/32`
advertisement 与 provider trap action plan；operator 不应手写 per-address path 或 capture
procedure。
`MobilityPool` 一次性计划地址 ownership、capture 与 BGP delivery，并将类型化的本地
dataplane intent 与 FIB verdict 保存到 `DynamicConfigPart`。

每个 pool 通过 `membersFrom` 导入同一个 `SAMNodeSet`，并且只保留本节点的
`nodeRef`、capture、discovery、`profileRef` 与 static-owned-address overlay。
`site`、`role`、placement、maintenance 和 capacity 只能在 `SAMNodeSet` 声明。
`SAMRRSet` 只是在 enrollment fetch/dynamic-state 路径中供 transport 的 `peersFrom`
使用的运行时 RR snapshot；它不可作为顶层 YAML resource，也绝不替代
`MobilityPool` 从 `SAMNodeSet` 导入的 authoring 或 membership/topology source。

`MobilityPool` 始终使用 BGP delivery，不公开 delivery mode selector。Provider action
plan 是 review artifact，只有在导入 action journal 并通过 `ProviderActionPolicy`、approval、allowlist
...
BGP 作为 `routerd serve` 的管理对象显示于 status。`routerctl get BGPRouter` 显示从存储的
...
记录。若由外部 AD DNS 等管理名称，请配置 `spec.externalDNS: true`。routerd 仅验证 hostname 语法，不发出 DNSZone coverage 警告也不自动发布。`routerctl get VirtualAddress` 显示角色、
...
`routerctl get IngressService` 显示 active backend 及各 backend 的健康状态。
同时显示 `hairpinMode`、是否需要 hairpin，以及预期的 nftables SNAT 规则是 present 还是 missing。调试 live dataplane 时，请直接检查主机的 `ip`、`nft` 和 conntrack state。从 Ingress、NAT 系、DS-Lite、IPv6 PD/RA、路由资源导出转发、redirect 抑制、reverse path filter 例外、各接口的 RA 接收等所需的 runtime sysctl。`routerctl apply` 会 plan / render 衍生配置，但主机变更仅限于明确的 `Sysctl` / `SysctlProfile` escape hatch。
衍生 runtime 配置的应用由 `routerd serve` 的控制器调和（reconcile）负责。
维护期间可用 `routerctl ingress drain
ingress/<service> backend=<name> --duration 10m` 将 backend 设为 drain 状态。控制器在 duration 结束或执行 `routerctl ingress undrain
