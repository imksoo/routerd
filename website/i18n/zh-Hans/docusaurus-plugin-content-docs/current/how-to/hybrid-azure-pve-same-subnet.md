...
- 不要让 Ubuntu 来宾 OS 持有已捕获的 `/32`。cloud-init 或 netplan 可能会自动为辅助 NIC IP 分配地址。请抑制或删除该配置。routerd 在每次 BGP `provider-secondary-ip` reconcile 时都会从本地接口 de-assign 该特定地址，并维持地址不存在的状态；即使 provider capture 将辅助 IP 保留在 Azure 也同样如此。
...
- 远端 owner 的 `/32` 交付路由由 BGP best-path import 经由 tunnel 接口得到，不变更默认路由。
...
对于 BGP `provider-secondary-ip`，确认已捕获的 `/32` 不存在于本地 `ip addr` 中、交付路由指向 tunnel、`ip_forward=1`。对于 `proxy-arp`，确认 `proxy_arp=1`、proxy neighbor、指向 tunnel 的交付路由、`ip_forward=1`。
