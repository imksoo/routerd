...
当前发布认证有意比可生成的功能列表更窄。FreeBSD IPv6 `EgressRoutePolicy` 会被明确拒绝，因为已认证的 PF `route-to` 仅覆盖 IPv4 static routehost。`TunnelInterface` 的 gif/GRE 以及发布 package 的 install/upgrade/uninstall 仍在完成 native 认证。已生成的 Tailscale 与 CARP rc.d artifact 可以使用，但其 lifecycle/failover 尚不是发布声明。
...
| IPv6 policy routing | 已认证的 PF `route-to` 仅为 IPv4 static routehost source affinity。 | FreeBSD IPv6 `EgressRoutePolicy` 被明确拒绝；这是经批准的 product boundary，而非已实现的 parity（[#904](https://github.com/imksoo/routerd/issues/904)）。 |
