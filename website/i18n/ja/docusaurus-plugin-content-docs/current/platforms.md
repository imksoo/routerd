...
FreeBSD でも `ClientPolicy` を利用できますが、DHCPv4 reservation を基にした IPv4 の pf 近似です。
guest または isolated の classification では `DHCPv4Reservation` を参照し、その予約 IPv4 アドレスに対する pf ルールを生成します。
これは Linux の MAC ベース隔離と同等ではありません。pf は nftables で使う Ethernet 送信元 selector に一致できず、IPv4 reservation から IPv6 の guest egress deny も生成しません。
IPv6 通信も隔離する必要がある場合は、明示的な IPv6 ポリシーを別途設定してください。この動作は 2026-05-11 に router04 FreeBSD lab で検証済みです。
dual-stack guest deny の検討は [#849](https://github.com/imksoo/routerd/issues/849) で追跡します。
...
| 領域 | 現在の差分 | 残課題 |
| --- | --- | --- |
| CI と実機検証の網羅 | CI は Ubuntu 上で単体テストと Linux 向け静的リンクの確認を実行します。FreeBSD はリリース時にクロスビルドします。 | FreeBSD VM のスモークジョブを追加し、検証、プラン、実際のパッケージマネージャー確認、サービス有効化、レンダラーの構文確認まで流すようにします。 |
| FreeBSD の機能制約 | `ClientPolicy` は DHCPv4 reservation ベースの IPv4 pf ルールを使います。MAC アドレス照合はできず、IPv4 reservation から IPv6 guest egress deny は生成しません。 | 制約を明示したまま維持し、IPv6 guest deny は dual-stack identity モデルを設計してから検討します（[#849](https://github.com/imksoo/routerd/issues/849)）。 |
