...
- FreeBSD に Linux `IP_FREEBIND` 相当がないため、non-local DNS resolver bind を明示拒否

direct BPF の ARP/RA observer と libndpi/DPI の実配信は未完です。native build と capability status はありますが、受理済みの FreeBSD packet delivery 観測はまだありません。
...
| 領域 | 現在の差分 | 残課題 |
| --- | --- | --- |
| CI と実機検証の網羅 | PR CI は FreeBSD amd64/arm64 binary と test package を compile し、release automation は FreeBSD archive を作成します。VM115 の native evidence は validate/render、pf/dnsmasq 構文、rc.d status、route lookup、BFD、対応 PF dataplane slice を対象にします。 | 保持済み VM evidence は lab acceptance であり、まだ自動 CI VM job ではありません。自動 runtime coverage を主張する前に provisioned FreeBSD VM smoke workflow を追加します。 |
