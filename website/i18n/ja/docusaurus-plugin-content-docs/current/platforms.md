...
現在のリリース認定は、生成できる機能の一覧より意図的に狭くなっています。FreeBSD の IPv6
`EgressRoutePolicy` は、認定済みの PF `route-to` が IPv4 static routehost に限られるため明示的に拒否されます。`TunnelInterface` の gif/GRE とリリース package の install/upgrade/uninstall は現在も native 認定中です。生成済みの Tailscale と CARP の rc.d artifact は利用できますが、その lifecycle/failover はまだリリース認定として主張しません。
...
| 領域 | 現在の差分 | 残課題 |
| --- | --- | --- |
| CI と実機検証の網羅 | PR CI は FreeBSD amd64/arm64 binary を compile します。provisioned FreeBSD 14.3 の native evidence は、省略なし `go test ./...`、live routerd smoke、ARP/RA observer、native nDPI と amd64/arm64 両方の runtime certification を対象にします。保持済み VM115 evidence は route lookup、BFD、対応 PF dataplane slice も対象にします。 | 現在の release package lifecycle は、amd64/arm64 ごとの native install/upgrade/uninstall evidence を認定中です。 |
...
| IPv6 policy routing | 認定済み PF `route-to` は IPv4 static routehost の source affinity に限られます。 | FreeBSD の IPv6 `EgressRoutePolicy` は明示的に拒否されます。これは実装済み parity ではなく承認済みの product boundary です（[#904](https://github.com/imksoo/routerd/issues/904)）。 |
