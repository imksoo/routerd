...
- FreeBSD の DHCPv6-PD レンダラは、`dhcp6c` で委譲プレフィックスを
  ...
  安定したサフィックスを二つ目のアドレスとして追加します。`dhcp6c` が
  下流アドレスを残していない場合でも、routerd が保存している
  プレフィックス委譲リースから LAN 側アドレスを導出し、
  `ifconfig ... prefixlen 64 alias` で設定できます。
...
- FreeBSD の反映処理では、生成した設定ファイルまたは該当する rc.conf
  ...
  `dhcp6c` を再起動します。通常の反映で不要な DHCPv6 Release を送らない
  ためです。生成設定では `dhcp6c` を `-n` 付きで起動し、必要な再起動でも
  SIGUSR1 で止めてから起動し直します。可能な限り委譲プレフィックスを
  解放しないための扱いです。
