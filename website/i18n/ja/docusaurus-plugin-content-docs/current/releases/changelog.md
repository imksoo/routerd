...
- 直接アクセス・自動更新でもDS-Liteリソースを取得。ブラウザー回帰テストを追加し、Web Console CIへ組み込みました。

### 検証

- 候補版`714e2cd5`は両系の画面直接アクセス・自動更新、HTTPS 480/480、4トンネル疎通の成功が報告されています。以前の手集計には非NAT監視通信が混入しており、SNAT件数の比較基準には使えません。公式アセットは別途展開検証が必要です。推奨Stableは`v20260914.0729`を維持します。

## v20260914.0729

### 修正

- 正常公開済みMASTERのVIPを、一時的なreadiness低下でも保持します。同一アドレス・
  インターフェースの公開履歴は観測エラーでも保持し、エラー自体は状態に記録します。
  降格・VIP不在の確認・公開失敗では履歴を失効させ、初回公開条件は維持します（#1248、#1252）。
- 設定世代切替で設定未変更の管理下DHCPクライアントを維持し、終了済みsupervisorを
  再作成します。削除・設定変更・serve終了時のライフサイクル処理は維持します（#1249、#1252）。
- 更新時に現行の生成`routerd.service`のCapability・Environmentを保持します。
  削除済み`--controller-chain`を含む旧unitは引き続き移行します（#1250、#1252）。

### 検証

- 候補版`f43b4451`のローカル試験・CI、2台のルーターの更新・reload、隔離Linux netnsでの
  観測障害試験が成功し、報告されたクライアントHTTPS試験は360/360成功しました。
  以前の候補版でのDNSタイムアウト1件は原因未確定です。あらゆる条件での無停止や、
  AWS/Azure/OCIの新たな実機認証を意味しません。証跡はPR #1252に記録しています。
...
- `LogRetention` が未設定でも SQLite イベントジャーナルを24時間、10万行、論理ペイロード
  64 MiB以内に制限し、運用状態テーブルは削除対象外としました。ストレージ満杯時の
  ジャーナル書き込み失敗でローカルの実行時イベント配送が停止しないようにしました。
  `EventJournalReadOnly` は実行時ステータス、`routerctl doctor`、DB外の実行時アラート
  ファイル、systemdサービス状態から確認できます。揮発性Live ISOと永続ディスクでは、
  それぞれに適した復旧案内を表示します（#1223）。
...
- VRRP の降格時に、クライアント側 VMAC の停止後、WAN VMAC の停止前に直下の `ip6tnl` を
  停止するようにしました。待機系への調整がロール連動の DS-Lite トンネルを削除するまでの間も、
  ローカルのヘルスチェックが利用不能な外側 IPv6 送信元へ送信することを防ぎます。
...
- VRRP の降格時は、WAN VMAC より先にクライアント側 VMAC を停止するようにしました。
  これにより、外側 IPv6 送信元が利用不能になった後に転送パケットが DS-Lite トンネルへ
  入る競合を防ぎます。昇格時は従来どおり WAN 側を先に起動します。
...
- 待機系の PD スナップショットを VMAC へ反映する処理でも、永続化された VRRP の選出役割を
  確認するようにしました。これにより、古い待機系調整が新しい MASTER の VMAC を再び停止する
  競合を防ぎます。稼働系の委譲 IPv6 アドレスと DS-Lite トンネルは、DAD が各外側送信元を
  利用可能にするまで準備中として扱います。
...
- Cloud SAM に amd64 の実機 Linux/FreeBSD 混在検証を追加しました。2 台の
  CARP router、複数 client、AWS route reflector/leaf、双方向通信、active
  router 障害、非 preemptive rejoin、成功時のみの infrastructure cleanup を
  対象とします。

### 変更

- FreeBSD Cloud SAM capture は、native の route-aware proxy ARP、PF、CARP、
  cloned tunnel interface を使いながら、Linux と同じ利用者向け ownership と
  fail-closed lifecycle に従うようになりました。

### 修正

- CARP backup router が自身の mobility BGP `/32` path を withdraw するようにし、
  rejoin 後に ECMP traffic が silent backup へ送られる問題を防止しました。
- FreeBSD SAM reconciliation は proxy ARP ownership を保持し、foreign enabled
  state を拒否し、cleanup 前に owned proxy ARP を無効化します。また、安定した
  tunnel peer/name を導出し、active-open livelock なしで BGP state を復旧します。
...
- DNS クエリログの SQLite 書き込みに失敗した場合、接続を再オープンして一度だけ
  再試行するようにしました。一時的または古いログ接続によって resolver の観測が
  再起動まで停止することを防ぎます（#857）。
