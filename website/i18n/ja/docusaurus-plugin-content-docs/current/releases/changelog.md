...
- **取り下げ (同じ retreat で再度訂正)**: 旧 "Unreleased" に
  `DHCPv6PrefixDelegation.spec.recovery.mode` の default を `auto-request` に
  ...
  `auto-rebind` を明示で opt-in する。active controller は `routerd dhcpv6 ...`
...
- `routerd dhcpv6` CLI に `--iaid` を追加。`DHCPv6PrefixDelegation.spec.iaid`
  を編集せずに 1 発限りの IAID 上書きが可能に。
- `routerd dhcpv6 request` は Reply 記録の無いリソースで呼び出すと警告を
  出すようになった (direct-claim Request は当該 HGW で phantom binding を
  作るため)。
- `DHCPv6PrefixDelegation` の observability 判定が DHCPv6 transaction evidence
...
- `VXLANSegment`、`Bridge`、`IPv4StaticRoute`、`IPv6StaticRoute`、
  `DHCPv4Reservation` が ownership intent を出すようになり
...
- `DnsmasqConfig` は dnsmasq の rule が未観測の DHCP リース由来の
  ...
  dhcpv4|dhcpv6`、`DHCPv4Scope.dnsSource: dhcpv4`、
  `DHCPv4Server.dns.upstreamSource: dhcpv4`) に、エラーで止まらず
...
- Added `DHCPv4Reservation` for dnsmasq-backed fixed IPv4 leases inside
  an existing `DHCPv4Scope`.
...
- `routerd serve` now observes WAN Router Advertisements for
  `DHCPv6PrefixDelegation`, accepts DHCPv6 client hook events over the local
...
- `routerd dhcpv6` now supports `solicit` and `rebind` in addition to
...
- DHCPv6 active-control packets sent by routerd are now summarized into
  `DHCPv6PrefixDelegation` status as recent transactions so operators can see
...
- `routerd serve` now starts a passive DHCPv6 packet recorder for
  `DHCPv6PrefixDelegation` on supported platforms. The Linux implementation uses
...
- `DHCPv6PrefixDelegation.spec.recovery.mode` now controls daemon-side hung
...
- NixOS rendering now uses the same effective DHCPv6PrefixDelegation client
  default as apply, so omitted NTT-profile clients render `dhcpcd` packages
  and avoid enabling systemd-networkd DHCPv6-PD.
- Switching DHCPv6PrefixDelegation away from systemd-networkd now writes
  neutralizing networkd drop-ins for the WAN and delegated LAN interfaces, so
  stale `90-routerd-dhcpv6-pd.conf` files cannot keep networkd sending DHCPv6-PD
  packets in parallel with `dhcp6c` or `dhcpcd`.
- The systemd-networkd renderer now resolves the same effective
  `DHCPv6PrefixDelegation` client default as apply. NTT-profile resources with an
...
- `routerd apply` now clears observed DHCPv6 identity fields in
  `DHCPv6PrefixDelegation` status when the effective client changes, preventing
  stale networkd IAID/DUID values from appearing after a move to `dhcpcd` or
  `dhcp6c`.
- Linux NTT-profile `DHCPv6PrefixDelegation` now defaults to `client: dhcpcd`,
...
- FreeBSD NTT-profile rendering now starts KAME `dhcp6c` with `-n` so service
  ...
  delegated to `dhcp6c`.
- Linux `DHCPv6PrefixDelegation` can now use `client: dhcp6c`. This renders a
  ...
  lifetimes.
- `DHCPv6PrefixDelegation` can now use `client: dhcpcd`. It is the Linux
...
- `routerd apply` now resolves an omitted `DHCPv6PrefixDelegation.spec.client`
...
- `routerd dhcpv6 request|renew` can now override requested T1/T2 and IA Prefix
...
- `DHCPv6PrefixDelegation` now has manual `serverID`, `priorPrefix`, and
...
- Added the first DHCPv6 active-controller command path: `routerd dhcpv6
