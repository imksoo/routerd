...
- BGP controller backend を GoBGP ベースの長寿命 `routerd-bgp` daemon に
  置き換えました。`BGPRouter` と `BGPPeer` は local gRPC Unix socket 経由で
  型付き GoBGP API object に直接 map され、`apply --once` は FRR artifact を
  render せず、`routerd` restart でも BGP process を restart せず established
  session を落としません。peer/path status は `vtysh` text parse ではなく
...
- 無変更 reconcile の冪等性を強化しました。stale client daemon unit cleanup は
  status/event に記録し、static/DHCP IPv4 route は live kernel route が一致する
  場合に skip し、動的 nftables address set は set 全体を flush せず element 差分で
  更新します。NTP/BGP の service 操作理由も status/event で確認できるようにしました。
...
- BGP controller の FRR readiness 判定から TCP VTY gate を取り除き、
  `vtysh -c "show running-config"` を control-plane probe と running config
  diff の入力として使うようにしました。これにより TCP VTY を無効にした
  Alpine FRR build でも初回収束時に `frr-reload.py` へ到達できます。
- FRR control 不可、権限不足、reload 試行、reload 後の反映不完全を status で
  明示するようにしました。
- Alpine Live ISO の autostart が、既に `routerd serve` が動いている場合に
...
- BGP controller reconcile 中の一時的な FRR reload lock 失敗を retry し、
  ...
  ようにしました。
- Alpine Live ISO の DHCP client を初回 lease 後も常駐させ、live router 用の
  安定した DHCP hostname を派生し、既定では DHCP option 61 を送らないことで
  Windows DHCP reservation が Ethernet MAC に一致し続けるようにしました。
