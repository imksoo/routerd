...
- VRRP のデーモン lifecycle は コントローラーランタイム に限定しました。
  `routerd apply --once` は keepalived の成果物を生成するのみで、reload / restart は
...
- BGP の `apply --once` を デーモン lifecycle から分離しました。`routerd apply
  --once` は FRR config とデーモン artifact の render のみ行い、`bgpd` の
...
- Kubernetes edge 用に、FRR backend の `BGPRouter` / `BGPPeer`、
  ...
  backend health/failover コントローラー を追加しました。
