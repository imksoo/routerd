...
- オーバーレイピアアドレスの明示的な `/32` ルートリソースを追加:
...
CloudEdge Event Federation Phase 2 トランスポート専用スモークが通過:
...
- EventSubscription、プラグイントリガー、DynamicConfigPart、ARP オブザーバー、プロバイダーアクション、クラウドの変更操作は使用されていない
