...
- クラウドルーター Azure NIC セカンダリキャプチャアドレス: `10.77.60.9`
...
- routerd の reconciliation 後、`routerd-cloud` のゲスト OS は `10.77.60.9` をローカルインターフェースアドレスとして保持しなかった。
- `10.77.60.9/32` は `wg-hybrid` 経由で配送。

## クラウド側

- `RemoteAddressClaim/onprem-client-10-77-60-9` は `Ready`。
- キャプチャタイプは `provider-secondary-ip`。
...
- エビデンスは Azure VM deallocate 前にキャプチャ。
- Azure VM はエビデンスキャプチャ後に deallocate; リソースグループは tear down せず。

## 既知のノート

- `routerd_filter` テーブルが利用不可の場合、FORWARD ポリシーの doctor チェックはスキップ; データプレーンスモークはそれでもパス。
- router06 のグローバルステータスは `Pending` のままだが、`doctor hybrid` はパスし SAM データプレーンパスは健全。
- 定常状態での `captureDeassignedOSAddress.deassigned=false` は、その reconcile 中に削除すべきアドレスがなかったことを意味; `enforced=true` + ローカルアドレスの doctor パスが関連するアサーション。
