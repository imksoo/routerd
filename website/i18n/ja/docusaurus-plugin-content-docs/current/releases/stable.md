...
の selectedSource 整合、ゲートウェイ健全性の独立画面化、`install.sh` の即時
...
- **Web Console のファイアウォール「Deny activity」を軸ラベル付き棒グラフに**
...
- **コントローラーごとの reconcile 失敗履歴を control API に公開
  ...
  コントローラーフレームワークにはオプショナルな `ResourceObserver` 拡張を
...
- **ゲートウェイ健全性の UI が独立画面になり、表示が安定しました。**
  Web Console はゲートウェイ健全性を概要から独立画面に分離し
  （コネクション / クライアント と同じ構成）、`selectedPath` /
  `preferredPath` / `fallbackReason` / `failedProbes` /
  `lastTransition` を含む完全な根拠情報を表示します。概要には
