...
- FRR `lib/vty.c` の `vty_serv_start(addr, port, path)`: TCP リッスンは `port != 0` のときだけ有効になります。Unix の `<daemon>.vty` ソケットはこれとは独立です（`#ifdef VTYSH`）。TCP VTY を無効化したビルドでも、Unix ソケットは `/run/frr/<daemon>.vty` または `/var/run/frr/<daemon>.vty` に存在します。
...
- FRR が落ちている間、または `:179` がリッスンしていない間は、`routerctl get status` が `Healthy` を報告しない。
...
13. status の正確さ: 一度 Healthy だった状態のあとに FRR を強制停止（`systemctl stop frr`）すると、次の調整は `Healthy` ではなく `FRRControlUnavailable` または `FRRServiceDown` を表に出さなければならない。失敗している間、BGPRouter status の `lastSuccessTime` は進んではいけない。
