...
Provider-secondary-IP capture の completion は、provider-capture BGP 広告ではなく
provider action journal の事実から導出します。#707/#740 では source-capture path
の観測側の足場だけが作られ、本番の広告配線は一度も存在しませんでした。

capture `/32` を fabric 内に広告する仕組みは、fabric 内 capture route に実需要が
出た場合だけ再導入します。その場合は、誰が route を発信し、どの RIB が観測し、
どの条件を complete とみなすかという到達条件から設計し直します。
