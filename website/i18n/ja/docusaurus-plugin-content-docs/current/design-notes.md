...
systemd-networkd を使えます。NTT HGW向けでは、検証した
...
### 5.8 重要: 統合 WAN クライアントとして `dhcpcd` を評価する

assert: 現在の routerd 設計では WAN 側取得経路が分かれている: NTT 向けには
KAME/WIDE の `dhcp6c`、一般 Linux には任意で `systemd-networkd`、NixOS は
未解決のまま。これは歴史的経緯であって、プロトコル上必須の分割ではない。
IPv4 DHCP、IPv6 RA/SLAAC、IPv6 DHCPv6 IA_NA、IPv6 DHCPv6-PD を全サポート OS
で 1 つのクライアントが扱えるなら、レンダラ・テストマトリックス・運用者
体験のすべてが単純化する。

believe: `dhcpcd` (BSD-2 ライセンス、Roy Marples が活発にメンテ) はその統合
クライアント役の最有力候補である。nixpkgs、FreeBSD ports、Debian/Ubuntu の
apt repository すべてに同梱されている。1 デーモンで IPv4 DHCP、IPv6 RA、
SLAAC、IA_NA、IA_PD、INIT-REBOOT を扱える。Renew のデフォルト形は
NTT HGW が受理しやすい方向にある: server から得た T1/T2 をそのまま
送り返し、Reconfigure-Accept を含め、設定で multicast に固定できる。
これは `wide-dhcpv6` の既定が出すものよりも HGW との相性が良いと見込める。

assert: 統合クライアント方針はラボ証拠が前提条件。`dhcpcd` を本線に採用する
のは、HGW が T1 boundary の Renew を受理することと、`dhcpcd` の INIT-REBOOT
バイトが routerd のアクティブ Request と同じ扱いを受けることと、
`routerd apply` 周回でリース状態を不要に壊さずに保てることをラボ計測で
確認できたあとに限る。これらが揃えば `dhcpcd` は `wide-dhcpv6` (5.1) と
NixOS のギャップ (5.7) の双方を 1 つで解決し、Section 5 の WAN 側面を
「2〜3 個のクライアント」から「1 つ」に畳む。

完了条件: `pkg/render/dhcpcd.go` が `pkg/render/dhcp6c.go` と並んで存在し、
`IPv6PrefixDelegationSpec.client` が `dhcpcd` を受け付ける。Ubuntu、
FreeBSD、NixOS のラボ計測で、それぞれ canonical Solicit による初回取得と、
NTT HGW からの自然な T1 Renew が少なくとも 1 回成功する。design-notes
Section 1.3 に `dhcpcd` の per-message-type 観測挙動を、既存の
`systemd-networkd`、`wide-dhcpv6`、`dnsmasq` の行と並べて記載する。
NTT 向けプロファイルの既定は、ラボ証拠で勝った側のクライアントに更新する。

assert: 評価が成功すれば、`dhcpcd` が NTT/HGW WAN の routerd 推奨デフォルト
になる。`wide-dhcpv6` は移行期間と `dhcpcd` を導入できない環境向けに
サポートを継続するが、推奨デフォルトではなくなる。systemd-networkd の PD
経路は 5.1 で既に述べたとおり opt-in のままにする。
...
#### 6.1.x フェーズ 1 サブタスク: `dhcpcd` 評価をフェーズ 1 に組み込む

assert: Section 5.8 で `dhcpcd` を統合 WAN クライアント候補として導入した。
`dhcpcd` を採用するとフェーズ 1 のフック設計が単純化し (`dhcpcd` の hook
は OS 横断で統一されており、OS ごとに異なる `dhcp6c-script` バリアントより
楽に統合できる)、Section 5.7 (NixOS DHCPv6-PD クライアントのギャップ) も
同じ作業で潰せる。よって `dhcpcd` 評価はフェーズ 4 まで待たず、フェーズ 1
のサブタスクとして並行で進める。

サブタスクはフェーズ 1 の他の作業と並列に走る:

- `IPv6PrefixDelegationSpec.client = "dhcpcd"` の背後で `pkg/render/dhcpcd.go`
  レンダラを追加。
- Ubuntu / FreeBSD / NixOS のラボ計測: canonical Solicit による初回取得、
  自然な T1 Renew が少なくとも 1 回成功、HGW ハング状態時の挙動 (ハング検知の
  カバレッジ確認)。
- 5.8 の完了条件をラボ証拠が満たせば、NTT/HGW プロファイルの既定を
  `dhcp6c` から `dhcpcd` に切替。`dhcp6c` は移行用にサポート継続するが、
  推奨 default ではなくなる。
- ラボ証拠が中立や否定的なら `dhcp6c` を本線維持し、統合クライアント化は
  フェーズ 4 に戻す。

believe: `dhcpcd` 採否の判断はフェーズ 2 開始前に確定すべき。フェーズ 2 の
LAN 側リソースは「本線の WAN クライアント」を前提に書くため、本線が今後
入れ替わる前提でフェーズ 2 を開始すると後退りが発生する。

assert: どの WAN クライアントが本線になっても、Section 5.2 の routerd
アクティブ・コントローラーは残る。OS DHCPv6 クライアントは steady-state の
refresh を担当、routerd の `pkg/dhcp6control` は bootstrap と recovery を
担当する。この役割分担は OS クライアント選定とは独立で、`dhcpcd` 採用に
よって譲歩されない。
