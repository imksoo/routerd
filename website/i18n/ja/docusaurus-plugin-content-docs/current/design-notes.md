...
(`routerd dhcpv6 request`) が成功して lab `router03` の binding を満期近い
...
| `objects` | リソース単位の状態 JSON を保存します。例: `DHCPv6PrefixDelegation/wan-pd` のリース、DUID、IAID、時刻。 |
...
sqlite3 /var/lib/routerd/routerd.db \
  "select json_extract(status, '$.lastPrefix') from objects where kind = 'DHCPv6PrefixDelegation' and name = 'wan-pd';"
```
...
- OS クライアントから T1/T2 や寿命を取れる場合は、`DHCPv6PrefixDelegation` の
  状態表示をさらに強化します。現在値、最後に見えた値、観測した DUID/IAID、
  期待する DUID/IAID、最後に観測した時刻、警告は混ぜずに表示します。
- `DHCPv6PrefixDelegation` の観測根拠を見直します。LAN 側に過去の委譲アドレスが
...
`DHCPv6PrefixDelegation/<name>` の `objects.status` JSON が、決まった field 名で
...
assert: 各 DHCPv6 クライアント用の renderer は、`DHCPv6PrefixDelegationSpec` と
...
`DHCPv6PrefixDelegation/<name>` status に lease / wanObserved field を記録し、
...
```mermaid
flowchart TD
  A[DHCPv6PrefixDelegation apply] --> B{spec.acquisitionStrategy}
...
完了条件: apply を2回続けて実行するテストで、変更のない `DHCPv6PrefixDelegation` が
...
完了条件: `DHCPv6Scope` または関連リソースで、DNS を DHCPv6 オプション、RA の
...
`DHCPv6PrefixDelegation.spec.client` を省略した NixOS では、Linux の
...
`DHCPv6PrefixDelegation.spec.client` が空の場合、apply は次の既定値を選びます。
...
- WAN インタフェースの RA をリッスンして `wanObserved.*` と派生 Server
  Identifier を `DHCPv6PrefixDelegation/<name>` の status に記録する RA リスナーを実装。
...
- 診断用の低レベル手動動詞は残す:
  `routerd dhcpv6 solicit|request|renew|rebind|release` は送信した試行を
  `DHCPv6PrefixDelegation` の状態に記録し、自動経路が完成する前でも
...
- `DHCPv6PrefixDelegationSpec.client` で `networkd`、`dhcp6c`、`dhcpcd` を
  選べる状態を保つ。
...
- 上流 PD リースから複数の下流インタフェースに /64 を切り出して配るしくみを、
  完全に宣言的なリソースで実装。
- アドレスを DHCPv6 で受けたい LAN クライアント向けに、`DHCPv6Scope.mode`
