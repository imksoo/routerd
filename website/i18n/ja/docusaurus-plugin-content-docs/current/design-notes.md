...
assert: routerd の正は、YAML 設定と `routerd apply` が書き込む状態・所有台帳です。
ルータの挙動を変えるときは、ファイルを変更して apply することを基本にします。
生成済みファイルを手で直したり、apply の外からデーモンだけを動かしたりすると、
意図が git の履歴、差分、apply の結果、ローカルデータベースに残らず、後から
追跡できなくなります。
