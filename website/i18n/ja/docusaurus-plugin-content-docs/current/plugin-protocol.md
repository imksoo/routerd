...
Private Endpoint などを区別してください。`status.self` もローカル router instance の
`resourceRef` / `resourceType` を設定できます。SAM はこの identity を使って、router
instance に capture された secondary IP と、同じ provider subnet 上の本来の home client
address を区別します。
