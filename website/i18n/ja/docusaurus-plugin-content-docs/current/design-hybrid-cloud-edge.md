...
- L3 ハイブリッドルーティング。`HybridRoute` はリモートの IPv4 プレフィクスを `OverlayPeer` 経由でローワリングします。
...
`HybridRoute` は保守的な L3 の柱です。`OverlayPeer` 経由でローワリングすべきリモートの非デフォルト IPv4 プレフィクスを表します。ローワリングと status パスは意図的に明示的です。デフォルトルートは拒否され、運用者は通常の routerctl plan と dry-run のフローで生成されたルートの意図をレビューできます。
