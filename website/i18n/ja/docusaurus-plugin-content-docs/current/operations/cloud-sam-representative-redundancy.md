...
既定値の `topology_scale = "full"`、`clients_per_site = 1` を使います。
...
別途範囲を定めた開発用構成にのみ使え、この代表検証プロファイルでは拒否します。

費用を抑えるサイズは AWS leaf `t3.small`（2 vCPU・2 GiB）、AWS client
`t3.micro`（2 vCPU・1 GiB）、Azure `Standard_B1s`（1 vCPU・1 GiB）、OCI
`VM.Standard.E4.Flex`（1 OCPU・1 GB）、PVE guest（1 vCPU・1024 MiB）です。
AWS の片 leaf が引き継ぐアドレスは remote client 3 個と primary IP で最大 4 個です。
単一 ENI に IPv4 4 個を持てる `t3.small` に合わせ、プロファイル内で
`SAM_E2E_MAX_SECONDARY_IPS=3` を設定します。汎用 generator の既定値は変えません。
旧 2 client 構成では remote 6 個＋primary の容量が必要になり得たため、旧 `t3.large`
はメモリ量だけで過大だったとは断定できません。これはソース既定値であり、
実環境での性能・低メモリ下の信頼性の証明ではありません。有料 run の前に
削減後メモリの PVE guest を認証します。
...
証跡には宣言された RR ごとの成功行が必要で、`pve_rr_nic_coverage` でも
検証済みの識別情報が宣言済みの 2 台と完全一致しなければなりません。
片方の行がない「both PVE RR」の集約 PASS は証拠不足であり、認証済みとは扱いません。
その過去証跡は変更せず、受入不可の訂正を別記録に残し、cloud qualification 前に
両方を確認した新しい証跡を取得します。bridge 一覧の SSH は `bash -s` 用の
スクリプトを標準入力で受け取りますが、RR ごとの `qm config` SSH は
`/dev/null` から読む必要があります。これにより次の RR の一覧行を SSH が
消費することを防ぎます。offline test はその標準入力消費を再現し、RR の未実行・
空応答・取得失敗が集約 PASS にならないことを独立に検証します。
...
この PVE template プロファイルでは、5 ゲスト共通の管理 NIC は `eth0`、
...
QGA の `ping`/`network-get-interfaces` 成功は、管理 IPv4 の準備完了を意味しません。
正常な応答で指定管理 NIC が正確に 1 個存在し、IPv4 の報告が 0 件の場合だけ、
既存の回数・時間上限付き retry 内で再観測します。NIC の欠落・名前違い・重複、
不正な応答、無効または複数の IPv4 は待機扱いにせず拒否します。
retry 回数・間隔・deadline・予算は増やしません。未準備時の観測と失敗証跡を保持し、
IPv4 の報告が空であることだけから DHCP 遅延やサーバー障害とは断定しません。
offline retry test は、実環境での DHCP 回復を証明するものではありません。
DHCP 設定変更や qualification 中の環境修復を追加するものではありません。
...
7. `aws-leaf-a`、`azure-leaf-a`、`oci-leaf-a`、`pve-leaf-a` の順に、
   ...
   gate と、全 12 client flow・全 9 cloud-origin ingress flow を必須にします。
...
| 費用方針の見積／実行許可判定の上限 | US$1.05／US$1.60 |
