...
同一ホスト上で routerd プロセスを 1 つだけにしたい場合は、ローカルパスで十分です。複数ホストの間で 1 つの適用ノードを選びたい場合は、共有ファイルシステム上のパスを使います。ファイルシステムは advisory lock が正しく動作する必要があります。動作しない場合は、このリソースはステータスの記録だけに使い、実際の apply サービスは 1 台のノードだけで有効にしてください。
