...
独立した `NAT44Rule`、`LocalServiceRedirect` は引き続き
...
既定の表示は現在の config から派生するものだけです。
現在の config から来ていない古い status row は、稼働中の resource に見えないよう
非表示にします。古い state database を掃除するときは `--include-stale` で確認できます。
...
古い state row が現在の schema にない resource kind に属している場合は、
`routerctl delete --force <kind>/<name>` で削除します。同じ kind/name が複数の
API group にある場合は、routerd が推測で消さないよう `--api-version <version>` を指定します。
