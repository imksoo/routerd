...
稼働中のルーターでは、`plan`、`observe`、dry-run apply は状態データベースを一時的な snapshot として読みます。デーモンが SQLite WAL に同時書き込みしている場合、その snapshot はわずかに古い可能性があるため、ロールバック記録ではなく事前確認用の参考入力として扱ってください。
...
`routerd serve` は、宣言された設定からコントローラーランタイムを直接実行します。コントローラーの部分的な選択を行う公開フラグはありません。リソースが宣言されていれば、対応するコントローラーが収束方法を決定します。`--dry-run` は適用前の確認手段であり、常駐する動作モードではありません。
