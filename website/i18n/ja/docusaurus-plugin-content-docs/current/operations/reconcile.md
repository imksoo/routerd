...
稼働中のルーターでは、`plan` と `observe` は状態データベースを一時的な snapshot として読みます。デーモンが SQLite WAL に同時書き込みしている場合、その snapshot はわずかに古い可能性があるため、ロールバック記録ではなく事前確認用の参考入力として扱ってください。
...
`routerd serve` は、宣言された設定からコントローラーランタイムを直接実行します。コントローラーの部分的な選択を行う公開フラグはありません。リソースが宣言されていれば、対応するコントローラーが収束方法を決定します。適用前の確認には `routerctl plan` を使います。

`IngressService`、`PortForward`、NAT、BGP、静的経路・ポリシー経路など、転送を伴うリソースがある場合、routerd は必要なランタイム sysctl を導出します。`routerctl apply` は、派生する設定を観測・計画・生成しますが、ホストには反映しません。反映は、`routerd serve` がコントローラーの調整（リコンサイル）の中で収束させます。これにより、一度きりの apply は設定の検証と成果物の生成にとどまり、デーモンとランタイムのカーネルのライフサイクルは、長時間動くコントローラーが所有します。

コントローラーを同期的に呼び出した際の panic は、調整の失敗として記録します。
既存の OpenTelemetry の回数・所要時間・エラーイベントにも反映しますが、panic の値は
エラーやログへ出力しません。runner はロックを解放して後続コントローラーを実行し、
周期的な再試行は設定された最短周期へ戻ります。Bootstrap は引き続き調整の失敗を記録し、
返却エラーは context のキャンセル・期限切れだけです。別 goroutine の panic や
プロセスの強制終了は、この回復処理の対象外です。
...
YAML からリソースが消えた場合、所有元のコントローラーは、自分が所有する成果物だけを
削除または無効化します。対応する所有リソースがなくなった `routerd-healthcheck@*.service` や管理対象のクライアントデーモンユニットは、無効化して削除します。NAT44 ルールが 0 件になった場合は、管理対象の `routerd_nat` テーブルまたは pf anchor を空にします。
...
routerd が削除するのは、所有を確認できる成果物（routerd が以前に作成したもの、または明示的に取り込んだもの）だけです。第三者の設定や手作業の変更には触れません。

世代単位のロールバックに対応しています。`routerctl rollback --list` で過去の apply が記録した世代一覧を表示し、`routerctl rollback --to <generation>` で保存済みの Router YAML を通常の apply 経路で再適用できます。ロールバックは宣言済み config と routerd 管理下の成果物を再適用するもので、conntrack や kernel の一時状態、デーモンのランタイム状態、routerd の台帳外で行ったホスト変更までは**復元しません**。削除を含む変更では、必ず `routerctl plan` で削除リストを確認してから適用してください。
