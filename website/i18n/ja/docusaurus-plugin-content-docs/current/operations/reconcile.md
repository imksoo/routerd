...
世代単位のロールバックに対応しています。`routerctl rollback --list` で過去の apply が記録した世代一覧を表示し、`routerctl rollback --to <generation>` で保存済みの Router YAML を通常の apply 経路で再適用できます。ロールバックは宣言済み config と routerd 管理下の artifact を再適用するもので、conntrack や kernel の一時状態、daemon のランタイム状態、routerd の ledger 外で行ったホスト変更までは**復元しません**。削除を含む変更では、必ず `routerd plan` と `routerd apply --dry-run` で削除リストを確認してから適用してください。
