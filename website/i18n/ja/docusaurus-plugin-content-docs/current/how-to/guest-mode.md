...
同じ LAN 上の端末を MAC アドレスで分類し、通常の zone-to-zone ファイアウォールマトリクスより先に、より厳しい転送方針を適用します。

まだ VLAN を分けない構成でも、信頼済み端末と、インターネットだけを使わせたい端末の境界を明確にできます。

## ユースケース

代表的な用途は次の通りです。

- 家庭内で、来客のスマートフォン、ゲーム機、家電、持ち込み PC を管理網や自宅サーバーへ届かせたくない場合。
- 集合住宅や共有住宅で、既定を guest とし、明示した端末だけを trusted にしたい場合。
- カメラ、HEMS、テレビ、スピーカーなどの IoT 端末を隔離する場合。DNS、DHCP、NTP、インターネットは使わせますが、横方向の通信は不要です。
- 小規模オフィスの visitor network で、物理 LAN は共有しつつ、RFC 1918 や ULA 宛てを遮断したい場合。

完全な例は [examples/guest-mode.yaml](https://github.com/imksoo/routerd/blob/main/examples/guest-mode.yaml) を参照してください。

## 仕組み

Linux では、routerd は `ClientPolicy` を nftables の `inet routerd_filter` テーブルへ生成します。

各方針から次の規則を生成します。

- `client_policy_guest_devices` のような nftables `ether_addr` set。
...
生成規則は `input` chain と `forward` chain の早い位置に入ります。
...
MAC 照合は Ethernet 送信元アドレスを直接見ます。
そのため、DHCP リースは必須ではありません。
ただし `DHCPv4Reservation` を併用すると、端末の IPv4 アドレス、名前、Web Console 表示が安定します。
...
1 つの MAC アドレスだけを guest として扱います。
...
## 例 3: BYOD 向け exclude mode

対象インターフェース上の端末を既定で guest として扱います。
...
既定のプライベート宛て拒否を保ちつつ、1 台のプリンターだけを許可します。
...
ルーター上でローカル探索の proxy や relay を動かす場合は、`mdns` や `ssdp` を明示的に追加します。
...
探索サービスは、公開される情報を理解した上で有効にしてください。
mDNS や SSDP は便利ですが、端末名やサービス情報を見せることがあります。

## 例 6: IoT 隔離と固定割り当て

固定割り当てがあると、調査しやすくなります。
Web Console の端末一覧や DNS レコードも分かりやすくなります。
...
## DHCPv4Reservation 連携

`classification[].ipv4Reservation` は参照確認です。
routerd は、指定した `DHCPv4Reservation` が存在することを検証します。
ファイアウォールの照合は、リースされた IP アドレスではなく MAC アドレスで行います。

この分離は意図的です。

- MAC 照合により、IP 層の判断より前に端末を分類できます。
- IPv4 固定リースにより、DNS と Web Console の表示が安定します。
...
設定を生成するか、稼働中の nftables table を確認します。
...
- public internet は成功します。
- private 宛先は timeout または失敗します。
- DNS、DHCP、NTP は動き続けます。

ルーター側では tcpdump でパケット経路を確認します。
...
private 宛てが拒否されると、生成された `ClientPolicy` 規則の counter が増えます。
...
スマートフォンやノート PC は private address を使うこともあります。
端末に印字された MAC アドレスではなく、ルーター側 LAN で見えているアドレスを使ってください。

### ゲスト端末から private network に届く

次を確認してください。

- 方針が正しい `Interface` を参照している。
- パケットがそのインターフェースから入っている。
- `routerd apply` で最新の nftables table が導入されている。
- `guestEgressAllow` に広い private prefix が入っていない。
- 端末側の VPN クライアントなど、ルーターを迂回する経路がない。

### ゲスト端末からインターネットに出られない

`ClientPolicy` は private 宛てと self 宛てを絞ります。
インターネット疎通が失敗する場合は、経路方針、NAT44、DS-Lite、DNS、IP forwarding を確認します。
...
private subnet への転送許可ではありません。
転送の例外は `guestEgressAllow` で表します。

## セキュリティ考慮

MAC アドレスによる隔離は実用的ですが、暗号学的な識別ではありません。
悪意ある利用者は、端末を操作できれば trusted MAC アドレスを偽装できます。

`ClientPolicy` は家庭や小規模オフィス向けの実用的な制御として使ってください。
...
- 端末ごとの資格情報。
- 専用 guest bridge または VRF。

それらと組み合わせても、`ClientPolicy` は有用です。
端末分類の意図を routerd のリソースモデルに残せるためです。

## OS 対応

Linux nftables に対応しています。

FreeBSD pf は、routerd が `FirewallZone` と `FirewallRule` で使う routed filtering path に、同じ MAC ベース分類モデルを持ちません。
そのため routerd は、FreeBSD で `ClientPolicy` を明示的に未対応として扱います。
弱い no-op 方針として黙って適用しません。

将来の FreeBSD 対応として、bridge 階層のフィルターや専用の layer 2 分離リソースは考えられます。
ただし、routed pf 規則とは等価ではないため、別設計として扱うべきです。
