...
デーモンは UDP/TCP で待ち受けます。
リゾルバに属する `DNSForwarder` が match ルールを表し、`DNSUpstream` が再利用できる上流エンドポイントを表します。
...
`DNSForwarder.spec.upstreams` の順序が優先度です。
まず優先度が最も高く健全なものを試し、失敗したらリストを順に下ります。
...
ホストがそのプロバイダー名をプロバイダー本体経由で解決しようとすると、リゾルバが健全になる前にループするか、失敗します。
...
`DNSUpstream.spec.bootstrap` は、同じ保護をより細かく行います。
暗号化 transport を確立する前に、その上流エンドポイント名を解決するためのリゾルバを指定します。
