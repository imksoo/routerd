...
**CloudEdge Event Federation**（ADR 0006）は、ある routerd ノードが観測したファクトに対して別のノードが宣言的に反応できる仕組みです。
Phase 3 は受信側のループを閉じます。
受信したイベントが `EventSubscription` にマッチし、信頼されたローカルプラグインが実行され、その出力が `routerctl dynamic render` で確認できる `DynamicConfigPart` になります。
...
3. **マッチ**: クラウドノードの `EventSubscription` が、タイプ（およびオプションで subject プレフィックスやソースノード）でイベントにマッチします。
...
- 受信側（クラウド）の配線: [`examples/event-federation/receiver-cloud.yaml`](https://github.com/imksoo/routerd/blob/main/examples/event-federation/receiver-cloud.yaml)。`EventGroup`、`EventSubscription`、`Plugin`、および結果の `RemoteAddressClaim` が解決されるハイブリッドコンテキスト（`OverlayPeer`、`AddressMobilityDomain`、`CloudProviderProfile`）を含みます。
- 送信側（on-prem）の配線: [`examples/event-federation/sender-onprem.yaml`](https://github.com/imksoo/routerd/blob/main/examples/event-federation/sender-onprem.yaml)。`EventGroup` と `EventPeer` プッシュターゲットを含みます。
...
- プロバイダーオペレーション（`actionPlan`）の実行は **Phase 4/5** です。MVP ではアクションプランを実行しません。
- routerd は設定や秘密をプラグインに渡しません。渡すのは観測されたイベントのみです。
- `EventSubscription.match.types` は必須です。サブスクリプションがグループ内のすべてのイベントでプラグインを無差別にトリガーすることはできません。ループを防ぐには `subjectPrefixes` と `sourceNodes` でさらに絞り込んでください。
