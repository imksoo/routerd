...
同時に描画できます。`spec.importPolicy.allowedPrefixes` と
`redistribute.*.allowedPrefixes` は IPv4/IPv6 混在のままでよく、routerd が
prefix-list、route-map、`address-family` block を address family ごとに分けます。
