...
`stickyHoldDays` は任意です。0 より大きい場合、routerd は DHCP リース履歴を短期間保持し、リース解放または期限切れ後に一時的な dnsmasq `dhcp-host` hold を生成します。同じ MAC は hold 期間内に同じアドレスを再取得でき、そのアドレスはすぐ別クライアントには割り当てられません。
...
DHCPv4 の domain-name、RA の DNSSL、DHCPv6 の domain-search が同じローカルゾーンを参照するため、ドメイン文字列を重複して書かずに済みます。
