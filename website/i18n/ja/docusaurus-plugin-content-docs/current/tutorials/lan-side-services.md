...
| DNS リゾルバーの待ち受け | `DNSResolver` | `routerd-dns-resolver` |
...
リゾルバーは、参照先の status から得られるすべてのアドレスで待ち受けます。
...
- dnsmasq のリースファイルを手で書き換えても、routerd には自動通知されません。サポートされる設定ワークフローで対象リースを再調整してください。`routerd-dhcp-event-relay` は再起動するサービスではなく、dnsmasq から都度呼ばれる one-shot フックです。リースの変更は、できる限り routerd 経由で行ってください。
