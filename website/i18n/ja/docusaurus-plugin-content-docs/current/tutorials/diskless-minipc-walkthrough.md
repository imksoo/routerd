...
シリアルコンソールでは、次のようにライブ ISO の案内とウィザードの開始が
表示されます。
...
保存する設定ファイルは `/media/routerd-usb/routerd/router.yaml` です。
...
PVE 検証では、隔離 LAN ブリッジに一時 network namespace を接続しました。
クライアントは routerd からリースを受け取り、routerd の NAT44 経由で
外部へ通信できました。
...
設定が復元されず、`/usr/local/etc/routerd/router.yaml` もなければ、
設定ウィザードを起動します。
