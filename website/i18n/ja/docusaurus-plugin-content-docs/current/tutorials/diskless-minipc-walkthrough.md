...
:::info 画像について
このチュートリアルの画像は、構造を示す placeholder です。
Proxmox VE、シリアルコンソール、LAN クライアントで実機検証したスクリーンショットを
取得したら差し替えます。
:::
...
![PVE VM 作成 placeholder](/img/tutorials/diskless-01-pve-vm-create.svg)
...
![シリアルコンソール placeholder](/img/tutorials/diskless-03-serial-console.svg)
...
![WAN/LAN wizard placeholder](/img/tutorials/diskless-04-wizard-wan-lan.svg)

USB 永続化を聞かれたら `yes` を選び、USB パーティションを指定します。
パーティションに `ROUTERD` ラベルがあれば、自動的に候補へ出ます。

![USB persistence wizard placeholder](/img/tutorials/diskless-05-usb-persistence.svg)
...
USB 永続化は既定で `async,noatime` としてマウントします。
USB への書き込みを減らすためです。
...
警告を出し、新しいログは USB が戻るまで tmpfs に残します。
