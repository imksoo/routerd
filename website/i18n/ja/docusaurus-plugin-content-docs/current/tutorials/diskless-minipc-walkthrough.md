...
FAT32 は `blkid` では通常 `vfat` と表示されます。
...
同じイメージでビデオコンソールとシリアルコンソールの両方を使えます。
...
ISO はビデオコンソールとシリアルコンソールの両方を有効にします。
Proxmox VE では、対話式ウィザードは `qm terminal` の方が読みやすいです。
VGA の画面キャプチャは起動の証跡として使い、実際の入力と結果は下のシリアルコンソールログで確認します。
...
USB 永続化は、USB への書き込みを減らすため、既定で `async,noatime` としてマウントします。
同期書き込みが必要な場合だけ、kernel command line に `routerd.usb_mount=sync` を追加します。

選択した USB パーティションは `/media/routerd-usb` にマウントされます。
保存先の設定ファイルは `/media/routerd-usb/routerd/router.yaml` です（`/mnt/routerd/router.yaml` ではありません）。
...
1. 記録済みデバイス、`routerd.usb=` 指定、`ROUTERD` ラベルの順で USB デバイスを探します。
...
設定が復元されず `/usr/local/etc/routerd/router.yaml` もない場合は、設定ウィザードが起動します。
...
Proxmox VE で試す場合は、クライアントと routerd の LAN NIC が同じ隔離ブリッジに接続されていることを確認してください。
