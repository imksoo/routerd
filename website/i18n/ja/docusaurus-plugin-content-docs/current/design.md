...
バージョン方針: routerd は `20260509.0` から `yyyymmdd.N` 形式の日付ベース版番号を使います。従来の `0.x.y` 形式のプレリリース番号は廃止します。
...
| apiVersion | 役割 |
| --- | --- |
| `net.routerd.net/v1alpha1` | ネットワーク機能 (Link、IPv4Static、DSLite、PPPoE、EgressRoute、HealthCheck など) |
...
| `system.routerd.net/v1alpha1` | OS bootstrap (Package、SysctlProfile、SystemdUnit、NetworkAdoption、WebConsole など) |
...
`addressFrom`、`ipv4From`、`ipv6From`、`upstreamFrom`、`prefixFrom`、`rdnssFrom`、`gatewayFrom` などが共通の参照スタイルです。
...
詳細は [reconcile loop の動作](./operations/reconcile.md) を参照してください。
