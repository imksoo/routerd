...
- HGW は最初の通常稼働中試験の数日前から無停止で稼働していた。その後の段階では、
  再起動直後の取得期間と通常稼働中の挙動を比べるため、意図的に HGW を再起動している。

## 状態モデルの要約

ラボで見えた挙動は、小さな状態機械として読むと理解しやすくなります。
正確なタイマーはファームウェアの挙動であり、公開 API ではありません。この図は
すべての HGW がこう実装されているという主張ではなく、routerd が扱うべき
状態を整理したものです。
...
### A.4 2026-05-01 の重要な訂正

- **observe**: 2026-05-01、同じ HGW に対して NEC IX2215 が
  `no ipv6 dhcp client / ipv6 dhcp client` で INIT-REBOOT を強制したところ、
  Solicit → Advertise → Reply の正常な handshake が動いた。同日 router05
  (Linux dhcpcd) も canonical Solicit/Advertise/Request/Reply で fresh
  `:1220` binding を取得した。
- **observe**: A 節の 34 variant Solicit は、当時の routerd active sender に
  packet 形の bug が複数あった (IPv6 hop limit `1`、Elapsed-Time が IA_PD より
  前、ORO 含む、Solicit IA_PD に T1/T2/lifetimes/prefix を載せる)。`c7f8b24`
  でこれらを IX2215 reference に合わせた後、active Solicit の byte は IX 同等
  になったが、過去に試行を繰り返した routerd VM からの cold acquisition Solicit
  は 2026-05-01 22:25 時点でまだ Advertise を引き出せていない。
- **assert**: 元の A 節「長時間稼働中の HGW で Solicit が信頼できない」という
  記述は **routerd 側の実装バグであって HGW の特性ではなかった**。同 HGW で
  IX2215 や router05 dhcpcd が再起動なしで Solicit を成立させていることが反例。
  A 節の観察記録は歴史的経緯として保存するが、HGW の挙動を記述したものとして
  引用してはいけない。
...
### B.4 2026-05-01 の取り下げ

- **measure (2026-05-01)**: routerd の active sender packet 形を修正した
  (`c7f8b24`: IPv6 hop limit、IA_PD を Elapsed-Time の前、ORO 削除、Solicit
  IA_PD は IAID-only で T1/T2 ゼロ) 後、router01 からの 1 回の active Request
  は HGW 側で受理されて HGW PD table に router01 MAC の `:1240` binding が
  記録された。
- **measure (2026-05-01)**: 同じ router01 から続けて送出した Renew、Rebind、
  Request は IAID や Server-ID を変えても全て silent drop。HGW PD table の
  lease 値も refresh されず、`vtnet0` および pve02 host の `vmbr0` 双方で
  HGW Reply が wire 上に存在しないことを確認した。
- **observe**: 同じ HGW で `router05` (Linux dhcpcd) は Renew で確実に Reply
  を受信している。router05 の binding は OS DHCPv6 client が canonical
  Solicit/Advertise/Request/Reply handshake で確立したもの。
- **assert**: Advertise を経由しない直接 Request は **HGW PD table に
  「phantom」 entry を作るが、後続 Renew/Rebind が機能しない不完全な binding**
  を生む。Advertise/Reply handshake 抜きには HGW 側に per-client server context
  が立たないため、binding を refresh できず lifetime 切れで失効する。
- **assert**: routerd の本 HGW での **本流 acquisition path は OS DHCPv6
  client** (Linux/NixOS は `dhcpcd`、FreeBSD は `dhcp6c`) による canonical
  RFC 8415 Solicit/Advertise/Request/Reply。`routerd dhcpv6 request` 直接送出は
  debug / lab tool に格下げし、本番 binding 確立に使ってはいけない。
- **assert**: active controller の有用性は **maintenance** path、つまり
  canonical handshake で完成した binding に対する Renew/Rebind/Release
  に残る。
...
- **assert**: routerd の controller は必要に応じて Solicit / Request / Renew / Rebind / Release / Confirm /
  Information-Request を発行する。`DHCPv6PrefixDelegation` リソースの `objects.status._variables` には次を保持する。
    - `lease.serverID`
    - `lease.prefix`
    - `lease.iaid`
    - `lease.t1`、`lease.t2`
...
- **assert**: `routerd dhcpv6 renew` および `routerd dhcpv6 release` の CLI
...
- **assert**: routerd は当該 `DHCPv6PrefixDelegation/<name>` の
...
## M. Ubuntu dhcpcd 試験記録（2026-04-30）

この節は、routerd が dhcpcd の管理サービスとフックを生成できるようになった後の、
最初の Linux `dhcpcd` レンダラ試験を記録する。

- **measure**: routerd は Linux 向けにリソース単位の dhcpcd service を生成し、
  UDP/546 を掴んだまま残っていた古い managed `dhcp6c` unit を停止した。
  また、dhcpcd 10 が呼び出せるよう、hook はインターフェイス内ではなく
  ファイル全体の `script` として生成した。
- **measure**: dhcpcd の lease と DUID ファイルを消し、実 WAN MAC 由来の
  DUID-LL を作り直した後、Ubuntu の dhcpcd は DUID-LL、IA_PD、
  DNS/opt_82/opt_83 を含む ORO、Vendor-Class を持つ Solicit を繰り返し送った。
