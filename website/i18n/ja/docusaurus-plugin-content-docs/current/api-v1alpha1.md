...
| `SAMEnrollmentClient` | leaf 側の bootstrap/refresh controller です。local の remote-request `SAMEnrollmentClaim` を RR endpoint に送信し、許可された RRSet と対象なら direct peer group を1つの topology snapshot として取得して、欠落・期限接近・claim 変更時だけ dynamic state に保存します。direct group は、全 bootstrap RR が同じ client identity と topology を確認した場合だけ有効です。`joinTokenFrom` があれば identity は HMAC で認証され、無い場合は trusted control-plane 入力です。RR が揮発した admission state だけを失った場合は、現行 RR の identity-aware 応答に限って自動再登録します。revoke、renewal 中の別 active identity、旧版／混在 RR、または不確実な応答では RR fallback を維持します。local policy や NodeSet は必要ありません。 |
