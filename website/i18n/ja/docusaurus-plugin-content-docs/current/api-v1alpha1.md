...
| `BFD` | BGP peer が参照する FRR BFD session profile と任意の interface binding を宣言します。 |
...
`BGPRouter`、`BGPPeer`、`BFD` は初期 backend として FRR を使います。routerd は
...
route selection state、直近 error を table で表示します。prefix status には、FRR JSON から
取得できる範囲で `best`、`valid`、`installed`、`selectDeferred`、
`selectionState`、`selectionReason` を保存するため、GR/EOR defer や
"no best path" の時間帯を raw `vtysh` output なしで確認できます。
`BGPRouter.spec.convergenceProfile: fast` は、graceful restart の stale-path
保持より初回 boot の速い収束を優先する Kubernetes/edge router 向けです。fast profile は
peer timer を短くし、`spec.gracefulRestart.enabled` が明示されていない場合は graceful
restart を無効化します。`BGPPeer.spec.bfd` は `BFD/<name>` 参照だけを受け付け、
inline BFD 設定は拒否します。`BFD.spec.peer` は IP address または `BGPPeer/<name>` です。
`profile`、`minRx`、`minTx`、`detectMultiplier` で failure detection intent を表します。
managed peer のいずれかが BFD を使う場合、routerd は FRR daemons file の `bgpd=yes` と
`bfdd=yes` も維持します。daemon toggle が変わったときだけ `frr.service` を restart します。
