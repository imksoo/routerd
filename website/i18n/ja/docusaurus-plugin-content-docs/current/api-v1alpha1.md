...
| `BGPRouter` | ローカル BGP router を宣言します。現在の backend は embedded GoBGP で、import policy は default deny です。 |
| `BGPPeer` | `BGPRouter` にぶら下がる GoBGP 管理の BGP peer を宣言します。Kubernetes BGP speaker などに使います。 |
| `BFD` | BFD session intent を宣言します。embedded GoBGP では FRR なしの BFD 実装が入るまで unsupported として報告します。 |
...
`BGPRouter` と `BGPPeer` は `routerd serve` 内の embedded GoBGP server を使います。
routerd は resource spec を型付き GoBGP API object に直接 map し、`ListPeer` と
`ListPath` で status を観測します。FRR text config、`frr-reload.py`、`vtysh` parse、
GoBGP の file config は使いません。`apply --once` は host artifact の render だけを行い、
BGP は `routerd serve` 管理として status に出します。`routerctl show bgp` は保存された
...
router ID は TCP の source address と同一である必要はありませんが、peer 側には実際に
host が使う BGP source address を設定する必要があります。LAN に複数 address がある
場合は Linux なら `ip route get <peer-address>` で source address を確認し、明確な理由が
なければ router ID もその運用上の source address に寄せると混乱を避けられます。

`BGPRouter` は connected/static IPv4 route を個別の `allowedPrefixes` 付きで広告できます。
`BGPRouter.spec.exportPolicy.allowedPrefixes` または redistribute allow-list に明示された
prefix だけが GoBGP local path として追加されます。BGP community policy は router または
peer に `communities.send`、`communities.accept`、`communities.set.in/out` として宣言でき、
GoBGP が報告する observed route community は status に保存されます。watcher は default で
15 秒間隔、prefix status 4096 entries cap です。`BGPRouter.spec.watcher` で
`pollInterval`、`maxPrefixes`、`peerStateChangeThrottle` を調整できます。validation は
3 秒未満の interval と 1,000,000 以上の prefix cap を拒否します。embedded GoBGP MVP は
routerd process ごとに 1 つの `BGPRouter` を support し、`spec.vrf` は未対応です。
multi-router、VRF、BFD resource は黙って無視せず Pending として報告します。
`spec.listen.address` と `spec.listen.port` は in-process GoBGP listener を bind します。
...
`BGPPeer.spec.password` は embedded GoBGP peer の TCP MD5 authentication password として
渡されます。
...
`spec.macs` は短縮形です。`classification[]` は構造化形式で、各 entry は
...
直接 match できない platform で address-based rendering を安定させるためにも使えます。
