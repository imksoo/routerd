...
| `SAMEnrollmentClient` | leaf 側の bootstrap/refresh controller です。local の remote-request `SAMEnrollmentClaim` を RR endpoint に送信し、許可された RRSet と対象なら direct peer group を1つの topology snapshot として取得して、欠落・期限接近・claim 変更時だけ dynamic state に保存します。direct group は、全 bootstrap RR が現在の署名済み claim を確認し同じ topology を返した場合だけ有効です。古い・応答不能な RR があっても RRSet fallback は維持し、未確認の direct peer は保存しません。local policy や NodeSet は必要ありません。 |
