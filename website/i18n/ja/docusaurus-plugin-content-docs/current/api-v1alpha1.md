...
| `WireGuardInterface` | WireGuard インターフェースを表します。暗号化を選ぶ場合、`peersFrom` で `SAMNodeSet`、`SAMEnrollmentPolicy`、取得済み `SAMRRSet`、または enrollment が返した direct `SAMPeerGroup` から peer 定義を取り込めます。 |
...
ファイルが存在しない場合、非 dry-run の apply は mode `0600` で生成し、
既存の非空鍵は上書きしません。
...
| `DNSUpstream` | `udp`、`tcp`、`dot`、`doh` のいずれかで、1 つの上流エンドポイントを表します。状態由来の address、bootstrap resolver、TLS 名、送信元 interface も指定できます。 |
...
`sourceInterface` は Linux で送信先 interface を束縛し、`bootstrap` は DoH/DoT のエンドポイント名の解決に使う補助 resolver です。
...
| `SAMEnrollmentPolicy` | hub/RR 専用で SAM transport enrollment claim を認可します。transport profile、出力 `rrSetRef`、必須の静的 RR topology `rrNodeSetRef`、join token/audience、tunnel/endpoint prefix、leaf ID policy、TTL、明示的な claim revoke、任意の WireGuard materialization、MobilityPool または直接 `mobilityPrefixes` の admission authority を結びます。`directMesh.peerGroupRef` で任意の leaf 間 direct group も認可できますが、RR は fallback のままです。leaf は copy を宣言しません。 |
| `SAMEnrollmentClaim` | leaf に置く remote enrollment request です。leaf identity、join nonce/timestamp/HMAC、remote policy/RRSet reference、tunnel address、endpoint、署名対象の任意 `directMesh` opt-in、任意の BGP identity、MobilityPool 所有 `/32`、任意の WireGuard credential、expiry、revocation state を運びます。 |
| `SAMEnrollmentClient` | leaf 側の bootstrap/refresh controller です。local の remote-request `SAMEnrollmentClaim` を RR endpoint に送信し、許可された RRSet と対象なら direct peer group を1つの topology snapshot として取得して、欠落・期限接近・claim 変更時だけ dynamic state に保存します。direct group は、全 bootstrap RR が同じ client identity と topology を確認した場合だけ有効です。全RRが同じclaimを受理済みでもpeer一覧だけが一時的に違う場合は、`directTopologyPending: true` としてRR経由を維持し、部分的なdirect経路は使わずGETだけで再確認します。`joinTokenFrom` があれば identity は HMAC で認証され、無い場合は trusted control-plane 入力です。RR が揮発した admission state だけを失った場合は、現行 RR の identity-aware 応答に限って自動再登録します。revoke、renewal 中の別 active identity、壊れた応答、旧版／混在 RR、または不確実な応答では RR fallback を維持します。local policy や NodeSet は必要ありません。 |
| `SAMTransportProfile` | この router の安定した `selfNodeRef`、inner tunnel prefix、underlay interface、BGP router、peer source を宣言します。`peersFrom` で `SAMNodeSet`、runtime-synchronized `SAMPeerGroup`、`SAMEnrollmentPolicy`、または取得済み `SAMRRSet` から topology と peer endpoint を取り込み、`nodeRefs` で adjacency を選択できます。enrollment が返した `SAMPeerGroup` を `direct: true` にすると、`pair-stable` addressing で任意の leaf 間 direct path を試行し、BGP local preference を高くします。direct source は必須の `SAMRRSet` fallback の後に置く必要があります。direct profile では通常の RR `bgp.importPolicy.localPreference` を明示し、`bgp.directLocalPreference` をそれより大きくする必要があります。`bgp.timers`、`bgp.timersPreset`、`bgp.convergenceProfile` を省略すると、生成された peer は参照先 `BGPRouter` の default を継承し、profile 側の指定があればそれを優先します。`SAMNodeSet` は identity/topology/endpoint の唯一の共有 source です。`SAMPeerGroup` は top-level resource として author できません。routerd は peer ごとの `TunnelInterface`、endpoint `/32` `IPv4Route`、`BGPPeer` を `DynamicConfigPart` として導出します。 |
...
| `BGPRouter` | ローカルの BGP router を宣言します。現在の backend は長寿命の `routerd-bgp` GoBGP デーモンで、import policy は default deny です。 |
...
trusted overlay path で non-TCP IPv4 の PMTU blackhole を避ける必要がある場合だけ、
`OverlayPeer.spec.pathMTU.forceFragmentIPv4` または
`TunnelInterface.spec.pathMTU.forceFragmentIPv4` により default-off の Linux
nftables `routerd_forcefrag` table を有効化できます。これは派生した forwarded path
上の過大 IPv4 packet だけの DF を消します。対応する overlay underlay は
`wireguard`、`ipip`、`gre`、`fou`、`gue` だけです。
...
降格します。validator は、`IngressService`、`LocalServiceRedirect`、routerd 管理デーモンの
...
DNS query の観測や provider エンドポイント feed を扱う、別のリソースが必要です。

`BGPRouter` と `BGPPeer` は、長寿命の `routerd-bgp` デーモンを使います。
...
BGP は `routerd serve` の管理として status に出します。`routerctl get BGPRouter` は、保存された
...
DNSZone の coverage warning と自動公開を行いません。`routerctl get VirtualAddress` は、role、
...
`routerctl get IngressService` は、active backend と backend ごとの health を表示します。
あわせて `hairpinMode`、hairpin が必要か、期待される nftables SNAT rule が
present/missing のどちらか、も出します。live dataplane のデバッグでは、host の
`ip`、`nft`、conntrack state を直接確認してください。Ingress、NAT 系、DS-Lite、IPv6 PD/RA、routing resource から、
...
保守中は `routerctl ingress drain
ingress/<service> backend=<name> --duration 10m` で、backend を runtime state 上の
drain 状態にできます。controller は、duration が切れるか `routerctl ingress undrain
...
`HealthCheck.spec.enabled: false` にすると、デーモンユニットは生成しますが、停止・無効化します。
...
`Telemetry` は、routerd 自身と管理対象デーモンの metrics / traces / logs を
OpenTelemetry のエンドポイントへ出すための resource です。`LogSink` は、運用イベント
や観測ログの転送経路を表します。OTLP へログ転送する場合は、collector のエンドポイントを
