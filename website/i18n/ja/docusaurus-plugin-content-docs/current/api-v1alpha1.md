...
TTL として設定します。
`BGPPeer.spec.passiveMode: true` は、その peer に対する GoBGP の能動的な TCP open
を無効化します。routerd-bgp へ相手側から接続する harness や peer、とくに能動的な
self-connection が inbound session と衝突し得る loopback speaker で使います。
