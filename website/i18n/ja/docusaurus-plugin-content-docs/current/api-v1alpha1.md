...
| `IngressService` | WAN 側の IPv4 TCP/UDP サービスを公開します。複数 backend、TCP health check intent、failover selection を受け付け、runtime failover は controller path で扱います。 |
...
`BGPStateWatcher` は 15 秒間隔で FRR JSON status を読み、peer、prefix、event、OTel
aggregate metric に反映します。prefix status は 4096 entries で上限をかけます。
...
旧 backend に残り、新規 flow が選択済み backend に向かいます。
