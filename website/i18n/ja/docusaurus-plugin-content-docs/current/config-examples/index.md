...
| [BGP 付き Kubernetes API VIP](./kubernetes-api-vip.md) | `routerd-bgp` GoBGP と keepalived で現在の実装で利用可能 | Kubernetes API VIP を routerd が保持し、control plane を health check し、Service prefix を BGP で受けたい。 |
...
| OSPF など BGP 以外の動的 routing | 未実装です。Kubernetes 風の Service prefix import には `routerd-bgp` GoBGP を利用できます。 |
