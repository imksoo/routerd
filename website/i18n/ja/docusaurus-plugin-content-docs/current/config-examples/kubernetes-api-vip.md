...
| `IngressService/kubernetes-api` | `/readyz` の HTTPS health check、kubeadm bootstrap の self-signed 証明書向け `tlsSkipVerify: true`、failover selection、healthy backend 不在時の reject。 |
...
raw status JSON ではなく表形式で確認できます。
