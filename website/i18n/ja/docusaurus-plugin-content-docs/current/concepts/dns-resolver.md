...
`DNSZone` はローカルの権威データを持ちます。
手で書いたレコードと、DHCP リースから派生したレコードを保存します。

`DNSResolver` はデーモンインスタンスを管理します。
待ち受けアドレス、キャッシュ、メトリクス、query log を定義します。
1 つの `DNSResolver` リソースが、1 つの `routerd-dns-resolver` プロセスを起動します。

`DNSForwarder` はリゾルバに属する 1 つの match ルールです。
`DNSZone` から応答するか、一致した問い合わせを `DNSUpstream` へ転送します。

`DNSUpstream` は 1 つの上流エンドポイントです。
平文の UDP/TCP DNS、DoT、DoH を表します。
...
なくなります（本番ルーターでの実測では、AFTR forwarder が `Degraded` を示している間も
...
`listen[].sources` の名前は `DNSForwarder` を参照します。
省略すると、そのリゾルバに属するすべてのフォワーダーを使います。
...
ユーザーが書く YAML では `DNSResolver.spec.sources` を受け付けません。
以前のインライン source は `DNSForwarder` と `DNSUpstream` に分けます。
