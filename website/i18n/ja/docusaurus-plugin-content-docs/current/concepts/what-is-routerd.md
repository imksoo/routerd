...
routerd は、Linux ホストと FreeBSD をルーターとして動かすための宣言型の制御プレーンです。ルーターの構成を YAML リソースとして書くと、routerd がその意図を、インターフェース、アドレス、DHCP、DNS、NAT、経路、トンネル、ヘルスチェック、パッケージ、sysctl、サービスユニット、ログといった実際の状態へ反映します。
...
- インターフェースのアドレスは、netplan、systemd-networkd、rc.d に分かれます。
...
FreeBSD も同じリソースモデルを使い、反映先だけが OS に合った機構になります。プラットフォームごとの差は対応表に記載します。
