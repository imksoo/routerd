...
たとえば `IPv4DHCPAddress` は「ens18 を使う」とは書きません。次のように書きます。
...
たとえば `IPv6PrefixDelegation` には次のようなフィールドがあります。
...
代表例は `IPv6PrefixDelegation.spec.profile` で、`ntt-hgw-lan-pd` のような上流環境を
