...
`HealthCheck` が `IPv4DefaultRoutePolicy` の candidate や
`IPv4PolicyRouteSet` の target から参照されている場合、routerd はその
route target の mark から socket mark を自動導出します。`fwmark` を直接
指定するのは、route target に紐づかない低レベルな probe に限ります。
