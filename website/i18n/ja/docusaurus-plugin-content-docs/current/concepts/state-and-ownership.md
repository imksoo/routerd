...
リリースインストールでは、正本の設定を `/usr/local/etc/routerd/router.yaml`
に置きます。
...
複数の tunnel から集約される TCP MSS clamp table は、トップレベルの `Router` が所有します。
...
新しい適用世代には、そのとき適用した YAML スナップショットを保存します。
Web Console は、そのスナップショットを使って読み取り専用の世代履歴と世代間の unified diff を表示します。
