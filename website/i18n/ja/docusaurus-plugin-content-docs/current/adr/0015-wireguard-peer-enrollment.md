...
同じ境界は RR 側の BGP admission にも伸びています。generated route-reflector client peer は
SAM topology と configured mobility prefix allowlist から import filter を導出します。各
leaf は許可された prefix 配下の `/32` だけを import でき、自分自身の node-identity community
を付ける必要があり、他 topology node の identity を付けると拒否されます。holder-beacon
community は leaf 自身の identity と許可済み host route の組み合わせでのみ有効で、単独の
権威としては扱いません。
