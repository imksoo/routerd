...
`action_executions` ジャーナルのみ**を導入する。
Phase 5.0 には**実行ステートマシン、
`routerctl action` コマンド、エグゼキューターの呼び出し、実際のプロバイダー CLI/SDK 呼び出しは
含まれない**。
フェイクエグゼキューターと実行パスは後続チャンクで到着する。

## 背景

- **Phase 4.1 で dry-run `actionPlans` を導入済み。**
  プランナープラグイン（capability `propose.providerAction`）は表示専用のプロバイダー操作を `DynamicConfigPart` に記録する。
  routerd は `actionPlan` を**決して実行せず**、そこからプロバイダー CLI/SDK を呼び出すこともない。
  `pkg/plugin.ValidateActionPlan` が `mode=execute` を拒否する。
  これらは EventSubscription 駆動の実行をレビュー可能に保つためだけに存在する。
- **SAM データプレーンは実クラウドで検証済み。**
  Selective Address Mobility は AWS、Azure、OCI でクリーンスモークを通過済み（3 クラウドパリティ）。
  オンプレミス側は claim されたアドレスをオーバーレイ経由で配送する。
  クラウド側は依然として、プロバイダーが NIC に secondary IP を実際に attach/detach する必要がある。
  現在、その attach/detach はオペレーターの手動操作である。
- **不足しているのはゲート付き実行。**
  routerd が承認済みのプロバイダー mutation を駆動できるようにしたいが、プロバイダー認証情報は routerd コアに入ってはならない。
...
allowlist 済み/リダクト済みコンテキストのみを渡す。
エグゼキューター自身がクラウドに対して認証する。
認証情報は routerd コアや `action_executions` ジャーナルを通過しない。
...
`RemoteAddressClaim` と `CloudProviderProfile` と同じ `hybrid` グループに定義する。
ゼロ値は安全なロックダウン状態：
...
`journal`、`rollback --dry-run`。
これらはオペレーターサーフェス。
Phase 5.0 では**いずれも出荷しない**。

### フェーズ分割

- **Phase 5.0（フレームワークとデータモデル）**: `ProviderActionPolicy` Kind、
  `action_executions` ジャーナル、スキーマ/バリデーション。
  **フェイクエグゼキューター**（実クラウドなし）が Phase 5.0 の後半チャンクでパスをエンドツーエンドで検証する。
...
上記の設計から導かれるセーフティストップの一覧：

1. `ProviderActionPolicy.enabled` のデフォルトは false、`dryRunOnly` のデフォルトは true。
   ポリシーを明示的に設定しない限り実行されない。
2. アクションはオペレーター承認、またはポリシーの `requireApproval=false`（`enabled` かつ非 `dryRunOnly` かつ allowlist 一致）の場合のみ実行される。
3. 承認済み `action_execution` がなければ `mode=execute` は拒否される。
4. `idempotencyKey` 必須。
   既に succeeded のキーは再実行されない（skipped / duplicate）。
   インポートは `ON CONFLICT DO NOTHING` で、繰り返しキーは 2 つ目の実行行を作らない。
5. すべての実行結果がジャーナルされる（`pending` / `approved` / `succeeded` / `failed` / `skipped` / `rolledBack`）。
6. Undo/ロールバックはベストエフォート。
   エグゼキューターがサポートしない場合がある。
   `allowUndo` でゲートされる。
7. プロバイダー認証情報は routerd コアを通過しない。
   エグゼキューターが自身のクラウドネイティブ識別情報を使用する。
8. Phase 5.0 はフェイクエグゼキューターのみで、実際のプロバイダー CLI/SDK を呼び出さない。

## 結論

- routerd は、クラウド認証情報を保持することなく、クラウド側 SAM の attach/detach を
  駆動するための、レビュー可能でデフォルトオフのパスを獲得する。
- ジャーナルが監査証跡であり冪等性のガードである。
  何が実行されたかの単一の正本となる。
- provision と de-provision の非対称性（ADR 0006 に従う TTL teardown のヒステリシス）は、
  すべてのイベントにリアクティブにではなく、実行をゲート付きかつジャーナル付きに保つことで遵守される。
