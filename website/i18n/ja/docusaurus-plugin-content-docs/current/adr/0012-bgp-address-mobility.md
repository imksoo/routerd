...
**復帰したノード** が稼働中のホルダーを奪い返し、データプレーンを揺らしてしまいます。
逆方向の失敗も存在しました。cold-start では両メンバーが互いに譲り合い、グループに
ホルダーが居なくなることがありました。この追補は、突発的な failover の速さを保ちつつ
...
**ホルダービーコン community。** active な capture ホルダー（active なホルダーのみ）が
...
と `64512:121` の両方を持つときだけ、そのノードをグループホルダーと判定します。これ
はプロバイダープラグイン非依存（BGP は常に存在）かつ standby の低 preference な
make-before-break 広告非依存（best-path を勝てない）です。next-hop 照合やプロバイダー
self-scan でホルダーシップを推定する旧来の試みを置き換えるもので、両者とも信頼できない
ことが判明していました。

**同優先度の no-preempt。** 同優先度のタイブレークでは、nodeRef タイブレークでなく
観測された incumbent ホルダーを優先するため、復帰した peer は稼働中のホルダーを preempt
しません。厳密に高優先のメンバーは依然として奪い返します。

**startup fence。** ノードはプロセス起動時に settle window を確定します
（`placementSettleStart`、VM の停止/起動や reboot ごとにリセット）が、現在の fence は
readiness-first です。active を主張しようとし、incumbent をまだ観測していないノードは、
BGP の初回観測が完了し、provider inventory に基づく capture では provider self-observation
も完了するまで defer します。readiness signal が使えない caller では wall-clock window が
保守的 fallback として残ります。これにより crash-loop するノードは観測が実際に揃えば
すぐ fence を抜けられ、分断中または blind なノードは 120 秒が経っただけでは active を
主張しません。

**ホルダー retention と優先度復帰。** ノードは自分のグループの capture を物理的に保持
している間 active を維持します（自分のホルダーシップを失ったときだけ譲り、一時的な peer
観測では譲りません）。唯一の例外は、ホルダービーコンが厳密に高優先の peer をホルダーと
示したときで、その場合は低優先の暫定ホルダーが譲り、設定された優先度復帰が進みます。
