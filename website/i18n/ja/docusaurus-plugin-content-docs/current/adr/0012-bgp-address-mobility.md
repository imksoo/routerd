...
![ADR 0012 の図。リースと epoch 所有権を BGP best-path /32 アドバタイズメントに置き換え、活性マーカー、Route Reflector パス、FIB インポート、バックグラウンドプロバイダー capture まで](/img/diagrams/adr-0012-bgp-address-mobility.png)
...
doctor の安全機構はバックグラウンド reconciliation およびローカル capture ガードとして
スコープ内に残る。
...
- planner がリースを `AddressMobilityDomain`、`RemoteAddressClaim`、
...
- **所有権:** モバイル `/32` のオーナーはそのプレフィックスの現在の BGP best path。
  別の `AddressLease`、ownership epoch、capture epoch レジストリはない。
...
- **Capture/trap:** クラウドプロバイダー secondary-IP アクションは BGP best-path ビューと
...
- **フェンシング:** プロバイダーアクションは現在のモビリティパスシグネチャ
  ...
  旧来の ownership/capture epoch テーブルは削除済み。
...
- **削除された状態:** B6 でモビリティリース、ownership epoch、capture epoch、
...
- Phase 1 でプロバイダー executor を削除しない。
...
| capture プレースメントのアクティブメンバー | パス優先度 / origin 適格性 |
...
6. パリティ証明後、旧リース/planner/epoch パスを削除。
...
- 最も複雑なカスタム状態（`AddressLease` 射影、capture プレースメント、
  capture/ownership epoch プランニング、deprovision マーカー）を
  マイグレーション後に大幅に削減できる。
...
- MobilityPool のデフォルト配送を BGP にする。旧 MobilityPool route-mode planner は
...
- テストとライブエビデンスが BGP パスをカバーした後、旧モビリティリース/planner パスが削除済み。
