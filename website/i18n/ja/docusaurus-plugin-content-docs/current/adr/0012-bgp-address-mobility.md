...
placement の no-preempt + ホルダービーコン追補を実装し、実機クラウドで検証
...
導入したカスタムオーバーレイ到達性の正本を置き換える。
旧来のプロバイダーアクション、VRRP、doctor の安全機構はバックグラウンド reconciliation
およびローカル捕捉ガードとしてスコープ内に残る。
...
依存させることにもなった。
ライブ 4-site テストでは、オーバーレイ/クラウドフェイルオーバーは
reconcile tick、リース/epoch 射影、アクションインポート/自動実行、プロバイダー API 動作、
クラウドファブリック伝搬に制約されたままだった。
最近のスモーク結果では AWS/OCI でクラウドフェイルオーバーに約 120 秒かかったが、
目標は 60 秒以下、できればオーバーレイトラフィックでは秒単位。
...
best path の観測/Linux IPv4 FIB へのインポートが可能。
GoBGP v3.37.0 は EVPN Type-2/Type-5 と MAC モビリティシーケンス番号もサポートするが、
routerd の現在の BGP リソースモデルと FIB syncer は IPv4/IPv6 unicast のみを公開している。
最速の有用なカットはプレーンな IPv4 unicast `/32` モビリティであり、EVPN ではない。

クラウドプロバイダーファブリックは別の制約を持つ。
AWS VPC ルートテーブル、Azure UDR/Route Server、OCI VCN ルートテーブルは、
明示的なクラウドルーティング統合が設定されない限り、
...
必要な場合がある。
BGP はプロバイダー API 呼び出しをオーバーレイ到達性のクリティカルパスから除去できるが、
プロバイダー ingress の問題を削除するわけではない。
...
- **活性:** モビリティフェイルオーバーは BGP withdrawal と best-path 収束に依存する。
  高速障害検出は FRR `bfdd` にレンダリングされる `BFD` リソースで提供する。
...
- **オンプレミス LAN 権限:** VRRP マスターゲーティング、proxy-ARP、GARP、
  非マスター fail-closed 動作、重複 proxy-ARP doctor チェックはローカル安全機構として維持する。
- **削除された状態:** B6 でモビリティリース、ownership epoch、捕捉 epoch、
  deprovision マーカーのテーブルと API を物理的に削除した。そのステージで約 6,200 行を
  純減。
...
- スプリットブレイン防止は依然として VRRP/プロバイダーフェンシング/doctor チェックに依存する。
...
- 4-site デモが BGP 学習済み `/32` オーバーレイルートを使って directed SSH マトリクスを通過する。
- 協調ドレインと stale オーナーフェイルオーバーが、オーバーレイパスでの手動プロバイダーアクション
  承認/実行なしに BGP を通じて収束する。
...
## 追補: placement の no-preempt、startup fence、ホルダービーコン（2026-06-15）

Phase 1 は BGP `/32` を真実の源として確立したが、隙が残っていた。
同優先度の placement メンバーが 2 台あると、決定的な best-path/nodeRef タイブレークにより
**復帰したノード**が稼働中のホルダーを奪い返し、データプレーンを揺らしてしまう。
逆方向の問題も存在した。
cold-start では両メンバーが互いに譲り合い、グループにホルダーが居なくなることがあった。
この追補は、突発的な failover の速さを保ちつつ両方の隙を塞ぐ機構を記録する。
実装は `pkg/controller/mobility/` にあり、実機の AWS/Azure/OCI で検証済み。
運用上の詳細は [CloudEdge SAM 内部実装](../reference/cloudedge-sam-internals) にあり、
本節は決定を記録する。

**ホルダービーコン community。**
active な capture ホルダー（active なホルダーのみ）が自分の owner `/32` を
community `64512:121`（`bgpMobilityCommunityActiveHolder`）付きで広告する。
peer は、その owner `/32` の best-path が node-identity community と `64512:121` の
両方を持つときだけ、そのノードをグループホルダーと判定する。
これはプロバイダープラグイン非依存（BGP は常に存在）かつ standby の低 preference な
make-before-break 広告非依存（best-path を勝てない）。
next-hop 照合やプロバイダー self-scan でホルダーシップを推定する旧来の手法を置き換えるもので、
両者とも信頼できないことが判明していた。

**同優先度の no-preempt。**
同優先度のタイブレークでは、nodeRef タイブレークでなく観測された incumbent ホルダーを優先するため、
復帰した peer は稼働中のホルダーを preempt しない。
厳密に高優先のメンバーは依然として奪い返す。

**startup fence。**
ノードはプロセス起動時に settle window を確定する
（`placementSettleStart`、VM の停止/起動や reboot ごとにリセット）。
window 内では、active を主張しようとするが incumbent をまだ観測していないノードは defer するため、
復帰ノードは fresh な BGP RIB / プロバイダー観測が収束する前に奪い返せない。
長く動いている standby は window を過ぎているため、active が死ねば即座に seize し、
本物の failover は遅延しない。

**ホルダー retention と優先度復帰。**
ノードは自分のグループの capture を物理的に保持している間 active を維持する
（自分のホルダーシップを失ったときだけ譲り、一時的な peer 観測では譲らない）。
唯一の例外は、ホルダービーコンが厳密に高優先の peer をホルダーと示したときで、
その場合は低優先の暫定ホルダーが譲り、設定された優先度復帰が進む。
`/32` は 1 つずつ移譲され、データプレーンの瞬断はない。

community 体系、placement 評価、上記 3 機構の完全な仕様は
[CloudEdge SAM 内部実装](../reference/cloudedge-sam-internals) にある。
