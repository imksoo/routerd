# ADR 0008: フェンシングトークンによる Capture 協調（epoch フェンス付きレベル射影）

![ADR 0008 の図。capture 協調のハザード、captureEpoch フェンシング、スタンプ付きプロバイダーアクション、stale アクションの拒否、冪等なレベル射影](/img/diagrams/adr-0008-capture-coordination-fencing.png)
...
Selective Address Mobility のモバイル `/32` は、**任意の時点で正確に 1 つの capture ホルダーを持つ、一意性制約付き共有リソース**である（any-origin 対称アービトレーションの単一所有者不変条件）。アドレスを「保持する」とは、物理的な capture を所有すること：クラウド NIC 上のプロバイダー **secondary IP** 割り当て（AWS ENI / Azure NIC / OCI VNIC）、またはオンプレミスの **proxy-ARP + GARP**。

capture はホルダー間で 2 つの方法で移動する：

- **協調的 / 計画的** — メンテナンスドレイン。アクティブホルダーが協力する。
- **突発的 / 障害** — ホルダーのホストが停止またはパーティションされる。*協力できない*ため、
  スタンバイが capture を seize（奪取）する必要がある。

de-provision（secondary IP の unassign / フォワーディングの無効化）は capture の**解放**、
assign は**取得**である。このバグはフラキーテストとして顕在化した
（`TestServeChainMobilityCancelsPendingDeprovisionWhenDesiredAgain`、`-race` なしで約 3/30 失敗）：
re-capture 時に進行中の de-provision がキャンセルされないことがあり、
...
- **フェンシングトークン**（Kleppmann, *How to do distributed locking*）：TTL 付きリース/ロックは
  ...
  ***安全性*には不十分** — 一時停止/遅延/復活した（「ゾンビ」）旧ホルダーが、リース期限切れ後も
  操作できてしまう。「書き込み直前に期限を確認しても修正できない。」唯一の修正策は、
...
- **スプリットブレイン / HA フェイルオーバー**（Pacemaker STONITH、keepalived VRRP + EC2
  ...
  ハートビート間隔は検出レイテンシとスプリットブレインリスクのトレードオフ — ただし
...
アクションインポート / executor 境界（「フェンシングプロキシ」パターン）。
...
`(pool, address, captureDomain)` をキーとし、**desired capture ホルダー**が変わるたびにインクリメントされる
— 以前のホルダーへの re-capture を含む。`AddressLease` の epoch とは**別のもの**：

- `AddressLease` epoch = **ロケーションオーナー**（アドレスを所有する者）の epoch。
- `captureEpoch` = **物理 capture ホルダー**（secondary IP を attach する /
...
- **オンプレミスの proxy-ARP はより弱い**。クラウドと同等に見せかけてはならない：
  ...
  **capture 権限としての VRRP/keepalived マスター状態** — 非アクティブノードは **fail-closed**
...
- フラキーな de-provision/re-capture レースは、上書きではなく抽象化レベルで除去された：
...
- routerd は、同じゲートが後に突発的フェイルオーバーの seize にも使える原則的な
  ...
  1 つの機構を共有する。
- 設計は、**クラウド capture は強く安全**（プロバイダーの単一割り当て + reassignment +
  NIC スコープ + epoch）であり、**オンプレミスの proxy-ARP はベストエフォート**
  （VRRP マスター権限 + fail-closed + GARP）であることを、等価と暗示するのではなく
  明示している。
