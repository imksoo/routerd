...
- non-secret な node-local 値は `spec.values` に置き、`capture.targetFrom` と
...
On-prem node では逆に、on-prem member が完全な self declaration になります。
...
`spec.values`
: この node の config normalize 時に使う non-secret な local 値です。region、
...
: generated provider action の target key を `spec.values` の key に対応させます。
...
`EventPeer` endpoint・BGP/BFD peering address に使ってください。mobility pool
