...
`SAMNodeSet` は同じ fabric の次の write-once 集約点です。現在
`EventPeer`、`WireGuardPeer`、`SAMTransportProfile` の peers/topology、
`MobilityPool` members に重複している node identity fields を集約します。この
release では Event Federation と WireGuard が peer target をここから導出でき、
残りの feature ごとの list も同じ source へ移していきます。node set から
`SAMTransportProfile` topology を導出する設計は、node 追加時に既存 tunnel `/31`
assignment が renumber されないよう `addressingMode: pair-stable` を前提にします。
...
WireGuard interface も同じ node registry から peer を取り込めます。
...
WireGuard peer entry を生成します。`nodeRef` が `selfNodeRef` と一致するノードは
除外されます。`selfNodeRef` を省略した場合は `Router.metadata.name` を使います。
取り込んだ peer を先に追加し、その後に静的 `WireGuardPeer` resource を
`metadata.name` 単位で重ねるため、remote `nodeRef` と同じ名前の手書き peer は
bootstrap または緊急 override として残せます。必須 source が見つからない場合、
interface は `Pending` になり、routerd は現在の WireGuard runtime config を
書き換えません。
...
非破壊のままです。interface status には、WireGuard runtime から観測できるか、
設定済み鍵素材から導出できる場合に `publicKey` を出します。

hub/spoke 構成では、信頼済み node registry が存在した後の peer 記述重複は
`peersFrom` で減らせますが、初回 registration そのものは解決しません。
[ADR 0015](../adr/0015-wireguard-peer-enrollment.md) は、leaf router が fixed
RR/spine endpoint に対して non-WireGuard bootstrap path で WireGuard identity を送信し、
RR/spine が承認してから生成 peer input にする、提案中の WireGuard peer enrollment flow
を説明しています。
...
動的な on-prem クライアントの検知は、オンプレミスメンバーに `ownershipDiscovery`
...
クラウドの `provider-secondary-ip` 捕捉では `members[].capture.strategy` を指定できます。デフォルトは `secondary-ip` で、従来どおり AWS ENI、Azure NIC ipConfig、OCI VNIC のセカンダリアドレスを使います。AWS と Azure では代わりに `strategy: route-table` を指定できます。AWS は `capture.target.routeTableRef` の `/32` ルートを `capture.nicRef` に向け、Azure は `capture.target.routeTableRef` の UDR を作成して `capture.target.nextHopIPAddress` を next-hop にします。routerd は、プロバイダー inventory がその route table がローカル router を指していることを確認してから、捕捉済み `/32` を BGP 広告します。
