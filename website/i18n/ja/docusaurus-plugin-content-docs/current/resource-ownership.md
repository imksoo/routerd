...
安定した owner identity は `apiVersion/kind/name` です。apply generation は owner
key に含めません。同じ resource は世代が変わっても同じ所有者として、生成済み
artifact の置換や削除を行えます。object status も owner metadata と lifecycle
class を持ち、stale status cleanup が apply path と同じ判断を行えるようにします。
...
| `MobilityPool` | 動的な SAM capture/control-plane resource、BGP `/32` advertisement、provider action plan、ownership observation |
| `RemoteAddressClaim` | 低レベル SAM capture state、proxy-ARP sysctl/neighbor state、provider-secondary capture status、resource-specific teardown |
