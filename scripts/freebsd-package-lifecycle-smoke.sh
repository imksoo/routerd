...
cleanup() {
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    for log in prior-install prior-service-start current-upgrade owned-uninstall foreign-install foreign-uninstall cleanup; do
  ...
  if [ "$rc_config_created" -eq 1 ]; then
    rm -f "$rc_config"
  fi
  rm -rf "$work"
  exit "$rc"
...
if [ -e "$rc_config" ] || [ -L "$rc_config" ]; then
  echo "refusing package lifecycle smoke with pre-existing routerd rc.conf.d override: $rc_config" >&2
  exit 1
fi
...
wait_routerd() {
  ...
  if [ -s "$status" ]; then
    echo "routerctl get status JSON:" >&2
    cat "$status" >&2 || true
  fi
  return 1
}
...
(
  cd "$prior_dir"
  ROUTERD_INSTALL_PACKAGE_MANAGER=none ./install.sh --prefix /usr/local --no-install-deps --no-config-update --enable-service
) >"$work/prior-install.log" 2>&1
owned_live=1
mkdir -p "$(dirname "$rc_config")"
