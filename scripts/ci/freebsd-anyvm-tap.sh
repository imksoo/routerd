...
ROUTERD_ANYVM_REBOOT_MARKER="$reboot_marker" python3 "$work/anyvm.py" \
  ...
  -- "cd /home/runner/work/routerd/routerd && pkg install -y go dnsmasq git hs-ShellCheck curl jq ndpi pkgconf strongswan mpd5 wireguard-tools tailscale && if [ '$lifecycle_runtime' = true ]; then pkg install -y kea sqlite3; fi && ROUTERD_FREEBSD_EXPECTED_ARCH=$arch ROUTERD_IPSEC_TOPOLOGY=tap ROUTERD_IPSEC_UNDERLAY_IF=vtnet1 ROUTERD_IPSEC_PEER_ADDR=$peer_addr ROUTERD_IPSEC_GUEST_ADDR=$guest_addr ROUTERD_IPV6_ROUTE_TO_CONSOLE_CANDIDATE=$ipv6_candidate ROUTERD_FREEBSD_KERNELMODULE_PERSISTENCE_RUNTIME=$kernelmodule_persistence ROUTERD_FREEBSD_CLIENTPOLICY_IDENTITY_RUNTIME=$clientpolicy_identity ROUTERD_FREEBSD_LIFECYCLE_RUNTIME=$lifecycle_runtime ROUTERD_FREEBSD_PPPOE_RUNTIME=$pppoe_runtime ROUTERD_FREEBSD_WIREGUARD_VXLAN_RUNTIME=$wireguard_vxlan_runtime ROUTERD_FREEBSD_TAILSCALE_BOUNDARY_RUNTIME=$tailscale_boundary_runtime ROUTERD_FREEBSD_CARP_RUNTIME=$carp_runtime sh scripts/freebsd-native-vm-smoke.sh"
