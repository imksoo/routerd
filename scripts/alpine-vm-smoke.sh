...
for cmd in routerd apk rc-update rc-service; do
...
routerd apply --config "$config" --once --status-file "${tmpdir}/status.json" >/dev/null
rc-update show default | awk '{ print $1 }' | grep -qx routerd_smoke
rc-service routerd_smoke status >/dev/null

routerd apply --config "$config" --once --status-file "${tmpdir}/status-2.json" >/dev/null
rc-service routerd_smoke status >/dev/null
