...
ROUTERD_VRRP_VMAC_BIN := $(BUILDDIR)/routerd-vrrp-vmac
...
build-daemons:
	...
	$(GO_BUILD_ENV) go build $(GO_BUILD_FLAGS) -o $(ROUTERD_VRRP_VMAC_BIN) ./cmd/routerd-vrrp-vmac
...
dist:
	...
	install -m 0755 $(ROUTERD_VRRP_VMAC_BIN) $(DISTROOT)/bin/routerd-vrrp-vmac
